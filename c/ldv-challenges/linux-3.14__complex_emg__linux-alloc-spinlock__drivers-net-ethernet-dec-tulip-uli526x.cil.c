...
struct net_device {
   ...
   enum ldv_28419 reg_state : 8 ;
   bool dismantle ;
   enum ldv_28420 rtnl_link_state : 16 ;
...
{ 
  ...
  {
  dev = ldv_alloc_etherdev_mqs_85(632, 1U, 1U);
  }
...
{ 
  ...
  db->cr0_data = 0U;
  uli526x_init(dev);
  ret = ldv_request_irq_90((db->pdev)->irq, & uli526x_interrupt, 128UL, (char const   *)(& dev->name),
                           (void *)dev);
  }
  if (ret != 0) {
    return (ret);
...
{ 
  ...
  {
  ldv___ldv_spin_lock_91(& db->lock);
  }
  if (db->tx_packet_cnt > 29UL) {
    {
    ldv_spin_unlock_irqrestore_92(& db->lock, flags);
...
{ 
  ...
  ioaddr = db->ioaddr;
  netif_stop_queue(dev);
  ldv_del_timer_sync_94(& db->timer);
  iowrite32(1U, ioaddr);
  __const_udelay(21475UL);
  (*(db->phy.write))(db, (int )db->phy_addr, 0, 32768);
  ldv_free_irq_95((db->pdev)->irq, (void *)dev);
...
{ 
  ...
  db = (struct uli526x_board_info *)tmp;
  ioaddr = db->ioaddr;
  ldv___ldv_spin_lock_96(& db->lock);
  iowrite32(0U, ioaddr + 56UL);
...
{ 
  ...
  {
  ldv___ldv_spin_lock_100(& db->lock);
  }
...
{ 
  ...
  ioaddr = db->ioaddr;
  tmp_cr12 = 0U;
  ldv___ldv_spin_lock_104(& db->lock);
...
{ 
  ...
  ldv_44004: 
  {
  tmp = ldv___pci_register_driver_107(& uli526x_driver, & __this_module, "uli526x");
  }
  return (tmp);
}
}
...
{ 
  ...
  {
  ldv_pci_unregister_driver_108(& uli526x_driver);
  }
  return;
}
}
...
{ 
  ...
  {
  {
  ldv_assert("linux:kernel:locking:spinlock::one thread:double lock", ldv_spin_lock_of_uli526x_board_info == 1);
...
{ 
  ...
  {
  {
  ldv_assert("linux:kernel:locking:spinlock::one thread:double unlock", ldv_spin_lock_of_uli526x_board_info == 2);
...
{ 
  ...
  {
  {
  ldv_assert("linux:kernel:locking:spinlock::one thread:double lock try", ldv_spin_lock_of_uli526x_board_info == 1);
  ldv_assume(ldv_spin_lock_of_uli526x_board_info == 1);
  is_spin_held_by_another_thread = ldv_undef_int();
  }
  if (is_spin_held_by_another_thread != 0) {
...
{ 
  ...
  ldv_assert("linux:kernel:locking:spinlock::one thread:locked at exit", ldv_spin_lock_of_uli526x_board_info == 1);
...
#include "model/linux-3.14__complex_emg__linux-alloc-spinlock__drivers-net-ethernet-dec-tulip-uli526x_true-unreach-call.cil.env.c"
