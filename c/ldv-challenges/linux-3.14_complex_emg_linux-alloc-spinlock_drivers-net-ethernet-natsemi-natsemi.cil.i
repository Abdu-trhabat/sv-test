...
struct net_device {
   ...
   enum ldv_28547 reg_state : 8 ;
   bool dismantle ;
   enum ldv_28548 rtnl_link_state : 16 ;
...
__inline static void ldv_spin_lock_irq_91(spinlock_t *lock ) ;
__inline static void ldv_spin_lock_irq_91(spinlock_t *lock ) ;
__inline static void ldv_spin_lock_irq_91(spinlock_t *lock ) ;
__inline static void ldv_spin_lock_irq_91(spinlock_t *lock ) ;
__inline static void ldv_spin_lock_irq_91(spinlock_t *lock ) ;
...
__inline static void ldv_spin_unlock_irq_92(spinlock_t *lock ) ;
__inline static void ldv_spin_unlock_irq_92(spinlock_t *lock ) ;
__inline static void ldv_spin_unlock_irq_92(spinlock_t *lock ) ;
__inline static void ldv_spin_unlock_irq_92(spinlock_t *lock ) ;
__inline static void ldv_spin_unlock_irq_92(spinlock_t *lock ) ;
...
static int ldv_del_timer_sync_134(struct timer_list *ldv_func_arg1 ) ;
...
static void ldv_free_netdev_141(struct net_device *ldv_func_arg1 ) ;
...
static void ldv_unregister_netdev_140(struct net_device *ldv_func_arg1 ) ;
...
static int natsemi_probe1(struct pci_dev *pdev , struct pci_device_id const *ent )
{
  ...
  irq = (int )pdev->irq;
  pci_set_master(pdev);
  dev = ldv_alloc_etherdev_mqs_85(1368, 1U, 1U);
  }
...
static int netdev_open(struct net_device *dev )
{
  ...
  natsemi_reset(dev);
  i = ldv_request_irq_89((unsigned int )irq, & intr_handler, 128UL, (char const *)(& dev->name),
                         (void *)dev);
  }
  if (i != 0) {
    return (i);
  ...
  if (i < 0) {
    {
    ldv_free_irq_90((unsigned int )irq, (void *)dev);
    }
    return (i);
  } else {
  }
...
static void netdev_timer(unsigned long data )
{
  ...
  if (next_tick > 1) {
    ...
    ldv_mod_timer_101(& np->timer, tmp___3);
    }
  } else {
    {
    ldv_mod_timer_102(& np->timer, (unsigned long )jiffies + (unsigned long )next_tick);
    }
  }
  return;
}
...
static int natsemi_poll(struct napi_struct *napi , int budget )
{
  ...
  if ((np->intr_status & 960U) != 0U) {
    {
    ldv_spin_lock_107(& np->lock);
    netdev_tx_done(dev);
    ldv_spin_unlock_108(& np->lock);
    }
  } else {
  }
...
static void netdev_rx(struct net_device *dev , int *work_done , int work_to_do )
{
  ...
  *work_done = *work_done + 1;
  ...
  if (np->oom != 0) {
    {
    ldv_mod_timer_113(& np->timer, (unsigned long )jiffies + 1UL);
    }
  } else {
    {
    writel(4U, (void volatile *)ioaddr);
...
static int netdev_close(struct net_device *dev )
{
  ...
  ldv_del_timer_sync_134(& np->timer);
  disable_irq((unsigned int )irq);
  ldv_spin_lock_irq_91(& np->lock);
  natsemi_irq_disable(dev);
  np->hands_off = 1;
  ldv_spin_unlock_irq_92(& np->lock);
  enable_irq((unsigned int )irq);
  ldv_free_irq_137((unsigned int )irq, (void *)dev);
  ldv_spin_lock_irq_91(& np->lock);
  np->hands_off = 0;
...
static void natsemi_remove1(struct pci_dev *pdev )
{
  ...
  ldv_unregister_netdev_140(dev);
  pci_release_regions(pdev);
  iounmap((void volatile *)ioaddr);
  ldv_free_netdev_141(dev);
  }
  return;
}
}
static int natsemi_suspend(struct pci_dev *pdev , pm_message_t state )
{
  ...
  if ((int )tmp___3) {
    {
    irq = (int const )(np->pci_dev)->irq;
    ldv_del_timer_sync_142(& np->timer);
    disable_irq((unsigned int )irq);
    ldv_spin_lock_irq_91(& np->lock);
    natsemi_irq_disable(dev);
    np->hands_off = 1;
...
static int natsemi_resume(struct pci_dev *pdev )
{
  ...
  if ((int )tmp___4) {
    ...
    ldv_mod_timer_147(& np->timer, tmp___3);
    }
  } else {
  }
  {
...
static int natsemi_init_mod(void)
{
  ...
  tmp = ldv___pci_register_driver_148(& natsemi_driver, & __this_module, "natsemi");
  }
  return (tmp);
}
}
static void natsemi_exit_mod(void)
{
  {
  {
  ldv_pci_unregister_driver_149(& natsemi_driver);
  }
  return;
}
}
...
void reach_error() { ((void) sizeof ((0) ? 1 : 0), __extension__ ({ if (0) ; else __assert_fail ("0", "linux-3.14_complex_emg_linux-alloc-spinlock_drivers-net-ethernet-natsemi-natsemi.cil.c", 15511, __extension__ __PRETTY_FUNCTION__); })); }
void ldv_assert_linux_alloc_spinlock__nonatomic(int expr )
{
  {
  if (! expr) {
