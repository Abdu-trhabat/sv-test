...
static int skfp_open(struct net_device *dev )
{
  ...
  {
  err = ldv_request_irq_19((unsigned int )dev->irq, & skfp_interrupt, 128UL, (char const *)(& dev->name),
                           (void *)dev);
  }
  if (err != 0) {
    return (err);
...
static int skfddi_pci_driver_init(void)
{
  ...
  {
  {
  tmp = ldv___pci_register_driver_21(& skfddi_pci_driver, & __this_module, "skfp");
  }
  return (tmp);
}
}
static void skfddi_pci_driver_exit(void)
{
  {
  {
  ldv_pci_unregister_driver_22(& skfddi_pci_driver);
  }
  return;
}
}
...
void reach_error() { ((void) sizeof ((0) ? 1 : 0), __extension__ ({ if (0) ; else __assert_fail ("0", "linux-3.14_complex_emg_linux-kernel-locking-mutex_drivers-net-fddi-skfp-skfp.cil.c", 26290, __extension__ __PRETTY_FUNCTION__); })); }
void ldv_assert_linux_kernel_locking_mutex__one_thread_double_lock(int expr )
{
  {
  if (! expr) {
