...
static void zoran_remove(struct pci_dev *pdev )
{
  ...
  {
  {
  tmp = ldv_dev_get_drvdata_18((struct device const *)(& pdev->dev));
  ...
  zoran_set_pci_master(zr, 0);
  writel(0U, (void volatile *)zr->zr36057_mem + 40U);
  ldv_free_irq_19((zr->pci_dev)->irq, (void *)zr);
  kfree((void const *)zr->stat_com);
...
static int zoran_probe(struct pci_dev *pdev , struct pci_device_id const *ent )
{
  ...
  {
  result = ldv_request_irq_20((zr->pci_dev)->irq, & zoran_irq, 128UL, (char const *)(& zr->name),
                              (void *)zr);
  }
  if (result < 0) {
    if (result == -22) {
  ...
  {
  writel(0U, (void volatile *)zr->zr36057_mem + 40U);
  ldv_free_irq_21((zr->pci_dev)->irq, (void *)zr);
  }
  zr_unmap:
  {
  iounmap((void volatile *)zr->zr36057_mem);
...
static int zoran_init(void)
{
  ...
  {
  res = ldv___pci_register_driver_22(& zoran_driver, & __this_module, "zr36067");
  }
  if (res != 0) {
    if (zr36067_debug > 0) {
      {
...
static void zoran_exit(void)
{
  {
  {
  ldv_pci_unregister_driver_23(& zoran_driver);
  }
  return;
}
}
...
void reach_error() { ((void) sizeof ((0) ? 1 : 0), __extension__ ({ if (0) ; else __assert_fail ("0", "linux-3.14_complex_emg_linux-drivers-clk1_drivers-media-pci-zoran-zr36067.cil.c", 26522, __extension__ __PRETTY_FUNCTION__); })); }
void ldv_assert_linux_drivers_clk1__more_at_exit(int expr )
{
  {
  if (! expr) {
