...
struct net_device {
   ...
   enum ldv_28001 reg_state : 8 ;
   bool dismantle ;
   enum ldv_28002 rtnl_link_state : 16 ;
...
static int ldv_del_timer_sync_43(struct timer_list *ldv_func_arg1 ) ;
...
static void ldv_free_netdev_46(struct net_device *ldv_func_arg1 ) ;
...
static int ldv_register_netdev_45(struct net_device *ldv_func_arg1 ) ;
extern void unregister_netdev(struct net_device * ) ;
static void ldv_unregister_netdev_47(struct net_device *ldv_func_arg1 ) ;
...
static int ldv___pci_register_driver_49(struct pci_driver *ldv_func_arg1 , struct module *ldv_func_arg2 ,
                                        char const *ldv_func_arg3 ) ;
extern void pci_unregister_driver(struct pci_driver * ) ;
static void ldv_pci_unregister_driver_50(struct pci_driver *ldv_func_arg1 ) ;
...
static void ldv_free_irq_32(unsigned int ldv_func_arg1 , void *ldv_func_arg2 ) ;
...
static int nv_change_mtu(struct net_device *dev , int new_mtu )
{
  ...
  if ((int )tmp___3) {
    ...
    if (tmp___1 != 0) {
      if (np->in_shutdown == 0) {
        {
        ldv_mod_timer_17(& np->oom_kick, (unsigned long )jiffies + 13UL);
        }
      } else {
      }
    } else {
...
static irqreturn_t nv_nic_irq_tx(int foo , void *data )
{
  ...
  if (tmp___6 != 0L) {
    ...
    if (np->in_shutdown == 0) {
      ...
      ldv_mod_timer_18(& np->nic_poll, (unsigned long )jiffies + 3UL);
      }
    } else {
    }
    {
...
static int nv_napi_poll(struct napi_struct *napi , int budget )
{
  ...
  if (retcode != 0) {
    ...
    if (np->in_shutdown == 0) {
      {
      ldv_mod_timer_19(& np->oom_kick, (unsigned long )jiffies + 13UL);
      }
    } else {
    }
    {
  ...
  if (tmp___17 != 0L) {
    ...
    if (np->in_shutdown == 0) {
      ...
      np->recover_error = 1;
      ldv_mod_timer_20(& np->nic_poll, (unsigned long )jiffies + 3UL);
      }
    } else {
    }
    {
...
static irqreturn_t nv_nic_irq_rx(int foo , void *data )
{
  ...
  if (tmp___6 != 0) {
    ...
    if (tmp___5 != 0L) {
      ...
      if (np->in_shutdown == 0) {
        {
        ldv_mod_timer_21(& np->oom_kick, (unsigned long )jiffies + 13UL);
        }
      } else {
      }
      {
  ...
  if (tmp___9 != 0L) {
    ...
    if (np->in_shutdown == 0) {
      ...
      ldv_mod_timer_22(& np->nic_poll, (unsigned long )jiffies + 3UL);
      }
    } else {
    }
    {
...
static irqreturn_t nv_nic_irq_other(int foo , void *data )
{
  ...
  if (tmp___9 != 0L) {
    ...
    if (np->in_shutdown == 0) {
      {
      np->nic_poll_irq = np->nic_poll_irq | 33376U;
      ldv_mod_timer_24(& np->nic_poll, (unsigned long )jiffies + 3UL);
      }
    } else {
    }
    {
...
static int nv_request_irq(struct net_device *dev , int intr_test )
{
  ...
  if ((np->msi_flags & 32U) != 0U) {
    ...
    if (ret == 0) {
      ...
      if ((optimization_mode | intr_test) == 0) {
        ...
        tmp___2 = ldv_request_irq_25(np->msi_x_entry[0].vector, & nv_nic_irq_rx, 128UL,
        ...
        tmp___3 = ldv_request_irq_26(np->msi_x_entry[1].vector, & nv_nic_irq_tx, 128UL,
        ...
        tmp___4 = ldv_request_irq_27(np->msi_x_entry[2].vector, & nv_nic_irq_other,
        ...
        {
        tmp___5 = ldv_request_irq_28(np->msi_x_entry[0].vector, handler, 128UL, (char const *)(& dev->name),
                                     (void *)dev);
        }
        if (tmp___5 != 0) {
          {
  ...
  if (ret != 0 && (np->msi_flags & 16U) != 0U) {
    ...
    if (ret == 0) {
      ...
      tmp___6 = ldv_request_irq_29((np->pci_dev)->irq, handler, 128UL, (char const *)(& dev->name),
                                   (void *)dev);
      }
      if (tmp___6 != 0) {
        {
  ...
  if (ret != 0) {
    {
    tmp___7 = ldv_request_irq_30((np->pci_dev)->irq, handler, 128UL, (char const *)(& dev->name),
                                 (void *)dev);
    }
    if (tmp___7 != 0) {
      goto out_err;
  ...
  out_free_tx:
  {
  ldv_free_irq_31(np->msi_x_entry[1].vector, (void *)dev);
  }
  out_free_rx:
  {
  ldv_free_irq_32(np->msi_x_entry[0].vector, (void *)dev);
  }
  out_err: ;
  return (1);
}
...
static void nv_free_irq(struct net_device *dev )
{
  ...
  if ((np->msi_flags & 128U) != 0U) {
    ...
    ldv_44722:
    {
    ldv_free_irq_33(np->msi_x_entry[i].vector, (void *)dev);
    i = i + 1;
    }
    ldv_44723: ;
    if ((u32 )i < (np->msi_flags & 15U)) {
    ...
    {
    ldv_free_irq_34((np->pci_dev)->irq, (void *)dev);
    }
    if ((np->msi_flags & 64U) != 0U) {
      {
      pci_disable_msi(np->pci_dev);
...
static void nv_do_stats_poll(unsigned long data )
{
  ...
  if (np->in_shutdown == 0) {
    ...
    ldv_mod_timer_36(& np->stats_poll, tmp___1);
    }
  } else {
  }
  return;
...
static int nv_open(struct net_device *dev )
{
  ...
  if (oom != 0) {
    {
    ldv_mod_timer_39(& np->oom_kick, (unsigned long )jiffies + 13UL);
    }
  } else {
  }
  if ((np->driver_data & 3584U) != 0U) {
    ...
    ldv_mod_timer_40(& np->stats_poll, tmp___9);
    }
  } else {
  }
  {
...
static int nv_close(struct net_device *dev )
{
  ...
  ldv_del_timer_sync_41(& np->oom_kick);
  ldv_del_timer_sync_42(& np->nic_poll);
  ldv_del_timer_sync_43(& np->stats_poll);
  netif_stop_queue(dev);
  spin_lock_irq(& np->lock);
...
static int nv_probe(struct pci_dev *pci_dev , struct pci_device_id const *id )
{
  ...
  {
  dev = ldv_alloc_etherdev_mqs_44(3080, 1U, 1U);
  err = -12;
  }
  ...
  np->duplex = 0;
  np->autoneg = 1;
  err = ldv_register_netdev_45(dev);
  }
  if (err != 0) {
    {
...
static int forcedeth_pci_driver_init(void)
{
  ...
  {
  {
  tmp = ldv___pci_register_driver_49(& forcedeth_pci_driver, & __this_module, "forcedeth");
  }
  return (tmp);
}
}
static void forcedeth_pci_driver_exit(void)
{
  {
  {
  ldv_pci_unregister_driver_50(& forcedeth_pci_driver);
  }
  return;
}
}
...
void reach_error() { ((void) sizeof ((0) ? 1 : 0), __extension__ ({ if (0) ; else __assert_fail ("0", "linux-3.14_linux-kernel-locking-mutex_drivers-net-ethernet-nvidia-forcedeth.cil.c", 21541, __extension__ __PRETTY_FUNCTION__); })); }
void ldv_assert_linux_kernel_locking_mutex__one_thread_double_lock(int expr )
{
  {
  if (! expr) {
