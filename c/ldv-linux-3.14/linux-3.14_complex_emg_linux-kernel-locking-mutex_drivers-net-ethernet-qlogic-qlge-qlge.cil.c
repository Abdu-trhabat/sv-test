...
{ 
  ...
  if (intr_context->hooked != 0U) {
    ...
    if (tmp != 0) {
      {
      ldv_free_irq_15((qdev->msi_x_entry + (unsigned long )i)->vector, (void *)(& qdev->rx_ring) + (unsigned long )i);
      }
    } else {
      {
      ldv_free_irq_16((qdev->pdev)->irq, (void *)(& qdev->rx_ring));
      }
    }
  } else {
...
{ 
  ...
  if (tmp___0 != 0) {
    {
    status = ldv_request_irq_17((qdev->msi_x_entry + (unsigned long )i)->vector, intr_context->handler,
    ...
    {
    tmp = constant_test_bit(2L, (unsigned long const volatile   *)(& qdev->flags));
    status = ldv_request_irq_18(pdev->irq, & qlge_isr, tmp != 0 ? 0UL : 128UL, (char const   *)(& intr_context->name),
                                (void *)(& qdev->rx_ring));
    }
    if (status != 0) {
      goto err_irq;
...
{ 
  ...
  {
  ldv_mod_timer_19(& qdev->timer, (unsigned long )jiffies + 1250UL);
  }
  return;
}
}
...
{ 
  ...
  _min2___0 = tmp___0;
  ndev = ldv_alloc_etherdev_mqs_20(20152, (unsigned int )(_min1___0 < _min2___0 ? _min1___0 : _min2___0),
  ...
  err = ldv_register_netdev_22(ndev);
  }
  if (err != 0) {
    {
...
{ 
  ...
  tmp___0 = netdev_priv((struct net_device  const  *)ndev);
  qdev = (struct ql_adapter *)tmp___0;
  ldv_del_timer_sync_24(& qdev->timer);
  ql_cancel_all_work_sync(qdev);
...
{ 
  ...
  {
  ldv_mod_timer_28(& qdev->timer, (unsigned long )jiffies + 1250UL);
...
{ 
  ...
  qdev = (struct ql_adapter *)tmp___0;
  netif_device_detach(ndev);
  ldv_del_timer_sync_29(& qdev->timer);
  tmp___1 = netif_running((struct net_device  const  *)ndev);
  }
  if ((int )tmp___1) {
    {
...
{ 
  ...
  {
  ldv_mod_timer_30(& qdev->timer, (unsigned long )jiffies + 1250UL);
  netif_device_attach(ndev);
  }
  return (0);
}
...
{ 
  ...
  {
  {
  tmp = ldv___pci_register_driver_31(& qlge_driver, & __this_module, "qlge");
  }
  return (tmp);
}
}
...
{ 
  ...
  {
  {
  ldv_pci_unregister_driver_32(& qlge_driver);
  }
  return;
}
}
...
{ 
  ...
  {
  {
  ldv_mutex_lock_15(& qdev->mpi_mutex);
  ql_write32((struct ql_adapter  const  *)qdev, 56, 65536U);
  ...
  {
  ql_write32((struct ql_adapter  const  *)qdev, 56, 65537U);
  ldv_mutex_unlock_16(& qdev->mpi_mutex);
  }
  return (status);
}
}
...
{ 
  ...
  mbcp = & mbc;
  err = 0;
  ldv_mutex_lock_17(& qdev->mpi_mutex);
  ql_write32((struct ql_adapter  const  *)qdev, 56, 65536U);
  }
  goto ldv_44170;
  ldv_44169: 
  ...
  {
  ql_write32((struct ql_adapter  const  *)qdev, 56, 65537U);
  ldv_mutex_unlock_18(& qdev->mpi_mutex);
...
ldv_set LDV_MUTEXES_mpi_mutex_of_ql_adapter  ;
void ldv_mutex_lock_mpi_mutex_of_ql_adapter(struct mutex *lock ) 
{ 
  ...
  {
  {
  ldv_assert_linux_kernel_locking_mutex__one_thread_double_lock(! LDV_MUTEXES_mpi_mutex_of_ql_adapter);
  LDV_MUTEXES_mpi_mutex_of_ql_adapter = 1;
  }
  return;
}
}
int ldv_mutex_lock_interruptible_or_killable_mpi_mutex_of_ql_adapter(struct mutex *lock ) 
{ 
  int tmp ;
...
{ 
  ...
  {
  if ((int )LDV_MUTEXES_mpi_mutex_of_ql_adapter) {
    return (1);
  } else {
    {
    tmp = ldv_undef_int();
...
{ 
  ...
  {
  {
  ldv_assert_linux_kernel_locking_mutex__one_thread_double_lock_try(! LDV_MUTEXES_mpi_mutex_of_ql_adapter);
  tmp = ldv_mutex_is_locked_mpi_mutex_of_ql_adapter(lock);
  }
  if (tmp != 0) {
    return (0);
  } else {
...
{ 
  ...
  if (cnt->counter != 0) {
    ...
    {
    ldv_mutex_lock_mpi_mutex_of_ql_adapter(lock);
    }
    return (1);
  }
}
...
{ 
  ...
  {
  {
  ldv_assert_linux_kernel_locking_mutex__one_thread_double_unlock((int )LDV_MUTEXES_mpi_mutex_of_ql_adapter);
  LDV_MUTEXES_mpi_mutex_of_ql_adapter = 0;
  }
  return;
}
}
...
{ 
  ...
  ldv_assert_linux_kernel_locking_mutex__one_thread_locked_at_exit(! LDV_MUTEXES_mpi_mutex_of_ql_adapter);
...
#include "model/linux-3.14__complex_emg__linux-kernel-locking-mutex__drivers-net-ethernet-qlogic-qlge-qlge_true-unreach-call.cil.env.c"
