...
typedef enum ldv_21993 socket_state;
struct socket_wq {
   wait_queue_head_t wait ;
...
static int ldv_pskb_expand_head_113(struct sk_buff *ldv_func_arg1 , int ldv_func_arg2 ,
...
int igbvf_setup_tx_resources(struct igbvf_adapter *adapter , struct igbvf_ring *tx_ring )
{
  ...
  tmp = ldv_vzalloc_101((unsigned long )size);
...
int igbvf_setup_rx_resources(struct igbvf_adapter *adapter , struct igbvf_ring *rx_ring )
{
  ...
  tmp = ldv_vzalloc_102((unsigned long )size);
...
static irqreturn_t igbvf_msix_other(int irq , void *data )
{
  ...
  if (tmp___0 == 0) {
    {
    ldv_mod_timer_103(& adapter->watchdog_timer, (unsigned long )jiffies + 1UL);
    }
  } else {
  }
  {
...
static int igbvf_request_msix(struct igbvf_adapter *adapter )
{
  ...
  {
  err = ldv_request_irq_104((adapter->msix_entries + (unsigned long )vector)->vector,
  ...
  vector = vector + 1;
  err = ldv_request_irq_105((adapter->msix_entries + (unsigned long )vector)->vector,
  ...
  vector = vector + 1;
  err = ldv_request_irq_106((adapter->msix_entries + (unsigned long )vector)->vector,
...
static void igbvf_free_irq(struct igbvf_adapter *adapter )
{
  ...
  if ((unsigned long )adapter->msix_entries != (unsigned long )((struct msix_entry *)0)) {
    ...
    ldv_51270:
    {
    ldv_free_irq_107((adapter->msix_entries + (unsigned long )vector)->vector, (void *)netdev);
    vector = vector + 1;
    }
    ldv_51271: ;
    if (vector <= 2) {
...
int igbvf_up(struct igbvf_adapter *adapter )
{
  ...
  hw->mac.get_link_status = 1;
  ldv_mod_timer_109(& adapter->watchdog_timer, (unsigned long )jiffies + 1UL);
  }
  return (0);
}
}
void igbvf_down(struct igbvf_adapter *adapter )
{
  ...
  ldv_del_timer_sync_110(& adapter->watchdog_timer);
  netif_carrier_off(netdev);
  igbvf_update_stats(adapter);
...
static int igbvf_open(struct net_device *netdev )
{
  ...
  igbvf_irq_enable(adapter);
  hw->mac.get_link_status = 1;
  ldv_mod_timer_111(& adapter->watchdog_timer, (unsigned long )jiffies + 1UL);
  }
  return (0);
  err_req_irq:
  {
...
static void igbvf_watchdog_task(struct work_struct *work )
{
  ...
  if (tmp___6 == 0) {
    ...
    ldv_mod_timer_112(& adapter->watchdog_timer, tmp___5);
    }
  } else {
  }
  return;
                     ...
                     struct sk_buff *skb , u32 tx_flags , u8 *hdr_len )
{
  ...
  *hdr_len = 0U;
  ...
  if (tmp != 0) {
    {
    err = ldv_pskb_expand_head_113(skb, 0, 0, 32U);
    }
    if (err != 0) {
      {
...
static int igbvf_probe(struct pci_dev *pdev , struct pci_device_id const *ent )
{
  ...
  pci_set_master(pdev);
  err = -12;
  netdev = ldv_alloc_etherdev_mqs_114(2688, 1U, 1U);
  }
...
static int igbvf_init_module(void)
{
  ...
  ret = ldv___pci_register_driver_120(& igbvf_driver, & __this_module, "igbvf");
  }
  return (ret);
}
}
static void igbvf_exit_module(void)
{
  {
  {
  ldv_pci_unregister_driver_121(& igbvf_driver);
  }
  return;
}
}
...
void reach_error() { ((void) sizeof ((0) ? 1 : 0), __extension__ ({ if (0) ; else __assert_fail ("0", "linux-3.14_complex_emg_linux-alloc-spinlock_drivers-net-ethernet-intel-igbvf-igbvf.cil.c", 19093, __extension__ __PRETTY_FUNCTION__); })); }
void ldv_assert_linux_alloc_spinlock__nonatomic(int expr )
{
  {
  if (! expr) {
