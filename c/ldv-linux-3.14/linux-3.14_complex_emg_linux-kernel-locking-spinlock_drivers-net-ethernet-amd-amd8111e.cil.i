...
struct net_device {
   ...
   enum ldv_28384 reg_state : 8 ;
   bool dismantle ;
   enum ldv_28385 rtnl_link_state : 16 ;
...
static int amd8111e_rx_poll(struct napi_struct *napi , int budget )
{
  ...
  if (rx_pkt_limit > 0) {
    {
    ldv___ldv_spin_lock_60(& lp->lock);
    __napi_complete(napi);
    writel(129U, (void volatile *)mmio + 64U);
    writel(8454144U, (void volatile *)mmio + 72U);
    ldv_spin_unlock_irqrestore_61(& lp->lock, flags);
    }
  } else {
  }
  rx_not_empty: ;
...
static int amd8111e_close(struct net_device *dev )
{
  ...
  if ((lp->options & 128) != 0) {
    {
    ldv_del_timer_sync_67(& lp->ipg_data.ipg_timer);
    }
  } else {
  }
  {
  ldv_spin_unlock_irq_68(& lp->lock);
  ldv_free_irq_69((unsigned int )dev->irq, (void *)dev);
...
static int amd8111e_open(struct net_device *dev )
{
  ...
  if (dev->irq == 0) {
    ...
    {
    tmp___0 = ldv_request_irq_70((unsigned int )dev->irq, & amd8111e_interrupt, 128UL,
...
static int amd8111e_suspend(struct pci_dev *pci_dev , pm_message_t state )
{
  ...
  if ((lp->options & 128) != 0) {
    {
    ldv_del_timer_sync_96(& lp->ipg_data.ipg_timer);
    }
  } else {
  }
  {
...
static int amd8111e_resume(struct pci_dev *pci_dev )
{
  ...
  if ((lp->options & 128) != 0) {
    {
    ldv_mod_timer_99(& lp->ipg_data.ipg_timer, (unsigned long )jiffies + 125UL);
    }
  } else {
  }
  {
...
static void amd8111e_config_ipg(struct net_device *dev )
{
  ...
  {
  ldv_mod_timer_103(& lp->ipg_data.ipg_timer, (unsigned long )jiffies + 125UL);
  }
  return;
}
}
...
static int amd8111e_probe_one(struct pci_dev *pdev , struct pci_device_id const *ent )
{
  ...
  dev = ldv_alloc_etherdev_mqs_104(1760, 1U, 1U);
  }
  if ((unsigned long )dev == (unsigned long )((struct net_device *)0)) {
    err = -12;
    goto err_free_reg;
  ...
  *(dev->dev_addr + (unsigned long )i) = readb((void const volatile *)(lp->mmio + ((unsigned long )i + 352UL)));
  ...
  amd8111e_set_rx_buff_len(dev);
  err = ldv_register_netdev_105(dev);
  }
  if (err != 0) {
    {
...
static int amd8111e_driver_init(void)
{
  ...
  {
  {
  tmp = ldv___pci_register_driver_107(& amd8111e_driver, & __this_module, "amd8111e");
  }
  return (tmp);
}
}
static void amd8111e_driver_exit(void)
{
  {
  {
  ldv_pci_unregister_driver_108(& amd8111e_driver);
  }
  return;
}
}
...
void ldv_spin_lock_lock_of_amd8111e_priv(void)
{
  {
  {
  ldv_assert_linux_kernel_locking_spinlock__one_thread_double_lock(ldv_spin_lock_of_amd8111e_priv == 1);
...
void ldv_spin_unlock_lock_of_amd8111e_priv(void)
{
  {
  {
  ldv_assert_linux_kernel_locking_spinlock__one_thread_double_unlock(ldv_spin_lock_of_amd8111e_priv == 2);
...
int ldv_spin_trylock_lock_of_amd8111e_priv(void)
{
  ...
  {
  {
  ldv_assert_linux_kernel_locking_spinlock__one_thread_double_lock_try(ldv_spin_lock_of_amd8111e_priv == 1);
  ldv_assume(ldv_spin_lock_of_amd8111e_priv == 1);
  is_spin_held_by_another_thread = ldv_undef_int();
  }
  if (is_spin_held_by_another_thread != 0) {
...
void ldv_check_final_state(void)
{
  ...
  ldv_assert_linux_kernel_locking_spinlock__one_thread_locked_at_exit(ldv_spin_lock_of_amd8111e_priv == 1);
...
void reach_error() { ((void) sizeof ((0) ? 1 : 0), __extension__ ({ if (0) ; else __assert_fail ("0", "linux-3.14_complex_emg_linux-kernel-locking-spinlock_drivers-net-ethernet-amd-amd8111e.cil.c", 13139, __extension__ __PRETTY_FUNCTION__); })); }
void ldv_assert_linux_kernel_locking_spinlock__one_thread_double_lock(int expr )
{
  {
  if (! expr) {
