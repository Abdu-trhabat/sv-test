...
void ldv_spin_lock_adapter_lock_of_ql3_adapter(void) ;
void ldv_spin_unlock_adapter_lock_of_ql3_adapter(void) ;
void ldv_spin_lock_hw_lock_of_ql3_adapter(void) ;
void ldv_spin_unlock_hw_lock_of_ql3_adapter(void) ;
...
static u32 ql_read_common_reg_l(struct ql3_adapter *qdev , u32 *reg )
{
  ...
  {
  {
  ldv___ldv_spin_lock_60(& qdev->hw_lock);
  value = readl((void const volatile *)reg);
  ldv_spin_unlock_irqrestore_61(& qdev->hw_lock, hw_flags);
  }
  return (value);
}
}
...
static u32 ql_read_page0_reg_l(struct ql3_adapter *qdev , u32 *reg )
{
  ...
  {
  {
  ldv___ldv_spin_lock_62(& qdev->hw_lock);
  }
...
static void ql_write_common_reg_l(struct ql3_adapter *qdev , u32 *reg , u32 value )
{
  ...
  {
  {
  ldv___ldv_spin_lock_64(& qdev->hw_lock);
...
static int ql_get_nvram_params(struct ql3_adapter *qdev )
{
  ...
  {
  checksum = 0U;
  ldv___ldv_spin_lock_66(& qdev->hw_lock);
  ...
  checksum = (int )checksum + (int )*pEEPROMData;
                               ...
                               u32 phyAddr )
{
  ...
  {
  ql_write_page0_reg(qdev, & port_regs->macMIIMgmtAddrReg, phyAddr | (u32 )regAddr);
                                       ...
                                       u16 phyIdReg1 )
{
  ...
  goto ldv_45178;
  ldv_45177: ;
  if (oui == (u32 )PHY_DEVICES[i].phyIdOUI && (int )model == (int )((unsigned short )PHY_DEVICES[i].phyIdModel)) {
    {
...
static int ql_is_full_dup(struct ql3_adapter *qdev )
{
  ...
  return (((unsigned int )reg & 4224U) == 4224U);
  case_1: ;
  switch_default:
  {
  tmp___0 = ql_mii_read_reg(qdev, 28, & reg);
...
static void ql_link_state_machine_work(struct work_struct *work )
{
  ...
  qdev = (struct ql3_adapter *)__mptr + 0xfffffffffffeb2c8UL;
  ldv___ldv_spin_lock_70(& qdev->hw_lock);
  ...
  if (tmp != 0) {
    ...
    {
    ldv_spin_unlock_irqrestore_61(& qdev->hw_lock, hw_flags);
    ldv_mod_timer_72(& qdev->adapter_timer, (unsigned long )jiffies + 250UL);
    }
    return;
  } else {
  }
  {
  if (qdev->port_link_state == 1U) {
    goto case_1;
  } else {
  }
  ...
  {
  ldv_spin_unlock_irqrestore_61(& qdev->hw_lock, hw_flags);
  ldv_mod_timer_74(& qdev->adapter_timer, (unsigned long )jiffies + 250UL);
  }
  return;
}
}
...
static int ql_get_auto_cfg_status(struct ql3_adapter *qdev )
{
  ...
  {
  {
  ldv___ldv_spin_lock_75(& qdev->hw_lock);
  tmp = ql_sem_spinlock(qdev, 58720256U, (qdev->mac_index * 2U | 4U) << 7);
  }
  if (tmp != 0) {
    {
...
static u32 ql_get_speed(struct ql3_adapter *qdev )
{
  ...
  {
  {
  ldv___ldv_spin_lock_78(& qdev->hw_lock);
  tmp = ql_sem_spinlock(qdev, 58720256U, (qdev->mac_index * 2U | 4U) << 7);
  }
  if (tmp != 0) {
    {
...
static int ql_get_full_dup(struct ql3_adapter *qdev )
{
  ...
  {
  {
  ldv___ldv_spin_lock_81(& qdev->hw_lock);
  tmp = ql_sem_spinlock(qdev, 58720256U, (qdev->mac_index * 2U | 4U) << 7);
  }
  if (tmp != 0) {
    {
...
static void ql_process_macip_rx_intr(struct ql3_adapter *qdev , struct ib_ip_iocb_rsp *ib_ip_rsp_ptr )
{
  ...
  if (qdev->device_id == 12322U) {
    ...
    if (((int )checksum & 16) != 0 || ((unsigned int )checksum & 12U) == 8U) {
      skb2->ip_summed = 1U;
    } else {
    }
  }
                          ...
                          int work_to_do )
{
  ...
  if ((int )net_rsp->opcode == 33) {
    goto case_33;
  } else {
  }
  if ((int )net_rsp->opcode == 249) {
    goto case_249;
  } else {
  }
  if ((int )net_rsp->opcode == 9) {
    goto case_9;
  } else {
  }
  if ((int )net_rsp->opcode == 250) {
    goto case_250;
  } else {
  }
  if ((int )net_rsp->opcode == 10) {
    goto case_10;
  } else {
  }
  goto switch_default;
...
static int ql_poll(struct napi_struct *napi , int budget )
{
  ...
  if (tx_cleaned + rx_cleaned != budget) {
    {
    ldv___ldv_spin_lock_84(& qdev->hw_lock);
...
static irqreturn_t ql3xxx_isr(int irq , void *dev_id )
{
  ...
  if ((value & 16392U) != 0U) {
    {
    ldv_spin_lock_86(& qdev->adapter_lock);
    ...
    ldv_spin_unlock_87(& qdev->adapter_lock);
    }
  } else
  if ((value & 16U) != 0U) {
    {
...
static netdev_tx_t ql3xxx_send(struct sk_buff *skb , struct net_device *ndev )
{
  ...
  if (qdev->device_id == 12338U && (unsigned int )*((unsigned char *)skb + 124UL) == 12U) {
    {
...
static int ql_adapter_initialize(struct ql3_adapter *qdev )
{
  ...
  *((__le16 *)qdev->prsp_producer_index) = 0U;
  ...
  {
  ldv_spin_unlock_irq_88(& qdev->hw_lock);
  msleep(500U);
  ldv_spin_lock_irq_89(& qdev->hw_lock);
  delay = delay - 1U;
  }
  if (delay != 0U) {
    goto ldv_45651;
...
static void ql_set_mac_info(struct ql3_adapter *qdev )
{
  ...
  {
  if ((value & 1792U) == 1024U) {
    goto case_1024;
  } else {
  }
  if ((value & 1792U) == 1536U) {
    goto case_1536;
  } else {
  }
  if ((value & 1792U) == 1280U) {
    goto case_1280;
  } else {
  }
  if ((value & 1792U) == 1792U) {
    goto case_1792;
  } else {
  }
  goto switch_default;
...
static int ql_adapter_down(struct ql3_adapter *qdev , int do_reset )
{
  ...
  clear_bit(6L, (unsigned long volatile *)(& qdev->flags));
  ql_disable_interrupts(qdev);
  ldv_free_irq_90((qdev->pdev)->irq, (void *)ndev);
  }
  if (qdev->msi != 0U) {
    {
  ...
  {
  ldv_del_timer_sync_91(& qdev->adapter_timer);
  napi_disable(& qdev->napi);
  }
  if (do_reset != 0) {
    {
    ldv___ldv_spin_lock_92(& qdev->hw_lock);
...
static int ql_adapter_up(struct ql3_adapter *qdev )
{
  ...
  {
  err = ldv_request_irq_94((qdev->pdev)->irq, & ql3xxx_isr, irq_flags, (char const *)(& ndev->name),
                           (void *)ndev);
  }
  if (err != 0) {
    {
  ...
  {
  ldv___ldv_spin_lock_95(& qdev->hw_lock);
  ...
  ldv_mod_timer_97(& qdev->adapter_timer, (unsigned long )jiffies + 250UL);
  ...
  {
  ldv_spin_unlock_irqrestore_61(& qdev->hw_lock, hw_flags);
  ldv_free_irq_99((qdev->pdev)->irq, (void *)ndev);
  }
  err_irq: ;
  if (qdev->msi != 0U) {
    {
...
static int ql3xxx_set_mac_address(struct net_device *ndev , void *p )
{
  ...
  ldv___ldv_spin_lock_100(& qdev->hw_lock);
  ql_write_page0_reg(qdev, & port_regs->macAddrIndirectPtrReg, 196608U);
...
static void ql_reset_work(struct work_struct *work )
{
  ...
  {
  __mptr = (struct work_struct const *)work;
  qdev = (struct ql3_adapter *)__mptr + 0xfffffffffffeb488UL;
  ndev = qdev->ndev;
  port_regs = qdev->mem_map_registers;
  ...
  if (tmp != 0) {
    ...
    ldv___ldv_spin_lock_102(& qdev->hw_lock);
    ql_write_common_reg(qdev, & port_regs->CommonRegs.ispControlStatus, 524296U);
    max_wait_time = 10;
    }
    ldv_45747:
    ...
    ldv_spin_unlock_irqrestore_61(& qdev->hw_lock, hw_flags);
    ssleep(1U);
    ldv___ldv_spin_lock_104(& qdev->hw_lock);
    max_wait_time = max_wait_time - 1;
    }
    if (max_wait_time != 0) {
      goto ldv_45747;
...
static void ql_tx_timeout_work(struct work_struct *work )
{
  ...
  {
  __mptr = (struct work_struct const *)work;
  qdev = (struct ql3_adapter *)__mptr + 0xfffffffffffeb3a8UL;
  ql_cycle_adapter(qdev, 1);
  }
  return;
}
...
static int ql3xxx_probe(struct pci_dev *pdev , struct pci_device_id const *pci_entry )
{
  ...
  {
  ndev = ldv_alloc_etherdev_mqs_106(85544, 1U, 1U);
  }
  ...
  __raw_spin_lock_init(& qdev->adapter_lock.__annonCompField19.rlock, "&(&qdev->adapter_lock)->rlock",
                       & __key);
  ...
  {
  err = ldv_register_netdev_107(ndev);
  }
  if (err != 0) {
    {
  ...
  tmp___8 = __alloc_workqueue_key("%s", 10U, 1, & __key___1, __lock_name, (char *)(& ndev->name));
  ...
  init_timer_key(& qdev->reset_work.timer, 2U, "(&(&qdev->reset_work)->timer)", & __key___3);
  ...
  init_timer_key(& qdev->tx_timeout_work.timer, 2U, "(&(&qdev->tx_timeout_work)->timer)",
                 & __key___5);
  ...
  init_timer_key(& qdev->link_state_work.timer, 2U, "(&(&qdev->link_state_work)->timer)",
                 & __key___7);
  ...
  init_timer_key(& qdev->adapter_timer, 0U, "(&qdev->adapter_timer)", & __key___8);
...
static void ql3xxx_remove(struct pci_dev *pdev )
{
  ...
  ldv_unregister_netdev_109(ndev);
  ql_disable_interrupts(qdev);
  }
...
static int ql3xxx_driver_init(void)
{
  ...
  {
  {
  tmp = ldv___pci_register_driver_111(& ql3xxx_driver, & __this_module, "qla3xxx");
  }
  return (tmp);
}
}
static void ql3xxx_driver_exit(void)
{
  {
  {
  ldv_pci_unregister_driver_112(& ql3xxx_driver);
  }
  return;
}
}
void ldv_EMGentry_exit_ql3xxx_driver_exit_14_2(void (*arg0)(void) ) ;
int ldv_EMGentry_init_ql3xxx_driver_init_14_7(int (*arg0)(void) ) ;
...
void (*ldv_14_exit_ql3xxx_driver_exit_default)(void) ;
int (*ldv_14_init_ql3xxx_driver_init_default)(void) ;
...
int ldv_statevar_2 ;
int ldv_statevar_3 ;
enum irqreturn (*ldv_0_callback_handler)(int , void * ) = & ql3xxx_isr;
void (*ldv_14_exit_ql3xxx_driver_exit_default)(void) = & ql3xxx_driver_exit;
int (*ldv_14_init_ql3xxx_driver_init_default)(void) = & ql3xxx_driver_init;
...
int (*ldv_1_callback_ndo_set_mac_address)(struct net_device * , void * ) = & ql3xxx_set_mac_address;
enum netdev_tx (*ldv_1_callback_ndo_start_xmit)(struct sk_buff * , struct net_device * ) = & ql3xxx_send;
void (*ldv_1_callback_ndo_tx_timeout)(struct net_device * ) = & ql3xxx_tx_timeout;
                                                   ...
                                                   void *arg2 )
{
  {
  {
  ql3xxx_set_mac_address(arg1, arg2);
  }
  return;
}
}
...
void ldv_entry_EMGentry_14(void *arg0 )
{
  ...
  ldv_EMGentry_exit_ql3xxx_driver_exit_14_2(ldv_14_exit_ql3xxx_driver_exit_default);
  ...
  {
  ldv_assume(ldv_statevar_2 == 20);
  ldv_14_ret_default = ldv_EMGentry_init_ql3xxx_driver_init_14_7(ldv_14_init_ql3xxx_driver_init_default);
...
void ldv_net_dummy_resourceless_instance_1(void *arg0 )
{
  ...
  case_9:
  {
  ldv_dummy_resourceless_instance_callback_1_9(ldv_1_callback_get_pauseparam, ldv_1_container_net_device,
  ...
  case_19:
  {
  ldv_dummy_resourceless_instance_callback_1_18(ldv_1_callback_set_msglevel, ldv_1_container_net_device,
                                ...
                                struct pci_dev *arg1 , struct pci_device_id *arg2 )
{
  ...
  {
  {
  tmp = ql3xxx_probe(arg1, (struct pci_device_id const *)arg2);
  }
  return (tmp);
}
}
...
int ldv_register_netdev(int arg0 , struct net_device *arg1 )
{
  ...
  if (tmp___0 != 0) {
    ...
    ldv_assume((ldv_statevar_0 == 2 || ldv_statevar_0 == 6) || ldv_statevar_3 == 3);
...
static void ldv___ldv_spin_lock_60(spinlock_t *ldv_func_arg1 )
{
  {
  {
  ldv_spin_lock_hw_lock_of_ql3_adapter();
...
__inline static void ldv_spin_unlock_irqrestore_61(spinlock_t *lock , unsigned long flags )
{
  {
  {
  ldv_spin_unlock_hw_lock_of_ql3_adapter();
...
__inline static void ldv_spin_lock_86(spinlock_t *lock )
{
  {
  {
  ldv_spin_lock_adapter_lock_of_ql3_adapter();
  spin_lock(lock);
  }
  return;
}
...
__inline static void ldv_spin_unlock_87(spinlock_t *lock )
{
  {
  {
  ldv_spin_unlock_adapter_lock_of_ql3_adapter();
  spin_unlock(lock);
  }
  return;
}
...
static int ldv_spin_adapter_lock_of_ql3_adapter = 1;
void ldv_spin_lock_adapter_lock_of_ql3_adapter(void)
{
  {
  {
  ldv_assert_linux_kernel_locking_spinlock__one_thread_double_lock(ldv_spin_adapter_lock_of_ql3_adapter == 1);
  ldv_assume(ldv_spin_adapter_lock_of_ql3_adapter == 1);
  ldv_spin_adapter_lock_of_ql3_adapter = 2;
  }
  return;
}
}
void ldv_spin_unlock_adapter_lock_of_ql3_adapter(void)
{
  {
  {
  ldv_assert_linux_kernel_locking_spinlock__one_thread_double_unlock(ldv_spin_adapter_lock_of_ql3_adapter == 2);
  ldv_assume(ldv_spin_adapter_lock_of_ql3_adapter == 2);
  ldv_spin_adapter_lock_of_ql3_adapter = 1;
  }
  return;
}
}
int ldv_spin_trylock_adapter_lock_of_ql3_adapter(void)
{
  int is_spin_held_by_another_thread ;
  {
  {
  ldv_assert_linux_kernel_locking_spinlock__one_thread_double_lock_try(ldv_spin_adapter_lock_of_ql3_adapter == 1);
  ldv_assume(ldv_spin_adapter_lock_of_ql3_adapter == 1);
  is_spin_held_by_another_thread = ldv_undef_int();
  }
  if (is_spin_held_by_another_thread != 0) {
...
void ldv_spin_unlock_wait_adapter_lock_of_ql3_adapter(void)
{
  {
  {
  ldv_assert_linux_kernel_locking_spinlock__one_thread_double_lock_try(ldv_spin_adapter_lock_of_ql3_adapter == 1);
...
int ldv_spin_is_locked_adapter_lock_of_ql3_adapter(void)
{
  int is_spin_held_by_another_thread ;
  {
  {
  ...
  if (ldv_spin_adapter_lock_of_ql3_adapter == 1 && is_spin_held_by_another_thread == 0) {
    return (0);
  } else {
    return (1);
  }
...
int ldv_spin_can_lock_adapter_lock_of_ql3_adapter(void)
{
  int tmp ;
  {
  {
  tmp = ldv_spin_is_locked_adapter_lock_of_ql3_adapter();
  }
  return (tmp == 0);
}
}
int ldv_spin_is_contended_adapter_lock_of_ql3_adapter(void)
{
  int is_spin_contended ;
  {
  {
...
int ldv_atomic_dec_and_lock_adapter_lock_of_ql3_adapter(void)
{
  int atomic_value_after_dec ;
  {
  {
...
static int ldv_spin_hw_lock_of_ql3_adapter = 1;
void ldv_spin_lock_hw_lock_of_ql3_adapter(void)
{
  {
  {
  ldv_assert_linux_kernel_locking_spinlock__one_thread_double_lock(ldv_spin_hw_lock_of_ql3_adapter == 1);
  ldv_assume(ldv_spin_hw_lock_of_ql3_adapter == 1);
  ldv_spin_hw_lock_of_ql3_adapter = 2;
  }
  return;
}
}
void ldv_spin_unlock_hw_lock_of_ql3_adapter(void)
{
  {
  {
  ldv_assert_linux_kernel_locking_spinlock__one_thread_double_unlock(ldv_spin_hw_lock_of_ql3_adapter == 2);
  ldv_assume(ldv_spin_hw_lock_of_ql3_adapter == 2);
  ldv_spin_hw_lock_of_ql3_adapter = 1;
  }
  return;
}
}
int ldv_spin_trylock_hw_lock_of_ql3_adapter(void)
{
  int is_spin_held_by_another_thread ;
  {
  {
  ldv_assert_linux_kernel_locking_spinlock__one_thread_double_lock_try(ldv_spin_hw_lock_of_ql3_adapter == 1);
  ldv_assume(ldv_spin_hw_lock_of_ql3_adapter == 1);
  is_spin_held_by_another_thread = ldv_undef_int();
  }
  if (is_spin_held_by_another_thread != 0) {
...
void ldv_spin_unlock_wait_hw_lock_of_ql3_adapter(void)
{
  {
  {
  ldv_assert_linux_kernel_locking_spinlock__one_thread_double_lock_try(ldv_spin_hw_lock_of_ql3_adapter == 1);
...
int ldv_spin_is_locked_hw_lock_of_ql3_adapter(void)
{
  int is_spin_held_by_another_thread ;
  {
  {
  ...
  if (ldv_spin_hw_lock_of_ql3_adapter == 1 && is_spin_held_by_another_thread == 0) {
    return (0);
  } else {
    return (1);
  }
...
int ldv_spin_can_lock_hw_lock_of_ql3_adapter(void)
{
  int tmp ;
  {
  {
  tmp = ldv_spin_is_locked_hw_lock_of_ql3_adapter();
  }
  return (tmp == 0);
}
}
int ldv_spin_is_contended_hw_lock_of_ql3_adapter(void)
{
  int is_spin_contended ;
  {
  {
...
int ldv_atomic_dec_and_lock_hw_lock_of_ql3_adapter(void)
{
  int atomic_value_after_dec ;
  {
  {
...
void ldv_check_final_state(void)
{
  ...
  ldv_assert_linux_kernel_locking_spinlock__one_thread_locked_at_exit(ldv_spin_adapter_lock_of_ql3_adapter == 1);
  ...
  ldv_assert_linux_kernel_locking_spinlock__one_thread_locked_at_exit(ldv_spin_hw_lock_of_ql3_adapter == 1);
...
int ldv_exclusive_spin_is_locked(void)
{
  ...
  if (ldv_spin_adapter_lock_of_ql3_adapter == 2) {
    return (1);
  } else {
  }
  ...
  if (ldv_spin_hw_lock_of_ql3_adapter == 2) {
    return (1);
  } else {
  }
...
void reach_error() { ((void) sizeof ((0) ? 1 : 0), __extension__ ({ if (0) ; else __assert_fail ("0", "linux-3.14_complex_emg_linux-kernel-locking-spinlock_drivers-net-ethernet-qlogic-qla3xxx.cil.c", 16390, __extension__ __PRETTY_FUNCTION__); })); }
void ldv_assert_linux_kernel_locking_spinlock__one_thread_double_lock(int expr )
{
  {
  if (! expr) {
