// This file is part of the SV-Benchmarks collection of verification tasks:
// https://gitlab.com/sosy-lab/benchmarking/sv-benchmarks
//
// SPDX-FileCopyrightText: 2023 Levente Bajczi, Department of Measurement and Information Systems, Budapest University of Technology and Economics
//
// SPDX-License-Identifier: Apache-2.0

// Original CHC file: llreve-bench/loop__while_after_while_if_000.smt2
extern void abort ();
extern int __VERIFIER_nondet_int ();
extern _Bool __VERIFIER_nondet__Bool ();
extern void __assert_fail(const char *, const char *, unsigned int, const char *) __attribute__ ((__nothrow__ , __leaf__)) __attribute__ ((__noreturn__));
void reach_error() { __assert_fail("0", "loop__while_after_while_if_000_range.c", 13, "reach_error"); }



int main ();

int
main ()
{
    // return parameter


    // variables
    int INV_MAIN_23_0;
    int INV_MAIN_23_1;
    int INV_MAIN_23_2;
    int INV_MAIN_23_3;
    int INV_MAIN_23_4;
    int INV_MAIN_13_0;
    int INV_MAIN_13_1;
    int INV_MAIN_13_2;
    int INV_MAIN_13_3;
    int INV_MAIN_42_0;
    int INV_MAIN_42_1;
    int INV_MAIN_42_2;
    int INV_MAIN_42_3;
    int INV_MAIN_42_4;
    int INV_MAIN_42_5;
    int INV_MAIN_42_6;
    int A_0;
    int B_0;
    int C_0;
    int D_0;
    int E_0;
    int F_0;
    int v_6_0;
    int A_1;
    int B_1;
    int C_1;
    int D_1;
    int E_1;
    int F_1;
    int v_6_1;
    int v_7_1;
    int A_2;
    int B_2;
    int C_2;
    int D_2;
    int E_2;
    int F_2;
    int G_2;
    int H_2;
    int A_3;
    int B_3;
    int C_3;
    int D_3;
    int E_3;
    int F_3;
    int A_4;
    int B_4;
    int C_4;
    int D_4;
    int E_4;
    int F_4;
    int A_5;
    int B_5;
    int C_5;
    int D_5;
    int E_5;
    int v_5_5;
    int A_6;
    int B_6;
    int C_6;
    int D_6;
    int E_6;
    int F_6;
    int G_6;
    int A_7;
    int B_7;
    int C_7;
    int D_7;
    int E_7;
    int F_7;
    int G_7;
    int A_8;
    int B_8;
    int C_8;
    int D_8;
    int E_8;
    int F_8;
    int G_8;
    int H_8;
    int I_8;
    int J_8;
    int K_8;
    int A_9;
    int B_9;
    int C_9;
    int D_9;
    int E_9;
    int F_9;
    int G_9;
    int H_9;
    int I_9;
    int A_10;
    int B_10;
    int C_10;
    int D_10;
    int E_10;
    int F_10;
    int G_10;
    int H_10;
    int I_10;
    int A_11;
    int B_11;
    int C_11;
    int D_11;

    if (((INV_MAIN_23_0 <= -1000000000) || (INV_MAIN_23_0 >= 1000000000))
        || ((INV_MAIN_23_1 <= -1000000000) || (INV_MAIN_23_1 >= 1000000000))
        || ((INV_MAIN_23_2 <= -1000000000) || (INV_MAIN_23_2 >= 1000000000))
        || ((INV_MAIN_23_3 <= -1000000000) || (INV_MAIN_23_3 >= 1000000000))
        || ((INV_MAIN_23_4 <= -1000000000) || (INV_MAIN_23_4 >= 1000000000))
        || ((INV_MAIN_13_0 <= -1000000000) || (INV_MAIN_13_0 >= 1000000000))
        || ((INV_MAIN_13_1 <= -1000000000) || (INV_MAIN_13_1 >= 1000000000))
        || ((INV_MAIN_13_2 <= -1000000000) || (INV_MAIN_13_2 >= 1000000000))
        || ((INV_MAIN_13_3 <= -1000000000) || (INV_MAIN_13_3 >= 1000000000))
        || ((INV_MAIN_42_0 <= -1000000000) || (INV_MAIN_42_0 >= 1000000000))
        || ((INV_MAIN_42_1 <= -1000000000) || (INV_MAIN_42_1 >= 1000000000))
        || ((INV_MAIN_42_2 <= -1000000000) || (INV_MAIN_42_2 >= 1000000000))
        || ((INV_MAIN_42_3 <= -1000000000) || (INV_MAIN_42_3 >= 1000000000))
        || ((INV_MAIN_42_4 <= -1000000000) || (INV_MAIN_42_4 >= 1000000000))
        || ((INV_MAIN_42_5 <= -1000000000) || (INV_MAIN_42_5 >= 1000000000))
        || ((INV_MAIN_42_6 <= -1000000000) || (INV_MAIN_42_6 >= 1000000000))
        || ((A_0 <= -1000000000) || (A_0 >= 1000000000))
        || ((B_0 <= -1000000000) || (B_0 >= 1000000000))
        || ((C_0 <= -1000000000) || (C_0 >= 1000000000))
        || ((D_0 <= -1000000000) || (D_0 >= 1000000000))
        || ((E_0 <= -1000000000) || (E_0 >= 1000000000))
        || ((F_0 <= -1000000000) || (F_0 >= 1000000000))
        || ((v_6_0 <= -1000000000) || (v_6_0 >= 1000000000))
        || ((A_1 <= -1000000000) || (A_1 >= 1000000000))
        || ((B_1 <= -1000000000) || (B_1 >= 1000000000))
        || ((C_1 <= -1000000000) || (C_1 >= 1000000000))
        || ((D_1 <= -1000000000) || (D_1 >= 1000000000))
        || ((E_1 <= -1000000000) || (E_1 >= 1000000000))
        || ((F_1 <= -1000000000) || (F_1 >= 1000000000))
        || ((v_6_1 <= -1000000000) || (v_6_1 >= 1000000000))
        || ((v_7_1 <= -1000000000) || (v_7_1 >= 1000000000))
        || ((A_2 <= -1000000000) || (A_2 >= 1000000000))
        || ((B_2 <= -1000000000) || (B_2 >= 1000000000))
        || ((C_2 <= -1000000000) || (C_2 >= 1000000000))
        || ((D_2 <= -1000000000) || (D_2 >= 1000000000))
        || ((E_2 <= -1000000000) || (E_2 >= 1000000000))
        || ((F_2 <= -1000000000) || (F_2 >= 1000000000))
        || ((G_2 <= -1000000000) || (G_2 >= 1000000000))
        || ((H_2 <= -1000000000) || (H_2 >= 1000000000))
        || ((A_3 <= -1000000000) || (A_3 >= 1000000000))
        || ((B_3 <= -1000000000) || (B_3 >= 1000000000))
        || ((C_3 <= -1000000000) || (C_3 >= 1000000000))
        || ((D_3 <= -1000000000) || (D_3 >= 1000000000))
        || ((E_3 <= -1000000000) || (E_3 >= 1000000000))
        || ((F_3 <= -1000000000) || (F_3 >= 1000000000))
        || ((A_4 <= -1000000000) || (A_4 >= 1000000000))
        || ((B_4 <= -1000000000) || (B_4 >= 1000000000))
        || ((C_4 <= -1000000000) || (C_4 >= 1000000000))
        || ((D_4 <= -1000000000) || (D_4 >= 1000000000))
        || ((E_4 <= -1000000000) || (E_4 >= 1000000000))
        || ((F_4 <= -1000000000) || (F_4 >= 1000000000))
        || ((A_5 <= -1000000000) || (A_5 >= 1000000000))
        || ((B_5 <= -1000000000) || (B_5 >= 1000000000))
        || ((C_5 <= -1000000000) || (C_5 >= 1000000000))
        || ((D_5 <= -1000000000) || (D_5 >= 1000000000))
        || ((E_5 <= -1000000000) || (E_5 >= 1000000000))
        || ((v_5_5 <= -1000000000) || (v_5_5 >= 1000000000))
        || ((A_6 <= -1000000000) || (A_6 >= 1000000000))
        || ((B_6 <= -1000000000) || (B_6 >= 1000000000))
        || ((C_6 <= -1000000000) || (C_6 >= 1000000000))
        || ((D_6 <= -1000000000) || (D_6 >= 1000000000))
        || ((E_6 <= -1000000000) || (E_6 >= 1000000000))
        || ((F_6 <= -1000000000) || (F_6 >= 1000000000))
        || ((G_6 <= -1000000000) || (G_6 >= 1000000000))
        || ((A_7 <= -1000000000) || (A_7 >= 1000000000))
        || ((B_7 <= -1000000000) || (B_7 >= 1000000000))
        || ((C_7 <= -1000000000) || (C_7 >= 1000000000))
        || ((D_7 <= -1000000000) || (D_7 >= 1000000000))
        || ((E_7 <= -1000000000) || (E_7 >= 1000000000))
        || ((F_7 <= -1000000000) || (F_7 >= 1000000000))
        || ((G_7 <= -1000000000) || (G_7 >= 1000000000))
        || ((A_8 <= -1000000000) || (A_8 >= 1000000000))
        || ((B_8 <= -1000000000) || (B_8 >= 1000000000))
        || ((C_8 <= -1000000000) || (C_8 >= 1000000000))
        || ((D_8 <= -1000000000) || (D_8 >= 1000000000))
        || ((E_8 <= -1000000000) || (E_8 >= 1000000000))
        || ((F_8 <= -1000000000) || (F_8 >= 1000000000))
        || ((G_8 <= -1000000000) || (G_8 >= 1000000000))
        || ((H_8 <= -1000000000) || (H_8 >= 1000000000))
        || ((I_8 <= -1000000000) || (I_8 >= 1000000000))
        || ((J_8 <= -1000000000) || (J_8 >= 1000000000))
        || ((K_8 <= -1000000000) || (K_8 >= 1000000000))
        || ((A_9 <= -1000000000) || (A_9 >= 1000000000))
        || ((B_9 <= -1000000000) || (B_9 >= 1000000000))
        || ((C_9 <= -1000000000) || (C_9 >= 1000000000))
        || ((D_9 <= -1000000000) || (D_9 >= 1000000000))
        || ((E_9 <= -1000000000) || (E_9 >= 1000000000))
        || ((F_9 <= -1000000000) || (F_9 >= 1000000000))
        || ((G_9 <= -1000000000) || (G_9 >= 1000000000))
        || ((H_9 <= -1000000000) || (H_9 >= 1000000000))
        || ((I_9 <= -1000000000) || (I_9 >= 1000000000))
        || ((A_10 <= -1000000000) || (A_10 >= 1000000000))
        || ((B_10 <= -1000000000) || (B_10 >= 1000000000))
        || ((C_10 <= -1000000000) || (C_10 >= 1000000000))
        || ((D_10 <= -1000000000) || (D_10 >= 1000000000))
        || ((E_10 <= -1000000000) || (E_10 >= 1000000000))
        || ((F_10 <= -1000000000) || (F_10 >= 1000000000))
        || ((G_10 <= -1000000000) || (G_10 >= 1000000000))
        || ((H_10 <= -1000000000) || (H_10 >= 1000000000))
        || ((I_10 <= -1000000000) || (I_10 >= 1000000000))
        || ((A_11 <= -1000000000) || (A_11 >= 1000000000))
        || ((B_11 <= -1000000000) || (B_11 >= 1000000000))
        || ((C_11 <= -1000000000) || (C_11 >= 1000000000))
        || ((D_11 <= -1000000000) || (D_11 >= 1000000000)))
        abort ();

    // main logic
    goto main_init;

  main_init:
    switch (__VERIFIER_nondet__Bool ())
      {
      case 0:
          if (!
              ((B_0 == E_0) && (A_0 == D_0) && (A_0 <= 0) && (C_0 == F_0)
               && (0 == v_6_0)))
              abort ();
          INV_MAIN_23_0 = C_0;
          INV_MAIN_23_1 = E_0;
          INV_MAIN_23_2 = F_0;
          INV_MAIN_23_3 = D_0;
          INV_MAIN_23_4 = v_6_0;
          A_6 = __VERIFIER_nondet_int ();
          if (((A_6 <= -1000000000) || (A_6 >= 1000000000)))
              abort ();
          B_6 = __VERIFIER_nondet_int ();
          if (((B_6 <= -1000000000) || (B_6 >= 1000000000)))
              abort ();
          C_6 = INV_MAIN_23_0;
          D_6 = INV_MAIN_23_1;
          E_6 = INV_MAIN_23_2;
          F_6 = INV_MAIN_23_3;
          G_6 = INV_MAIN_23_4;
          if (!
              ((B_6 == (D_6 + -1)) && (!(D_6 <= 0))
               && (A_6 == ((F_6 <= 0) ? G_6 : (G_6 + 1)))))
              abort ();
          INV_MAIN_23_0 = C_6;
          INV_MAIN_23_1 = B_6;
          INV_MAIN_23_2 = E_6;
          INV_MAIN_23_3 = F_6;
          INV_MAIN_23_4 = A_6;
          goto INV_MAIN_23_2;

      case 1:
          if (!
              ((B_1 == E_1) && (A_1 == D_1) && (!(A_1 <= 0)) && (C_1 == F_1)
               && (0 == v_6_1) && (0 == v_7_1)))
              abort ();
          INV_MAIN_42_0 = B_1;
          INV_MAIN_42_1 = C_1;
          INV_MAIN_42_2 = v_6_1;
          INV_MAIN_42_3 = E_1;
          INV_MAIN_42_4 = F_1;
          INV_MAIN_42_5 = D_1;
          INV_MAIN_42_6 = v_7_1;
          A_9 = __VERIFIER_nondet_int ();
          if (((A_9 <= -1000000000) || (A_9 >= 1000000000)))
              abort ();
          B_9 = __VERIFIER_nondet_int ();
          if (((B_9 <= -1000000000) || (B_9 >= 1000000000)))
              abort ();
          C_9 = INV_MAIN_42_0;
          D_9 = INV_MAIN_42_1;
          E_9 = INV_MAIN_42_2;
          F_9 = INV_MAIN_42_3;
          G_9 = INV_MAIN_42_4;
          H_9 = INV_MAIN_42_5;
          I_9 = INV_MAIN_42_6;
          if (!
              ((B_9 == (C_9 + -1)) && (!(C_9 <= 0)) && (F_9 <= 0)
               && (A_9 == (E_9 + 1))))
              abort ();
          INV_MAIN_42_0 = B_9;
          INV_MAIN_42_1 = D_9;
          INV_MAIN_42_2 = A_9;
          INV_MAIN_42_3 = F_9;
          INV_MAIN_42_4 = G_9;
          INV_MAIN_42_5 = H_9;
          INV_MAIN_42_6 = I_9;
          goto INV_MAIN_42_6;

      default:
          abort ();
      }
  main_error:
    reach_error ();
  main_final:
    goto main_final;
  INV_MAIN_13_0:
    switch (__VERIFIER_nondet_int ())
      {
      case 0:
          A_11 = INV_MAIN_13_0;
          B_11 = INV_MAIN_13_1;
          C_11 = INV_MAIN_13_2;
          D_11 = INV_MAIN_13_3;
          if (!((C_11 <= 0) && (A_11 <= 0) && (!(B_11 == D_11))))
              abort ();
          goto main_error;

      case 1:
          A_2 = __VERIFIER_nondet_int ();
          if (((A_2 <= -1000000000) || (A_2 >= 1000000000)))
              abort ();
          B_2 = __VERIFIER_nondet_int ();
          if (((B_2 <= -1000000000) || (B_2 >= 1000000000)))
              abort ();
          C_2 = __VERIFIER_nondet_int ();
          if (((C_2 <= -1000000000) || (C_2 >= 1000000000)))
              abort ();
          D_2 = __VERIFIER_nondet_int ();
          if (((D_2 <= -1000000000) || (D_2 >= 1000000000)))
              abort ();
          E_2 = INV_MAIN_13_0;
          F_2 = INV_MAIN_13_1;
          G_2 = INV_MAIN_13_2;
          H_2 = INV_MAIN_13_3;
          if (!
              ((B_2 == (G_2 + -1)) && (D_2 == (E_2 + -1))
               && (C_2 == (F_2 + 2)) && (!(G_2 <= 0)) && (!(E_2 <= 0))
               && (A_2 == (H_2 + 2))))
              abort ();
          INV_MAIN_13_0 = D_2;
          INV_MAIN_13_1 = C_2;
          INV_MAIN_13_2 = B_2;
          INV_MAIN_13_3 = A_2;
          A_4 = __VERIFIER_nondet_int ();
          if (((A_4 <= -1000000000) || (A_4 >= 1000000000)))
              abort ();
          B_4 = __VERIFIER_nondet_int ();
          if (((B_4 <= -1000000000) || (B_4 >= 1000000000)))
              abort ();
          C_4 = INV_MAIN_13_0;
          D_4 = INV_MAIN_13_1;
          E_4 = INV_MAIN_13_2;
          F_4 = INV_MAIN_13_3;
          if (!
              ((A_4 == (F_4 + 2)) && (!(E_4 <= 0)) && (C_4 <= 0)
               && (B_4 == (E_4 + -1))))
              abort ();
          INV_MAIN_13_0 = C_4;
          INV_MAIN_13_1 = D_4;
          INV_MAIN_13_2 = B_4;
          INV_MAIN_13_3 = A_4;
          goto INV_MAIN_13_4;

      case 2:
          A_4 = __VERIFIER_nondet_int ();
          if (((A_4 <= -1000000000) || (A_4 >= 1000000000)))
              abort ();
          B_4 = __VERIFIER_nondet_int ();
          if (((B_4 <= -1000000000) || (B_4 >= 1000000000)))
              abort ();
          C_4 = INV_MAIN_13_0;
          D_4 = INV_MAIN_13_1;
          E_4 = INV_MAIN_13_2;
          F_4 = INV_MAIN_13_3;
          if (!
              ((A_4 == (F_4 + 2)) && (!(E_4 <= 0)) && (C_4 <= 0)
               && (B_4 == (E_4 + -1))))
              abort ();
          INV_MAIN_13_0 = C_4;
          INV_MAIN_13_1 = D_4;
          INV_MAIN_13_2 = B_4;
          INV_MAIN_13_3 = A_4;
          A_4 = __VERIFIER_nondet_int ();
          if (((A_4 <= -1000000000) || (A_4 >= 1000000000)))
              abort ();
          B_4 = __VERIFIER_nondet_int ();
          if (((B_4 <= -1000000000) || (B_4 >= 1000000000)))
              abort ();
          C_4 = INV_MAIN_13_0;
          D_4 = INV_MAIN_13_1;
          E_4 = INV_MAIN_13_2;
          F_4 = INV_MAIN_13_3;
          if (!
              ((A_4 == (F_4 + 2)) && (!(E_4 <= 0)) && (C_4 <= 0)
               && (B_4 == (E_4 + -1))))
              abort ();
          INV_MAIN_13_0 = C_4;
          INV_MAIN_13_1 = D_4;
          INV_MAIN_13_2 = B_4;
          INV_MAIN_13_3 = A_4;
          goto INV_MAIN_13_4;

      case 3:
          A_4 = __VERIFIER_nondet_int ();
          if (((A_4 <= -1000000000) || (A_4 >= 1000000000)))
              abort ();
          B_4 = __VERIFIER_nondet_int ();
          if (((B_4 <= -1000000000) || (B_4 >= 1000000000)))
              abort ();
          C_4 = INV_MAIN_13_0;
          D_4 = INV_MAIN_13_1;
          E_4 = INV_MAIN_13_2;
          F_4 = INV_MAIN_13_3;
          if (!
              ((A_4 == (F_4 + 2)) && (!(E_4 <= 0)) && (C_4 <= 0)
               && (B_4 == (E_4 + -1))))
              abort ();
          INV_MAIN_13_0 = C_4;
          INV_MAIN_13_1 = D_4;
          INV_MAIN_13_2 = B_4;
          INV_MAIN_13_3 = A_4;
          goto INV_MAIN_13_4;

      default:
          abort ();
      }
  INV_MAIN_42_1:
    switch (__VERIFIER_nondet_int ())
      {
      case 0:
          A_7 = INV_MAIN_42_0;
          B_7 = INV_MAIN_42_1;
          C_7 = INV_MAIN_42_2;
          D_7 = INV_MAIN_42_3;
          E_7 = INV_MAIN_42_4;
          F_7 = INV_MAIN_42_5;
          G_7 = INV_MAIN_42_6;
          if (!((D_7 <= 0) && (A_7 <= 0)))
              abort ();
          INV_MAIN_13_0 = B_7;
          INV_MAIN_13_1 = C_7;
          INV_MAIN_13_2 = E_7;
          INV_MAIN_13_3 = G_7;
          A_4 = __VERIFIER_nondet_int ();
          if (((A_4 <= -1000000000) || (A_4 >= 1000000000)))
              abort ();
          B_4 = __VERIFIER_nondet_int ();
          if (((B_4 <= -1000000000) || (B_4 >= 1000000000)))
              abort ();
          C_4 = INV_MAIN_13_0;
          D_4 = INV_MAIN_13_1;
          E_4 = INV_MAIN_13_2;
          F_4 = INV_MAIN_13_3;
          if (!
              ((A_4 == (F_4 + 2)) && (!(E_4 <= 0)) && (C_4 <= 0)
               && (B_4 == (E_4 + -1))))
              abort ();
          INV_MAIN_13_0 = C_4;
          INV_MAIN_13_1 = D_4;
          INV_MAIN_13_2 = B_4;
          INV_MAIN_13_3 = A_4;
          goto INV_MAIN_13_4;

      case 1:
          A_8 = __VERIFIER_nondet_int ();
          if (((A_8 <= -1000000000) || (A_8 >= 1000000000)))
              abort ();
          B_8 = __VERIFIER_nondet_int ();
          if (((B_8 <= -1000000000) || (B_8 >= 1000000000)))
              abort ();
          C_8 = __VERIFIER_nondet_int ();
          if (((C_8 <= -1000000000) || (C_8 >= 1000000000)))
              abort ();
          D_8 = __VERIFIER_nondet_int ();
          if (((D_8 <= -1000000000) || (D_8 >= 1000000000)))
              abort ();
          E_8 = INV_MAIN_42_0;
          F_8 = INV_MAIN_42_1;
          G_8 = INV_MAIN_42_2;
          H_8 = INV_MAIN_42_3;
          I_8 = INV_MAIN_42_4;
          J_8 = INV_MAIN_42_5;
          K_8 = INV_MAIN_42_6;
          if (!
              ((B_8 == (H_8 + -1)) && (C_8 == (G_8 + 1))
               && (D_8 == (E_8 + -1)) && (!(E_8 <= 0)) && (!(H_8 <= 0))
               && (A_8 == ((J_8 <= 0) ? K_8 : (K_8 + 1)))))
              abort ();
          INV_MAIN_42_0 = D_8;
          INV_MAIN_42_1 = F_8;
          INV_MAIN_42_2 = C_8;
          INV_MAIN_42_3 = B_8;
          INV_MAIN_42_4 = I_8;
          INV_MAIN_42_5 = J_8;
          INV_MAIN_42_6 = A_8;
          A_9 = __VERIFIER_nondet_int ();
          if (((A_9 <= -1000000000) || (A_9 >= 1000000000)))
              abort ();
          B_9 = __VERIFIER_nondet_int ();
          if (((B_9 <= -1000000000) || (B_9 >= 1000000000)))
              abort ();
          C_9 = INV_MAIN_42_0;
          D_9 = INV_MAIN_42_1;
          E_9 = INV_MAIN_42_2;
          F_9 = INV_MAIN_42_3;
          G_9 = INV_MAIN_42_4;
          H_9 = INV_MAIN_42_5;
          I_9 = INV_MAIN_42_6;
          if (!
              ((B_9 == (C_9 + -1)) && (!(C_9 <= 0)) && (F_9 <= 0)
               && (A_9 == (E_9 + 1))))
              abort ();
          INV_MAIN_42_0 = B_9;
          INV_MAIN_42_1 = D_9;
          INV_MAIN_42_2 = A_9;
          INV_MAIN_42_3 = F_9;
          INV_MAIN_42_4 = G_9;
          INV_MAIN_42_5 = H_9;
          INV_MAIN_42_6 = I_9;
          goto INV_MAIN_42_6;

      case 2:
          A_9 = __VERIFIER_nondet_int ();
          if (((A_9 <= -1000000000) || (A_9 >= 1000000000)))
              abort ();
          B_9 = __VERIFIER_nondet_int ();
          if (((B_9 <= -1000000000) || (B_9 >= 1000000000)))
              abort ();
          C_9 = INV_MAIN_42_0;
          D_9 = INV_MAIN_42_1;
          E_9 = INV_MAIN_42_2;
          F_9 = INV_MAIN_42_3;
          G_9 = INV_MAIN_42_4;
          H_9 = INV_MAIN_42_5;
          I_9 = INV_MAIN_42_6;
          if (!
              ((B_9 == (C_9 + -1)) && (!(C_9 <= 0)) && (F_9 <= 0)
               && (A_9 == (E_9 + 1))))
              abort ();
          INV_MAIN_42_0 = B_9;
          INV_MAIN_42_1 = D_9;
          INV_MAIN_42_2 = A_9;
          INV_MAIN_42_3 = F_9;
          INV_MAIN_42_4 = G_9;
          INV_MAIN_42_5 = H_9;
          INV_MAIN_42_6 = I_9;
          A_9 = __VERIFIER_nondet_int ();
          if (((A_9 <= -1000000000) || (A_9 >= 1000000000)))
              abort ();
          B_9 = __VERIFIER_nondet_int ();
          if (((B_9 <= -1000000000) || (B_9 >= 1000000000)))
              abort ();
          C_9 = INV_MAIN_42_0;
          D_9 = INV_MAIN_42_1;
          E_9 = INV_MAIN_42_2;
          F_9 = INV_MAIN_42_3;
          G_9 = INV_MAIN_42_4;
          H_9 = INV_MAIN_42_5;
          I_9 = INV_MAIN_42_6;
          if (!
              ((B_9 == (C_9 + -1)) && (!(C_9 <= 0)) && (F_9 <= 0)
               && (A_9 == (E_9 + 1))))
              abort ();
          INV_MAIN_42_0 = B_9;
          INV_MAIN_42_1 = D_9;
          INV_MAIN_42_2 = A_9;
          INV_MAIN_42_3 = F_9;
          INV_MAIN_42_4 = G_9;
          INV_MAIN_42_5 = H_9;
          INV_MAIN_42_6 = I_9;
          goto INV_MAIN_42_6;

      case 3:
          A_9 = __VERIFIER_nondet_int ();
          if (((A_9 <= -1000000000) || (A_9 >= 1000000000)))
              abort ();
          B_9 = __VERIFIER_nondet_int ();
          if (((B_9 <= -1000000000) || (B_9 >= 1000000000)))
              abort ();
          C_9 = INV_MAIN_42_0;
          D_9 = INV_MAIN_42_1;
          E_9 = INV_MAIN_42_2;
          F_9 = INV_MAIN_42_3;
          G_9 = INV_MAIN_42_4;
          H_9 = INV_MAIN_42_5;
          I_9 = INV_MAIN_42_6;
          if (!
              ((B_9 == (C_9 + -1)) && (!(C_9 <= 0)) && (F_9 <= 0)
               && (A_9 == (E_9 + 1))))
              abort ();
          INV_MAIN_42_0 = B_9;
          INV_MAIN_42_1 = D_9;
          INV_MAIN_42_2 = A_9;
          INV_MAIN_42_3 = F_9;
          INV_MAIN_42_4 = G_9;
          INV_MAIN_42_5 = H_9;
          INV_MAIN_42_6 = I_9;
          goto INV_MAIN_42_6;

      default:
          abort ();
      }
  INV_MAIN_23_2:
    switch (__VERIFIER_nondet__Bool ())
      {
      case 0:
          A_6 = __VERIFIER_nondet_int ();
          if (((A_6 <= -1000000000) || (A_6 >= 1000000000)))
              abort ();
          B_6 = __VERIFIER_nondet_int ();
          if (((B_6 <= -1000000000) || (B_6 >= 1000000000)))
              abort ();
          C_6 = INV_MAIN_23_0;
          D_6 = INV_MAIN_23_1;
          E_6 = INV_MAIN_23_2;
          F_6 = INV_MAIN_23_3;
          G_6 = INV_MAIN_23_4;
          if (!
              ((B_6 == (D_6 + -1)) && (!(D_6 <= 0))
               && (A_6 == ((F_6 <= 0) ? G_6 : (G_6 + 1)))))
              abort ();
          INV_MAIN_23_0 = C_6;
          INV_MAIN_23_1 = B_6;
          INV_MAIN_23_2 = E_6;
          INV_MAIN_23_3 = F_6;
          INV_MAIN_23_4 = A_6;
          goto INV_MAIN_23_2;

      case 1:
          v_5_5 = __VERIFIER_nondet_int ();
          if (((v_5_5 <= -1000000000) || (v_5_5 >= 1000000000)))
              abort ();
          A_5 = INV_MAIN_23_0;
          B_5 = INV_MAIN_23_1;
          C_5 = INV_MAIN_23_2;
          D_5 = INV_MAIN_23_3;
          E_5 = INV_MAIN_23_4;
          if (!((B_5 <= 0) && (0 == v_5_5)))
              abort ();
          INV_MAIN_13_0 = A_5;
          INV_MAIN_13_1 = v_5_5;
          INV_MAIN_13_2 = C_5;
          INV_MAIN_13_3 = E_5;
          A_4 = __VERIFIER_nondet_int ();
          if (((A_4 <= -1000000000) || (A_4 >= 1000000000)))
              abort ();
          B_4 = __VERIFIER_nondet_int ();
          if (((B_4 <= -1000000000) || (B_4 >= 1000000000)))
              abort ();
          C_4 = INV_MAIN_13_0;
          D_4 = INV_MAIN_13_1;
          E_4 = INV_MAIN_13_2;
          F_4 = INV_MAIN_13_3;
          if (!
              ((A_4 == (F_4 + 2)) && (!(E_4 <= 0)) && (C_4 <= 0)
               && (B_4 == (E_4 + -1))))
              abort ();
          INV_MAIN_13_0 = C_4;
          INV_MAIN_13_1 = D_4;
          INV_MAIN_13_2 = B_4;
          INV_MAIN_13_3 = A_4;
          goto INV_MAIN_13_4;

      default:
          abort ();
      }
  INV_MAIN_13_3:
    switch (__VERIFIER_nondet__Bool ())
      {
      case 0:
          A_3 = __VERIFIER_nondet_int ();
          if (((A_3 <= -1000000000) || (A_3 >= 1000000000)))
              abort ();
          B_3 = __VERIFIER_nondet_int ();
          if (((B_3 <= -1000000000) || (B_3 >= 1000000000)))
              abort ();
          C_3 = INV_MAIN_13_0;
          D_3 = INV_MAIN_13_1;
          E_3 = INV_MAIN_13_2;
          F_3 = INV_MAIN_13_3;
          if (!
              ((A_3 == (D_3 + 2)) && (E_3 <= 0) && (!(C_3 <= 0))
               && (B_3 == (C_3 + -1))))
              abort ();
          INV_MAIN_13_0 = B_3;
          INV_MAIN_13_1 = A_3;
          INV_MAIN_13_2 = E_3;
          INV_MAIN_13_3 = F_3;
          goto INV_MAIN_13_0;

      case 1:
          A_4 = __VERIFIER_nondet_int ();
          if (((A_4 <= -1000000000) || (A_4 >= 1000000000)))
              abort ();
          B_4 = __VERIFIER_nondet_int ();
          if (((B_4 <= -1000000000) || (B_4 >= 1000000000)))
              abort ();
          C_4 = INV_MAIN_13_0;
          D_4 = INV_MAIN_13_1;
          E_4 = INV_MAIN_13_2;
          F_4 = INV_MAIN_13_3;
          if (!
              ((A_4 == (F_4 + 2)) && (!(E_4 <= 0)) && (C_4 <= 0)
               && (B_4 == (E_4 + -1))))
              abort ();
          INV_MAIN_13_0 = C_4;
          INV_MAIN_13_1 = D_4;
          INV_MAIN_13_2 = B_4;
          INV_MAIN_13_3 = A_4;
          goto INV_MAIN_13_4;

      default:
          abort ();
      }
  INV_MAIN_13_4:
    switch (__VERIFIER_nondet__Bool ())
      {
      case 0:
          A_2 = __VERIFIER_nondet_int ();
          if (((A_2 <= -1000000000) || (A_2 >= 1000000000)))
              abort ();
          B_2 = __VERIFIER_nondet_int ();
          if (((B_2 <= -1000000000) || (B_2 >= 1000000000)))
              abort ();
          C_2 = __VERIFIER_nondet_int ();
          if (((C_2 <= -1000000000) || (C_2 >= 1000000000)))
              abort ();
          D_2 = __VERIFIER_nondet_int ();
          if (((D_2 <= -1000000000) || (D_2 >= 1000000000)))
              abort ();
          E_2 = INV_MAIN_13_0;
          F_2 = INV_MAIN_13_1;
          G_2 = INV_MAIN_13_2;
          H_2 = INV_MAIN_13_3;
          if (!
              ((B_2 == (G_2 + -1)) && (D_2 == (E_2 + -1))
               && (C_2 == (F_2 + 2)) && (!(G_2 <= 0)) && (!(E_2 <= 0))
               && (A_2 == (H_2 + 2))))
              abort ();
          INV_MAIN_13_0 = D_2;
          INV_MAIN_13_1 = C_2;
          INV_MAIN_13_2 = B_2;
          INV_MAIN_13_3 = A_2;
          goto INV_MAIN_13_3;

      case 1:
          A_4 = __VERIFIER_nondet_int ();
          if (((A_4 <= -1000000000) || (A_4 >= 1000000000)))
              abort ();
          B_4 = __VERIFIER_nondet_int ();
          if (((B_4 <= -1000000000) || (B_4 >= 1000000000)))
              abort ();
          C_4 = INV_MAIN_13_0;
          D_4 = INV_MAIN_13_1;
          E_4 = INV_MAIN_13_2;
          F_4 = INV_MAIN_13_3;
          if (!
              ((A_4 == (F_4 + 2)) && (!(E_4 <= 0)) && (C_4 <= 0)
               && (B_4 == (E_4 + -1))))
              abort ();
          INV_MAIN_13_0 = C_4;
          INV_MAIN_13_1 = D_4;
          INV_MAIN_13_2 = B_4;
          INV_MAIN_13_3 = A_4;
          goto INV_MAIN_13_4;

      default:
          abort ();
      }
  INV_MAIN_42_5:
    switch (__VERIFIER_nondet__Bool ())
      {
      case 0:
          A_10 = __VERIFIER_nondet_int ();
          if (((A_10 <= -1000000000) || (A_10 >= 1000000000)))
              abort ();
          B_10 = __VERIFIER_nondet_int ();
          if (((B_10 <= -1000000000) || (B_10 >= 1000000000)))
              abort ();
          C_10 = INV_MAIN_42_0;
          D_10 = INV_MAIN_42_1;
          E_10 = INV_MAIN_42_2;
          F_10 = INV_MAIN_42_3;
          G_10 = INV_MAIN_42_4;
          H_10 = INV_MAIN_42_5;
          I_10 = INV_MAIN_42_6;
          if (!
              ((B_10 == (F_10 + -1)) && (C_10 <= 0) && (!(F_10 <= 0))
               && (A_10 == ((H_10 <= 0) ? I_10 : (I_10 + 1)))))
              abort ();
          INV_MAIN_42_0 = C_10;
          INV_MAIN_42_1 = D_10;
          INV_MAIN_42_2 = E_10;
          INV_MAIN_42_3 = B_10;
          INV_MAIN_42_4 = G_10;
          INV_MAIN_42_5 = H_10;
          INV_MAIN_42_6 = A_10;
          goto INV_MAIN_42_1;

      case 1:
          A_9 = __VERIFIER_nondet_int ();
          if (((A_9 <= -1000000000) || (A_9 >= 1000000000)))
              abort ();
          B_9 = __VERIFIER_nondet_int ();
          if (((B_9 <= -1000000000) || (B_9 >= 1000000000)))
              abort ();
          C_9 = INV_MAIN_42_0;
          D_9 = INV_MAIN_42_1;
          E_9 = INV_MAIN_42_2;
          F_9 = INV_MAIN_42_3;
          G_9 = INV_MAIN_42_4;
          H_9 = INV_MAIN_42_5;
          I_9 = INV_MAIN_42_6;
          if (!
              ((B_9 == (C_9 + -1)) && (!(C_9 <= 0)) && (F_9 <= 0)
               && (A_9 == (E_9 + 1))))
              abort ();
          INV_MAIN_42_0 = B_9;
          INV_MAIN_42_1 = D_9;
          INV_MAIN_42_2 = A_9;
          INV_MAIN_42_3 = F_9;
          INV_MAIN_42_4 = G_9;
          INV_MAIN_42_5 = H_9;
          INV_MAIN_42_6 = I_9;
          goto INV_MAIN_42_6;

      default:
          abort ();
      }
  INV_MAIN_42_6:
    switch (__VERIFIER_nondet__Bool ())
      {
      case 0:
          A_8 = __VERIFIER_nondet_int ();
          if (((A_8 <= -1000000000) || (A_8 >= 1000000000)))
              abort ();
          B_8 = __VERIFIER_nondet_int ();
          if (((B_8 <= -1000000000) || (B_8 >= 1000000000)))
              abort ();
          C_8 = __VERIFIER_nondet_int ();
          if (((C_8 <= -1000000000) || (C_8 >= 1000000000)))
              abort ();
          D_8 = __VERIFIER_nondet_int ();
          if (((D_8 <= -1000000000) || (D_8 >= 1000000000)))
              abort ();
          E_8 = INV_MAIN_42_0;
          F_8 = INV_MAIN_42_1;
          G_8 = INV_MAIN_42_2;
          H_8 = INV_MAIN_42_3;
          I_8 = INV_MAIN_42_4;
          J_8 = INV_MAIN_42_5;
          K_8 = INV_MAIN_42_6;
          if (!
              ((B_8 == (H_8 + -1)) && (C_8 == (G_8 + 1))
               && (D_8 == (E_8 + -1)) && (!(E_8 <= 0)) && (!(H_8 <= 0))
               && (A_8 == ((J_8 <= 0) ? K_8 : (K_8 + 1)))))
              abort ();
          INV_MAIN_42_0 = D_8;
          INV_MAIN_42_1 = F_8;
          INV_MAIN_42_2 = C_8;
          INV_MAIN_42_3 = B_8;
          INV_MAIN_42_4 = I_8;
          INV_MAIN_42_5 = J_8;
          INV_MAIN_42_6 = A_8;
          goto INV_MAIN_42_5;

      case 1:
          A_9 = __VERIFIER_nondet_int ();
          if (((A_9 <= -1000000000) || (A_9 >= 1000000000)))
              abort ();
          B_9 = __VERIFIER_nondet_int ();
          if (((B_9 <= -1000000000) || (B_9 >= 1000000000)))
              abort ();
          C_9 = INV_MAIN_42_0;
          D_9 = INV_MAIN_42_1;
          E_9 = INV_MAIN_42_2;
          F_9 = INV_MAIN_42_3;
          G_9 = INV_MAIN_42_4;
          H_9 = INV_MAIN_42_5;
          I_9 = INV_MAIN_42_6;
          if (!
              ((B_9 == (C_9 + -1)) && (!(C_9 <= 0)) && (F_9 <= 0)
               && (A_9 == (E_9 + 1))))
              abort ();
          INV_MAIN_42_0 = B_9;
          INV_MAIN_42_1 = D_9;
          INV_MAIN_42_2 = A_9;
          INV_MAIN_42_3 = F_9;
          INV_MAIN_42_4 = G_9;
          INV_MAIN_42_5 = H_9;
          INV_MAIN_42_6 = I_9;
          goto INV_MAIN_42_6;

      default:
          abort ();
      }

    // return expression

}

