// This file is part of the SV-Benchmarks collection of verification tasks:
// https://gitlab.com/sosy-lab/benchmarking/sv-benchmarks
//
// SPDX-FileCopyrightText: 2023 Levente Bajczi, Department of Measurement and Information Systems, Budapest University of Technology and Economics
//
// SPDX-License-Identifier: Apache-2.0

// Original CHC file: tricera-benchmarks/s3_srvr_1a.cil.c-1.smt2.gz_000.smt2
extern void abort ();
extern int __VERIFIER_nondet_int ();
extern _Bool __VERIFIER_nondet__Bool ();
extern void __assert_fail(const char *, const char *, unsigned int, const char *) __attribute__ ((__nothrow__ , __leaf__)) __attribute__ ((__noreturn__));
void reach_error() { __assert_fail("0", "s3_srvr_1a.cil.c-1.smt2.gz_000_range.c", 13, "reach_error"); }



int main ();

int
main ()
{
    // return parameter


    // variables
    int inv_main69_0;
    int inv_main69_1;
    int inv_main69_2;
    int inv_main69_3;
    int inv_main69_4;
    int inv_main69_5;
    int inv_main69_6;
    int inv_main69_7;
    int inv_main69_8;
    int inv_main3_0;
    int inv_main17_0;
    int inv_main17_1;
    int inv_main17_2;
    int inv_main17_3;
    int inv_main17_4;
    int inv_main17_5;
    int inv_main17_6;
    int inv_main17_7;
    int inv_main17_8;
    int inv_main109_0;
    int inv_main109_1;
    int inv_main109_2;
    int inv_main109_3;
    int inv_main109_4;
    int inv_main109_5;
    int inv_main109_6;
    int inv_main109_7;
    int inv_main109_8;
    int A_0;
    int A_1;
    int B_1;
    int C_1;
    int D_1;
    int E_1;
    int F_1;
    int G_1;
    int H_1;
    int I_1;
    int A_2;
    int B_2;
    int C_2;
    int D_2;
    int E_2;
    int F_2;
    int G_2;
    int H_2;
    int I_2;
    int A_3;
    int B_3;
    int C_3;
    int D_3;
    int E_3;
    int F_3;
    int G_3;
    int H_3;
    int I_3;
    int J_3;
    int A_4;
    int B_4;
    int C_4;
    int D_4;
    int E_4;
    int F_4;
    int G_4;
    int H_4;
    int I_4;
    int J_4;
    int A_5;
    int B_5;
    int C_5;
    int D_5;
    int E_5;
    int F_5;
    int G_5;
    int H_5;
    int I_5;
    int J_5;
    int A_6;
    int B_6;
    int C_6;
    int D_6;
    int E_6;
    int F_6;
    int G_6;
    int H_6;
    int I_6;
    int J_6;
    int A_7;
    int B_7;
    int C_7;
    int D_7;
    int E_7;
    int F_7;
    int G_7;
    int H_7;
    int I_7;
    int J_7;
    int A_8;
    int B_8;
    int C_8;
    int D_8;
    int E_8;
    int F_8;
    int G_8;
    int H_8;
    int I_8;
    int J_8;
    int K_8;
    int A_9;
    int B_9;
    int C_9;
    int D_9;
    int E_9;
    int F_9;
    int G_9;
    int H_9;
    int I_9;
    int J_9;
    int A_10;
    int B_10;
    int C_10;
    int D_10;
    int E_10;
    int F_10;
    int G_10;
    int H_10;
    int I_10;
    int J_10;
    int K_10;
    int A_11;
    int B_11;
    int C_11;
    int D_11;
    int E_11;
    int F_11;
    int G_11;
    int H_11;
    int I_11;
    int J_11;
    int K_11;
    int A_12;
    int B_12;
    int C_12;
    int D_12;
    int E_12;
    int F_12;
    int G_12;
    int H_12;
    int I_12;
    int J_12;
    int A_13;
    int B_13;
    int C_13;
    int D_13;
    int E_13;
    int F_13;
    int G_13;
    int H_13;
    int I_13;
    int J_13;
    int A_14;
    int B_14;
    int C_14;
    int D_14;
    int E_14;
    int F_14;
    int G_14;
    int H_14;
    int I_14;
    int J_14;
    int K_14;
    int A_15;
    int B_15;
    int C_15;
    int D_15;
    int E_15;
    int F_15;
    int G_15;
    int H_15;
    int I_15;
    int J_15;
    int K_15;
    int A_16;
    int B_16;
    int C_16;
    int D_16;
    int E_16;
    int F_16;
    int G_16;
    int H_16;
    int I_16;
    int J_16;
    int K_16;
    int A_17;
    int B_17;
    int C_17;
    int D_17;
    int E_17;
    int F_17;
    int G_17;
    int H_17;
    int I_17;
    int J_17;
    int A_18;
    int B_18;
    int C_18;
    int D_18;
    int E_18;
    int F_18;
    int G_18;
    int H_18;
    int I_18;
    int J_18;
    int K_18;
    int A_19;
    int B_19;
    int C_19;
    int D_19;
    int E_19;
    int F_19;
    int G_19;
    int H_19;
    int I_19;
    int J_19;
    int A_20;
    int B_20;
    int C_20;
    int D_20;
    int E_20;
    int F_20;
    int G_20;
    int H_20;
    int I_20;
    int J_20;
    int K_20;
    int A_21;
    int B_21;
    int C_21;
    int D_21;
    int E_21;
    int F_21;
    int G_21;
    int H_21;
    int I_21;
    int J_21;
    int A_22;
    int B_22;
    int C_22;
    int D_22;
    int E_22;
    int F_22;
    int G_22;
    int H_22;
    int I_22;
    int J_22;
    int A_23;
    int B_23;
    int C_23;
    int D_23;
    int E_23;
    int F_23;
    int G_23;
    int H_23;
    int I_23;
    int J_23;
    int A_24;
    int B_24;
    int C_24;
    int D_24;
    int E_24;
    int F_24;
    int G_24;
    int H_24;
    int I_24;
    int J_24;
    int A_25;
    int B_25;
    int C_25;
    int D_25;
    int E_25;
    int F_25;
    int G_25;
    int H_25;
    int I_25;
    int J_25;
    int A_26;
    int B_26;
    int C_26;
    int D_26;
    int E_26;
    int F_26;
    int G_26;
    int H_26;
    int I_26;
    int J_26;
    int A_27;
    int B_27;
    int C_27;
    int D_27;
    int E_27;
    int F_27;
    int G_27;
    int H_27;
    int I_27;
    int J_27;
    int A_28;
    int B_28;
    int C_28;
    int D_28;
    int E_28;
    int F_28;
    int G_28;
    int H_28;
    int I_28;
    int A_29;
    int B_29;
    int C_29;
    int D_29;
    int E_29;
    int F_29;
    int G_29;
    int H_29;
    int I_29;
    int A_30;
    int B_30;
    int C_30;
    int D_30;
    int E_30;
    int F_30;
    int G_30;
    int H_30;
    int I_30;

    if (((inv_main69_0 <= -1000000000) || (inv_main69_0 >= 1000000000))
        || ((inv_main69_1 <= -1000000000) || (inv_main69_1 >= 1000000000))
        || ((inv_main69_2 <= -1000000000) || (inv_main69_2 >= 1000000000))
        || ((inv_main69_3 <= -1000000000) || (inv_main69_3 >= 1000000000))
        || ((inv_main69_4 <= -1000000000) || (inv_main69_4 >= 1000000000))
        || ((inv_main69_5 <= -1000000000) || (inv_main69_5 >= 1000000000))
        || ((inv_main69_6 <= -1000000000) || (inv_main69_6 >= 1000000000))
        || ((inv_main69_7 <= -1000000000) || (inv_main69_7 >= 1000000000))
        || ((inv_main69_8 <= -1000000000) || (inv_main69_8 >= 1000000000))
        || ((inv_main3_0 <= -1000000000) || (inv_main3_0 >= 1000000000))
        || ((inv_main17_0 <= -1000000000) || (inv_main17_0 >= 1000000000))
        || ((inv_main17_1 <= -1000000000) || (inv_main17_1 >= 1000000000))
        || ((inv_main17_2 <= -1000000000) || (inv_main17_2 >= 1000000000))
        || ((inv_main17_3 <= -1000000000) || (inv_main17_3 >= 1000000000))
        || ((inv_main17_4 <= -1000000000) || (inv_main17_4 >= 1000000000))
        || ((inv_main17_5 <= -1000000000) || (inv_main17_5 >= 1000000000))
        || ((inv_main17_6 <= -1000000000) || (inv_main17_6 >= 1000000000))
        || ((inv_main17_7 <= -1000000000) || (inv_main17_7 >= 1000000000))
        || ((inv_main17_8 <= -1000000000) || (inv_main17_8 >= 1000000000))
        || ((inv_main109_0 <= -1000000000) || (inv_main109_0 >= 1000000000))
        || ((inv_main109_1 <= -1000000000) || (inv_main109_1 >= 1000000000))
        || ((inv_main109_2 <= -1000000000) || (inv_main109_2 >= 1000000000))
        || ((inv_main109_3 <= -1000000000) || (inv_main109_3 >= 1000000000))
        || ((inv_main109_4 <= -1000000000) || (inv_main109_4 >= 1000000000))
        || ((inv_main109_5 <= -1000000000) || (inv_main109_5 >= 1000000000))
        || ((inv_main109_6 <= -1000000000) || (inv_main109_6 >= 1000000000))
        || ((inv_main109_7 <= -1000000000) || (inv_main109_7 >= 1000000000))
        || ((inv_main109_8 <= -1000000000) || (inv_main109_8 >= 1000000000))
        || ((A_0 <= -1000000000) || (A_0 >= 1000000000))
        || ((A_1 <= -1000000000) || (A_1 >= 1000000000))
        || ((B_1 <= -1000000000) || (B_1 >= 1000000000))
        || ((C_1 <= -1000000000) || (C_1 >= 1000000000))
        || ((D_1 <= -1000000000) || (D_1 >= 1000000000))
        || ((E_1 <= -1000000000) || (E_1 >= 1000000000))
        || ((F_1 <= -1000000000) || (F_1 >= 1000000000))
        || ((G_1 <= -1000000000) || (G_1 >= 1000000000))
        || ((H_1 <= -1000000000) || (H_1 >= 1000000000))
        || ((I_1 <= -1000000000) || (I_1 >= 1000000000))
        || ((A_2 <= -1000000000) || (A_2 >= 1000000000))
        || ((B_2 <= -1000000000) || (B_2 >= 1000000000))
        || ((C_2 <= -1000000000) || (C_2 >= 1000000000))
        || ((D_2 <= -1000000000) || (D_2 >= 1000000000))
        || ((E_2 <= -1000000000) || (E_2 >= 1000000000))
        || ((F_2 <= -1000000000) || (F_2 >= 1000000000))
        || ((G_2 <= -1000000000) || (G_2 >= 1000000000))
        || ((H_2 <= -1000000000) || (H_2 >= 1000000000))
        || ((I_2 <= -1000000000) || (I_2 >= 1000000000))
        || ((A_3 <= -1000000000) || (A_3 >= 1000000000))
        || ((B_3 <= -1000000000) || (B_3 >= 1000000000))
        || ((C_3 <= -1000000000) || (C_3 >= 1000000000))
        || ((D_3 <= -1000000000) || (D_3 >= 1000000000))
        || ((E_3 <= -1000000000) || (E_3 >= 1000000000))
        || ((F_3 <= -1000000000) || (F_3 >= 1000000000))
        || ((G_3 <= -1000000000) || (G_3 >= 1000000000))
        || ((H_3 <= -1000000000) || (H_3 >= 1000000000))
        || ((I_3 <= -1000000000) || (I_3 >= 1000000000))
        || ((J_3 <= -1000000000) || (J_3 >= 1000000000))
        || ((A_4 <= -1000000000) || (A_4 >= 1000000000))
        || ((B_4 <= -1000000000) || (B_4 >= 1000000000))
        || ((C_4 <= -1000000000) || (C_4 >= 1000000000))
        || ((D_4 <= -1000000000) || (D_4 >= 1000000000))
        || ((E_4 <= -1000000000) || (E_4 >= 1000000000))
        || ((F_4 <= -1000000000) || (F_4 >= 1000000000))
        || ((G_4 <= -1000000000) || (G_4 >= 1000000000))
        || ((H_4 <= -1000000000) || (H_4 >= 1000000000))
        || ((I_4 <= -1000000000) || (I_4 >= 1000000000))
        || ((J_4 <= -1000000000) || (J_4 >= 1000000000))
        || ((A_5 <= -1000000000) || (A_5 >= 1000000000))
        || ((B_5 <= -1000000000) || (B_5 >= 1000000000))
        || ((C_5 <= -1000000000) || (C_5 >= 1000000000))
        || ((D_5 <= -1000000000) || (D_5 >= 1000000000))
        || ((E_5 <= -1000000000) || (E_5 >= 1000000000))
        || ((F_5 <= -1000000000) || (F_5 >= 1000000000))
        || ((G_5 <= -1000000000) || (G_5 >= 1000000000))
        || ((H_5 <= -1000000000) || (H_5 >= 1000000000))
        || ((I_5 <= -1000000000) || (I_5 >= 1000000000))
        || ((J_5 <= -1000000000) || (J_5 >= 1000000000))
        || ((A_6 <= -1000000000) || (A_6 >= 1000000000))
        || ((B_6 <= -1000000000) || (B_6 >= 1000000000))
        || ((C_6 <= -1000000000) || (C_6 >= 1000000000))
        || ((D_6 <= -1000000000) || (D_6 >= 1000000000))
        || ((E_6 <= -1000000000) || (E_6 >= 1000000000))
        || ((F_6 <= -1000000000) || (F_6 >= 1000000000))
        || ((G_6 <= -1000000000) || (G_6 >= 1000000000))
        || ((H_6 <= -1000000000) || (H_6 >= 1000000000))
        || ((I_6 <= -1000000000) || (I_6 >= 1000000000))
        || ((J_6 <= -1000000000) || (J_6 >= 1000000000))
        || ((A_7 <= -1000000000) || (A_7 >= 1000000000))
        || ((B_7 <= -1000000000) || (B_7 >= 1000000000))
        || ((C_7 <= -1000000000) || (C_7 >= 1000000000))
        || ((D_7 <= -1000000000) || (D_7 >= 1000000000))
        || ((E_7 <= -1000000000) || (E_7 >= 1000000000))
        || ((F_7 <= -1000000000) || (F_7 >= 1000000000))
        || ((G_7 <= -1000000000) || (G_7 >= 1000000000))
        || ((H_7 <= -1000000000) || (H_7 >= 1000000000))
        || ((I_7 <= -1000000000) || (I_7 >= 1000000000))
        || ((J_7 <= -1000000000) || (J_7 >= 1000000000))
        || ((A_8 <= -1000000000) || (A_8 >= 1000000000))
        || ((B_8 <= -1000000000) || (B_8 >= 1000000000))
        || ((C_8 <= -1000000000) || (C_8 >= 1000000000))
        || ((D_8 <= -1000000000) || (D_8 >= 1000000000))
        || ((E_8 <= -1000000000) || (E_8 >= 1000000000))
        || ((F_8 <= -1000000000) || (F_8 >= 1000000000))
        || ((G_8 <= -1000000000) || (G_8 >= 1000000000))
        || ((H_8 <= -1000000000) || (H_8 >= 1000000000))
        || ((I_8 <= -1000000000) || (I_8 >= 1000000000))
        || ((J_8 <= -1000000000) || (J_8 >= 1000000000))
        || ((K_8 <= -1000000000) || (K_8 >= 1000000000))
        || ((A_9 <= -1000000000) || (A_9 >= 1000000000))
        || ((B_9 <= -1000000000) || (B_9 >= 1000000000))
        || ((C_9 <= -1000000000) || (C_9 >= 1000000000))
        || ((D_9 <= -1000000000) || (D_9 >= 1000000000))
        || ((E_9 <= -1000000000) || (E_9 >= 1000000000))
        || ((F_9 <= -1000000000) || (F_9 >= 1000000000))
        || ((G_9 <= -1000000000) || (G_9 >= 1000000000))
        || ((H_9 <= -1000000000) || (H_9 >= 1000000000))
        || ((I_9 <= -1000000000) || (I_9 >= 1000000000))
        || ((J_9 <= -1000000000) || (J_9 >= 1000000000))
        || ((A_10 <= -1000000000) || (A_10 >= 1000000000))
        || ((B_10 <= -1000000000) || (B_10 >= 1000000000))
        || ((C_10 <= -1000000000) || (C_10 >= 1000000000))
        || ((D_10 <= -1000000000) || (D_10 >= 1000000000))
        || ((E_10 <= -1000000000) || (E_10 >= 1000000000))
        || ((F_10 <= -1000000000) || (F_10 >= 1000000000))
        || ((G_10 <= -1000000000) || (G_10 >= 1000000000))
        || ((H_10 <= -1000000000) || (H_10 >= 1000000000))
        || ((I_10 <= -1000000000) || (I_10 >= 1000000000))
        || ((J_10 <= -1000000000) || (J_10 >= 1000000000))
        || ((K_10 <= -1000000000) || (K_10 >= 1000000000))
        || ((A_11 <= -1000000000) || (A_11 >= 1000000000))
        || ((B_11 <= -1000000000) || (B_11 >= 1000000000))
        || ((C_11 <= -1000000000) || (C_11 >= 1000000000))
        || ((D_11 <= -1000000000) || (D_11 >= 1000000000))
        || ((E_11 <= -1000000000) || (E_11 >= 1000000000))
        || ((F_11 <= -1000000000) || (F_11 >= 1000000000))
        || ((G_11 <= -1000000000) || (G_11 >= 1000000000))
        || ((H_11 <= -1000000000) || (H_11 >= 1000000000))
        || ((I_11 <= -1000000000) || (I_11 >= 1000000000))
        || ((J_11 <= -1000000000) || (J_11 >= 1000000000))
        || ((K_11 <= -1000000000) || (K_11 >= 1000000000))
        || ((A_12 <= -1000000000) || (A_12 >= 1000000000))
        || ((B_12 <= -1000000000) || (B_12 >= 1000000000))
        || ((C_12 <= -1000000000) || (C_12 >= 1000000000))
        || ((D_12 <= -1000000000) || (D_12 >= 1000000000))
        || ((E_12 <= -1000000000) || (E_12 >= 1000000000))
        || ((F_12 <= -1000000000) || (F_12 >= 1000000000))
        || ((G_12 <= -1000000000) || (G_12 >= 1000000000))
        || ((H_12 <= -1000000000) || (H_12 >= 1000000000))
        || ((I_12 <= -1000000000) || (I_12 >= 1000000000))
        || ((J_12 <= -1000000000) || (J_12 >= 1000000000))
        || ((A_13 <= -1000000000) || (A_13 >= 1000000000))
        || ((B_13 <= -1000000000) || (B_13 >= 1000000000))
        || ((C_13 <= -1000000000) || (C_13 >= 1000000000))
        || ((D_13 <= -1000000000) || (D_13 >= 1000000000))
        || ((E_13 <= -1000000000) || (E_13 >= 1000000000))
        || ((F_13 <= -1000000000) || (F_13 >= 1000000000))
        || ((G_13 <= -1000000000) || (G_13 >= 1000000000))
        || ((H_13 <= -1000000000) || (H_13 >= 1000000000))
        || ((I_13 <= -1000000000) || (I_13 >= 1000000000))
        || ((J_13 <= -1000000000) || (J_13 >= 1000000000))
        || ((A_14 <= -1000000000) || (A_14 >= 1000000000))
        || ((B_14 <= -1000000000) || (B_14 >= 1000000000))
        || ((C_14 <= -1000000000) || (C_14 >= 1000000000))
        || ((D_14 <= -1000000000) || (D_14 >= 1000000000))
        || ((E_14 <= -1000000000) || (E_14 >= 1000000000))
        || ((F_14 <= -1000000000) || (F_14 >= 1000000000))
        || ((G_14 <= -1000000000) || (G_14 >= 1000000000))
        || ((H_14 <= -1000000000) || (H_14 >= 1000000000))
        || ((I_14 <= -1000000000) || (I_14 >= 1000000000))
        || ((J_14 <= -1000000000) || (J_14 >= 1000000000))
        || ((K_14 <= -1000000000) || (K_14 >= 1000000000))
        || ((A_15 <= -1000000000) || (A_15 >= 1000000000))
        || ((B_15 <= -1000000000) || (B_15 >= 1000000000))
        || ((C_15 <= -1000000000) || (C_15 >= 1000000000))
        || ((D_15 <= -1000000000) || (D_15 >= 1000000000))
        || ((E_15 <= -1000000000) || (E_15 >= 1000000000))
        || ((F_15 <= -1000000000) || (F_15 >= 1000000000))
        || ((G_15 <= -1000000000) || (G_15 >= 1000000000))
        || ((H_15 <= -1000000000) || (H_15 >= 1000000000))
        || ((I_15 <= -1000000000) || (I_15 >= 1000000000))
        || ((J_15 <= -1000000000) || (J_15 >= 1000000000))
        || ((K_15 <= -1000000000) || (K_15 >= 1000000000))
        || ((A_16 <= -1000000000) || (A_16 >= 1000000000))
        || ((B_16 <= -1000000000) || (B_16 >= 1000000000))
        || ((C_16 <= -1000000000) || (C_16 >= 1000000000))
        || ((D_16 <= -1000000000) || (D_16 >= 1000000000))
        || ((E_16 <= -1000000000) || (E_16 >= 1000000000))
        || ((F_16 <= -1000000000) || (F_16 >= 1000000000))
        || ((G_16 <= -1000000000) || (G_16 >= 1000000000))
        || ((H_16 <= -1000000000) || (H_16 >= 1000000000))
        || ((I_16 <= -1000000000) || (I_16 >= 1000000000))
        || ((J_16 <= -1000000000) || (J_16 >= 1000000000))
        || ((K_16 <= -1000000000) || (K_16 >= 1000000000))
        || ((A_17 <= -1000000000) || (A_17 >= 1000000000))
        || ((B_17 <= -1000000000) || (B_17 >= 1000000000))
        || ((C_17 <= -1000000000) || (C_17 >= 1000000000))
        || ((D_17 <= -1000000000) || (D_17 >= 1000000000))
        || ((E_17 <= -1000000000) || (E_17 >= 1000000000))
        || ((F_17 <= -1000000000) || (F_17 >= 1000000000))
        || ((G_17 <= -1000000000) || (G_17 >= 1000000000))
        || ((H_17 <= -1000000000) || (H_17 >= 1000000000))
        || ((I_17 <= -1000000000) || (I_17 >= 1000000000))
        || ((J_17 <= -1000000000) || (J_17 >= 1000000000))
        || ((A_18 <= -1000000000) || (A_18 >= 1000000000))
        || ((B_18 <= -1000000000) || (B_18 >= 1000000000))
        || ((C_18 <= -1000000000) || (C_18 >= 1000000000))
        || ((D_18 <= -1000000000) || (D_18 >= 1000000000))
        || ((E_18 <= -1000000000) || (E_18 >= 1000000000))
        || ((F_18 <= -1000000000) || (F_18 >= 1000000000))
        || ((G_18 <= -1000000000) || (G_18 >= 1000000000))
        || ((H_18 <= -1000000000) || (H_18 >= 1000000000))
        || ((I_18 <= -1000000000) || (I_18 >= 1000000000))
        || ((J_18 <= -1000000000) || (J_18 >= 1000000000))
        || ((K_18 <= -1000000000) || (K_18 >= 1000000000))
        || ((A_19 <= -1000000000) || (A_19 >= 1000000000))
        || ((B_19 <= -1000000000) || (B_19 >= 1000000000))
        || ((C_19 <= -1000000000) || (C_19 >= 1000000000))
        || ((D_19 <= -1000000000) || (D_19 >= 1000000000))
        || ((E_19 <= -1000000000) || (E_19 >= 1000000000))
        || ((F_19 <= -1000000000) || (F_19 >= 1000000000))
        || ((G_19 <= -1000000000) || (G_19 >= 1000000000))
        || ((H_19 <= -1000000000) || (H_19 >= 1000000000))
        || ((I_19 <= -1000000000) || (I_19 >= 1000000000))
        || ((J_19 <= -1000000000) || (J_19 >= 1000000000))
        || ((A_20 <= -1000000000) || (A_20 >= 1000000000))
        || ((B_20 <= -1000000000) || (B_20 >= 1000000000))
        || ((C_20 <= -1000000000) || (C_20 >= 1000000000))
        || ((D_20 <= -1000000000) || (D_20 >= 1000000000))
        || ((E_20 <= -1000000000) || (E_20 >= 1000000000))
        || ((F_20 <= -1000000000) || (F_20 >= 1000000000))
        || ((G_20 <= -1000000000) || (G_20 >= 1000000000))
        || ((H_20 <= -1000000000) || (H_20 >= 1000000000))
        || ((I_20 <= -1000000000) || (I_20 >= 1000000000))
        || ((J_20 <= -1000000000) || (J_20 >= 1000000000))
        || ((K_20 <= -1000000000) || (K_20 >= 1000000000))
        || ((A_21 <= -1000000000) || (A_21 >= 1000000000))
        || ((B_21 <= -1000000000) || (B_21 >= 1000000000))
        || ((C_21 <= -1000000000) || (C_21 >= 1000000000))
        || ((D_21 <= -1000000000) || (D_21 >= 1000000000))
        || ((E_21 <= -1000000000) || (E_21 >= 1000000000))
        || ((F_21 <= -1000000000) || (F_21 >= 1000000000))
        || ((G_21 <= -1000000000) || (G_21 >= 1000000000))
        || ((H_21 <= -1000000000) || (H_21 >= 1000000000))
        || ((I_21 <= -1000000000) || (I_21 >= 1000000000))
        || ((J_21 <= -1000000000) || (J_21 >= 1000000000))
        || ((A_22 <= -1000000000) || (A_22 >= 1000000000))
        || ((B_22 <= -1000000000) || (B_22 >= 1000000000))
        || ((C_22 <= -1000000000) || (C_22 >= 1000000000))
        || ((D_22 <= -1000000000) || (D_22 >= 1000000000))
        || ((E_22 <= -1000000000) || (E_22 >= 1000000000))
        || ((F_22 <= -1000000000) || (F_22 >= 1000000000))
        || ((G_22 <= -1000000000) || (G_22 >= 1000000000))
        || ((H_22 <= -1000000000) || (H_22 >= 1000000000))
        || ((I_22 <= -1000000000) || (I_22 >= 1000000000))
        || ((J_22 <= -1000000000) || (J_22 >= 1000000000))
        || ((A_23 <= -1000000000) || (A_23 >= 1000000000))
        || ((B_23 <= -1000000000) || (B_23 >= 1000000000))
        || ((C_23 <= -1000000000) || (C_23 >= 1000000000))
        || ((D_23 <= -1000000000) || (D_23 >= 1000000000))
        || ((E_23 <= -1000000000) || (E_23 >= 1000000000))
        || ((F_23 <= -1000000000) || (F_23 >= 1000000000))
        || ((G_23 <= -1000000000) || (G_23 >= 1000000000))
        || ((H_23 <= -1000000000) || (H_23 >= 1000000000))
        || ((I_23 <= -1000000000) || (I_23 >= 1000000000))
        || ((J_23 <= -1000000000) || (J_23 >= 1000000000))
        || ((A_24 <= -1000000000) || (A_24 >= 1000000000))
        || ((B_24 <= -1000000000) || (B_24 >= 1000000000))
        || ((C_24 <= -1000000000) || (C_24 >= 1000000000))
        || ((D_24 <= -1000000000) || (D_24 >= 1000000000))
        || ((E_24 <= -1000000000) || (E_24 >= 1000000000))
        || ((F_24 <= -1000000000) || (F_24 >= 1000000000))
        || ((G_24 <= -1000000000) || (G_24 >= 1000000000))
        || ((H_24 <= -1000000000) || (H_24 >= 1000000000))
        || ((I_24 <= -1000000000) || (I_24 >= 1000000000))
        || ((J_24 <= -1000000000) || (J_24 >= 1000000000))
        || ((A_25 <= -1000000000) || (A_25 >= 1000000000))
        || ((B_25 <= -1000000000) || (B_25 >= 1000000000))
        || ((C_25 <= -1000000000) || (C_25 >= 1000000000))
        || ((D_25 <= -1000000000) || (D_25 >= 1000000000))
        || ((E_25 <= -1000000000) || (E_25 >= 1000000000))
        || ((F_25 <= -1000000000) || (F_25 >= 1000000000))
        || ((G_25 <= -1000000000) || (G_25 >= 1000000000))
        || ((H_25 <= -1000000000) || (H_25 >= 1000000000))
        || ((I_25 <= -1000000000) || (I_25 >= 1000000000))
        || ((J_25 <= -1000000000) || (J_25 >= 1000000000))
        || ((A_26 <= -1000000000) || (A_26 >= 1000000000))
        || ((B_26 <= -1000000000) || (B_26 >= 1000000000))
        || ((C_26 <= -1000000000) || (C_26 >= 1000000000))
        || ((D_26 <= -1000000000) || (D_26 >= 1000000000))
        || ((E_26 <= -1000000000) || (E_26 >= 1000000000))
        || ((F_26 <= -1000000000) || (F_26 >= 1000000000))
        || ((G_26 <= -1000000000) || (G_26 >= 1000000000))
        || ((H_26 <= -1000000000) || (H_26 >= 1000000000))
        || ((I_26 <= -1000000000) || (I_26 >= 1000000000))
        || ((J_26 <= -1000000000) || (J_26 >= 1000000000))
        || ((A_27 <= -1000000000) || (A_27 >= 1000000000))
        || ((B_27 <= -1000000000) || (B_27 >= 1000000000))
        || ((C_27 <= -1000000000) || (C_27 >= 1000000000))
        || ((D_27 <= -1000000000) || (D_27 >= 1000000000))
        || ((E_27 <= -1000000000) || (E_27 >= 1000000000))
        || ((F_27 <= -1000000000) || (F_27 >= 1000000000))
        || ((G_27 <= -1000000000) || (G_27 >= 1000000000))
        || ((H_27 <= -1000000000) || (H_27 >= 1000000000))
        || ((I_27 <= -1000000000) || (I_27 >= 1000000000))
        || ((J_27 <= -1000000000) || (J_27 >= 1000000000))
        || ((A_28 <= -1000000000) || (A_28 >= 1000000000))
        || ((B_28 <= -1000000000) || (B_28 >= 1000000000))
        || ((C_28 <= -1000000000) || (C_28 >= 1000000000))
        || ((D_28 <= -1000000000) || (D_28 >= 1000000000))
        || ((E_28 <= -1000000000) || (E_28 >= 1000000000))
        || ((F_28 <= -1000000000) || (F_28 >= 1000000000))
        || ((G_28 <= -1000000000) || (G_28 >= 1000000000))
        || ((H_28 <= -1000000000) || (H_28 >= 1000000000))
        || ((I_28 <= -1000000000) || (I_28 >= 1000000000))
        || ((A_29 <= -1000000000) || (A_29 >= 1000000000))
        || ((B_29 <= -1000000000) || (B_29 >= 1000000000))
        || ((C_29 <= -1000000000) || (C_29 >= 1000000000))
        || ((D_29 <= -1000000000) || (D_29 >= 1000000000))
        || ((E_29 <= -1000000000) || (E_29 >= 1000000000))
        || ((F_29 <= -1000000000) || (F_29 >= 1000000000))
        || ((G_29 <= -1000000000) || (G_29 >= 1000000000))
        || ((H_29 <= -1000000000) || (H_29 >= 1000000000))
        || ((I_29 <= -1000000000) || (I_29 >= 1000000000))
        || ((A_30 <= -1000000000) || (A_30 >= 1000000000))
        || ((B_30 <= -1000000000) || (B_30 >= 1000000000))
        || ((C_30 <= -1000000000) || (C_30 >= 1000000000))
        || ((D_30 <= -1000000000) || (D_30 >= 1000000000))
        || ((E_30 <= -1000000000) || (E_30 >= 1000000000))
        || ((F_30 <= -1000000000) || (F_30 >= 1000000000))
        || ((G_30 <= -1000000000) || (G_30 >= 1000000000))
        || ((H_30 <= -1000000000) || (H_30 >= 1000000000))
        || ((I_30 <= -1000000000) || (I_30 >= 1000000000)))
        abort ();

    // main logic
    goto main_init;

  main_init:
    if (!1)
        abort ();
    inv_main3_0 = A_0;
    A_27 = __VERIFIER_nondet_int ();
    if (((A_27 <= -1000000000) || (A_27 >= 1000000000)))
        abort ();
    B_27 = __VERIFIER_nondet_int ();
    if (((B_27 <= -1000000000) || (B_27 >= 1000000000)))
        abort ();
    C_27 = __VERIFIER_nondet_int ();
    if (((C_27 <= -1000000000) || (C_27 >= 1000000000)))
        abort ();
    E_27 = __VERIFIER_nondet_int ();
    if (((E_27 <= -1000000000) || (E_27 >= 1000000000)))
        abort ();
    F_27 = __VERIFIER_nondet_int ();
    if (((F_27 <= -1000000000) || (F_27 >= 1000000000)))
        abort ();
    G_27 = __VERIFIER_nondet_int ();
    if (((G_27 <= -1000000000) || (G_27 >= 1000000000)))
        abort ();
    H_27 = __VERIFIER_nondet_int ();
    if (((H_27 <= -1000000000) || (H_27 >= 1000000000)))
        abort ();
    I_27 = __VERIFIER_nondet_int ();
    if (((I_27 <= -1000000000) || (I_27 >= 1000000000)))
        abort ();
    J_27 = __VERIFIER_nondet_int ();
    if (((J_27 <= -1000000000) || (J_27 >= 1000000000)))
        abort ();
    D_27 = inv_main3_0;
    if (!((G_27 == 0) && (H_27 == 8466)))
        abort ();
    inv_main17_0 = H_27;
    inv_main17_1 = F_27;
    inv_main17_2 = J_27;
    inv_main17_3 = B_27;
    inv_main17_4 = A_27;
    inv_main17_5 = I_27;
    inv_main17_6 = C_27;
    inv_main17_7 = G_27;
    inv_main17_8 = E_27;
    G_8 = __VERIFIER_nondet_int ();
    if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
        abort ();
    H_8 = __VERIFIER_nondet_int ();
    if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
        abort ();
    D_8 = inv_main17_0;
    B_8 = inv_main17_1;
    A_8 = inv_main17_2;
    K_8 = inv_main17_3;
    E_8 = inv_main17_4;
    J_8 = inv_main17_5;
    I_8 = inv_main17_6;
    C_8 = inv_main17_7;
    F_8 = inv_main17_8;
    if (!
        ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
         && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
        abort ();
    inv_main17_0 = H_8;
    inv_main17_1 = B_8;
    inv_main17_2 = A_8;
    inv_main17_3 = K_8;
    inv_main17_4 = E_8;
    inv_main17_5 = J_8;
    inv_main17_6 = I_8;
    inv_main17_7 = G_8;
    inv_main17_8 = F_8;
    goto inv_main17_20;
  main_error:
    reach_error ();
  main_final:
    goto main_final;
  inv_main69:
    switch (__VERIFIER_nondet_int ())
      {
      case 0:
          F_24 = __VERIFIER_nondet_int ();
          if (((F_24 <= -1000000000) || (F_24 >= 1000000000)))
              abort ();
          C_24 = inv_main69_0;
          E_24 = inv_main69_1;
          D_24 = inv_main69_2;
          I_24 = inv_main69_3;
          A_24 = inv_main69_4;
          B_24 = inv_main69_5;
          J_24 = inv_main69_6;
          H_24 = inv_main69_7;
          G_24 = inv_main69_8;
          if (!
              ((F_24 == 8560) && (D_24 == -2) && (0 <= A_24)
               && (!(A_24 == -256))))
              abort ();
          inv_main17_0 = F_24;
          inv_main17_1 = E_24;
          inv_main17_2 = D_24;
          inv_main17_3 = I_24;
          inv_main17_4 = A_24;
          inv_main17_5 = B_24;
          inv_main17_6 = J_24;
          inv_main17_7 = H_24;
          inv_main17_8 = G_24;
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      case 1:
          A_25 = __VERIFIER_nondet_int ();
          if (((A_25 <= -1000000000) || (A_25 >= 1000000000)))
              abort ();
          C_25 = inv_main69_0;
          G_25 = inv_main69_1;
          B_25 = inv_main69_2;
          E_25 = inv_main69_3;
          D_25 = inv_main69_4;
          F_25 = inv_main69_5;
          J_25 = inv_main69_6;
          H_25 = inv_main69_7;
          I_25 = inv_main69_8;
          if (!((D_25 == -256) && (0 <= D_25) && (A_25 == 8576)))
              abort ();
          inv_main17_0 = A_25;
          inv_main17_1 = G_25;
          inv_main17_2 = B_25;
          inv_main17_3 = E_25;
          inv_main17_4 = D_25;
          inv_main17_5 = F_25;
          inv_main17_6 = J_25;
          inv_main17_7 = H_25;
          inv_main17_8 = I_25;
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      case 2:
          D_26 = __VERIFIER_nondet_int ();
          if (((D_26 <= -1000000000) || (D_26 >= 1000000000)))
              abort ();
          J_26 = inv_main69_0;
          B_26 = inv_main69_1;
          A_26 = inv_main69_2;
          H_26 = inv_main69_3;
          F_26 = inv_main69_4;
          G_26 = inv_main69_5;
          C_26 = inv_main69_6;
          E_26 = inv_main69_7;
          I_26 = inv_main69_8;
          if (!
              ((!(F_26 == -256)) && (D_26 == 8576) && (0 <= F_26)
               && (!(A_26 == -2))))
              abort ();
          inv_main17_0 = D_26;
          inv_main17_1 = B_26;
          inv_main17_2 = A_26;
          inv_main17_3 = H_26;
          inv_main17_4 = F_26;
          inv_main17_5 = G_26;
          inv_main17_6 = C_26;
          inv_main17_7 = E_26;
          inv_main17_8 = I_26;
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      default:
          abort ();
      }
  inv_main17_0:
    switch (__VERIFIER_nondet_int ())
      {
      case 0:
          I_28 = inv_main17_0;
          C_28 = inv_main17_1;
          E_28 = inv_main17_2;
          F_28 = inv_main17_3;
          H_28 = inv_main17_4;
          D_28 = inv_main17_5;
          A_28 = inv_main17_6;
          B_28 = inv_main17_7;
          G_28 = inv_main17_8;
          if (!
              ((!(E_28 == -1)) && (!(I_28 == 8466)) && (!(I_28 == 8496))
               && (!(I_28 == 8528)) && (I_28 == 8544) && (!(I_28 == 8512))
               && (0 <= H_28) && ((!(3 <= B_28)) || (!(I_28 <= 8512)))
               && (F_28 == 0)))
              abort ();
          inv_main69_0 = I_28;
          inv_main69_1 = C_28;
          inv_main69_2 = E_28;
          inv_main69_3 = F_28;
          inv_main69_4 = H_28;
          inv_main69_5 = D_28;
          inv_main69_6 = A_28;
          inv_main69_7 = B_28;
          inv_main69_8 = G_28;
          goto inv_main69;

      case 1:
          C_29 = inv_main17_0;
          E_29 = inv_main17_1;
          I_29 = inv_main17_2;
          D_29 = inv_main17_3;
          B_29 = inv_main17_4;
          H_29 = inv_main17_5;
          A_29 = inv_main17_6;
          G_29 = inv_main17_7;
          F_29 = inv_main17_8;
          if (!
              ((!(C_29 == 8466)) && (!(C_29 == 8496)) && (!(C_29 == 8528))
               && (C_29 == 8544) && (!(C_29 == 8512)) && (I_29 == -4)
               && (!(I_29 == -1)) && (0 <= B_29) && ((!(C_29 <= 8512))
                                                     || (!(3 <= G_29)))
               && (!(D_29 == 0))))
              abort ();
          inv_main69_0 = C_29;
          inv_main69_1 = E_29;
          inv_main69_2 = I_29;
          inv_main69_3 = D_29;
          inv_main69_4 = B_29;
          inv_main69_5 = H_29;
          inv_main69_6 = A_29;
          inv_main69_7 = G_29;
          inv_main69_8 = F_29;
          goto inv_main69;

      case 2:
          A_4 = __VERIFIER_nondet_int ();
          if (((A_4 <= -1000000000) || (A_4 >= 1000000000)))
              abort ();
          B_4 = inv_main17_0;
          H_4 = inv_main17_1;
          E_4 = inv_main17_2;
          F_4 = inv_main17_3;
          C_4 = inv_main17_4;
          J_4 = inv_main17_5;
          I_4 = inv_main17_6;
          G_4 = inv_main17_7;
          D_4 = inv_main17_8;
          if (!
              ((!(B_4 == 8466)) && (!(B_4 == 8496)) && (B_4 == 8528)
               && (!(B_4 == 8512)) && (0 <= C_4) && ((!(3 <= G_4))
                                                     || (!(B_4 <= 8512)))
               && (A_4 == 8544)))
              abort ();
          inv_main17_0 = A_4;
          inv_main17_1 = H_4;
          inv_main17_2 = E_4;
          inv_main17_3 = F_4;
          inv_main17_4 = C_4;
          inv_main17_5 = J_4;
          inv_main17_6 = I_4;
          inv_main17_7 = G_4;
          inv_main17_8 = D_4;
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      case 3:
          F_5 = __VERIFIER_nondet_int ();
          if (((F_5 <= -1000000000) || (F_5 >= 1000000000)))
              abort ();
          J_5 = inv_main17_0;
          G_5 = inv_main17_1;
          I_5 = inv_main17_2;
          C_5 = inv_main17_3;
          B_5 = inv_main17_4;
          H_5 = inv_main17_5;
          E_5 = inv_main17_6;
          A_5 = inv_main17_7;
          D_5 = inv_main17_8;
          if (!
              ((!(J_5 == 8466)) && (!(J_5 == 8496)) && (!(J_5 == 8528))
               && (!(J_5 == 8544)) && (J_5 == 8560) && (!(J_5 == 8512))
               && (0 <= B_5) && ((!(3 <= A_5)) || (!(J_5 <= 8512)))
               && (F_5 == 8576)))
              abort ();
          inv_main17_0 = F_5;
          inv_main17_1 = G_5;
          inv_main17_2 = I_5;
          inv_main17_3 = C_5;
          inv_main17_4 = B_5;
          inv_main17_5 = H_5;
          inv_main17_6 = E_5;
          inv_main17_7 = A_5;
          inv_main17_8 = D_5;
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      case 4:
          J_6 = __VERIFIER_nondet_int ();
          if (((J_6 <= -1000000000) || (J_6 >= 1000000000)))
              abort ();
          H_6 = inv_main17_0;
          F_6 = inv_main17_1;
          D_6 = inv_main17_2;
          B_6 = inv_main17_3;
          G_6 = inv_main17_4;
          E_6 = inv_main17_5;
          I_6 = inv_main17_6;
          C_6 = inv_main17_7;
          A_6 = inv_main17_8;
          if (!
              ((!(H_6 == 8496)) && (!(H_6 == 8528)) && (!(H_6 == 8544))
               && (!(H_6 == 8560)) && (!(H_6 == 8576)) && (H_6 == 8592)
               && (!(H_6 == 8512)) && (J_6 == 8608) && (0 <= G_6)
               && ((!(3 <= C_6)) || (!(H_6 <= 8512))) && (!(H_6 == 8466))))
              abort ();
          inv_main17_0 = J_6;
          inv_main17_1 = F_6;
          inv_main17_2 = D_6;
          inv_main17_3 = B_6;
          inv_main17_4 = G_6;
          inv_main17_5 = E_6;
          inv_main17_6 = I_6;
          inv_main17_7 = C_6;
          inv_main17_8 = A_6;
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      case 5:
          G_7 = __VERIFIER_nondet_int ();
          if (((G_7 <= -1000000000) || (G_7 >= 1000000000)))
              abort ();
          H_7 = inv_main17_0;
          C_7 = inv_main17_1;
          J_7 = inv_main17_2;
          F_7 = inv_main17_3;
          I_7 = inv_main17_4;
          A_7 = inv_main17_5;
          D_7 = inv_main17_6;
          B_7 = inv_main17_7;
          E_7 = inv_main17_8;
          if (!
              ((!(H_7 == 8496)) && (!(H_7 == 8528)) && (!(H_7 == 8544))
               && (!(H_7 == 8560)) && (!(H_7 == 8576)) && (!(H_7 == 8592))
               && (H_7 == 8608) && (!(H_7 == 8512)) && (G_7 == 8640)
               && (0 <= I_7) && ((!(H_7 <= 8512)) || (!(3 <= B_7)))
               && (!(H_7 == 8466))))
              abort ();
          inv_main17_0 = G_7;
          inv_main17_1 = C_7;
          inv_main17_2 = J_7;
          inv_main17_3 = F_7;
          inv_main17_4 = I_7;
          inv_main17_5 = A_7;
          inv_main17_6 = D_7;
          inv_main17_7 = B_7;
          inv_main17_8 = E_7;
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      case 6:
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      case 7:
          G_9 = __VERIFIER_nondet_int ();
          if (((G_9 <= -1000000000) || (G_9 >= 1000000000)))
              abort ();
          C_9 = inv_main17_0;
          E_9 = inv_main17_1;
          D_9 = inv_main17_2;
          B_9 = inv_main17_3;
          F_9 = inv_main17_4;
          I_9 = inv_main17_5;
          H_9 = inv_main17_6;
          A_9 = inv_main17_7;
          J_9 = inv_main17_8;
          if (!
              ((G_9 == 8496) && (C_9 == 8466) && (0 <= F_9)
               && ((!(3 <= A_9)) || (!(C_9 <= 8512))) && (!(A_9 == 0))))
              abort ();
          inv_main17_0 = G_9;
          inv_main17_1 = E_9;
          inv_main17_2 = D_9;
          inv_main17_3 = B_9;
          inv_main17_4 = F_9;
          inv_main17_5 = I_9;
          inv_main17_6 = H_9;
          inv_main17_7 = A_9;
          inv_main17_8 = J_9;
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      case 8:
          F_10 = __VERIFIER_nondet_int ();
          if (((F_10 <= -1000000000) || (F_10 >= 1000000000)))
              abort ();
          G_10 = __VERIFIER_nondet_int ();
          if (((G_10 <= -1000000000) || (G_10 >= 1000000000)))
              abort ();
          B_10 = inv_main17_0;
          E_10 = inv_main17_1;
          A_10 = inv_main17_2;
          D_10 = inv_main17_3;
          H_10 = inv_main17_4;
          J_10 = inv_main17_5;
          I_10 = inv_main17_6;
          K_10 = inv_main17_7;
          C_10 = inv_main17_8;
          if (!
              ((B_10 == 8496) && (G_10 == 8656) && (F_10 == 2)
               && (!(E_10 == 0)) && (K_10 == 1) && (0 <= H_10)
               && ((!(3 <= K_10)) || (!(B_10 <= 8512))) && (!(B_10 == 8466))))
              abort ();
          inv_main17_0 = G_10;
          inv_main17_1 = E_10;
          inv_main17_2 = A_10;
          inv_main17_3 = D_10;
          inv_main17_4 = H_10;
          inv_main17_5 = J_10;
          inv_main17_6 = I_10;
          inv_main17_7 = F_10;
          inv_main17_8 = C_10;
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      case 9:
          C_11 = __VERIFIER_nondet_int ();
          if (((C_11 <= -1000000000) || (C_11 >= 1000000000)))
              abort ();
          H_11 = __VERIFIER_nondet_int ();
          if (((H_11 <= -1000000000) || (H_11 >= 1000000000)))
              abort ();
          B_11 = inv_main17_0;
          K_11 = inv_main17_1;
          D_11 = inv_main17_2;
          E_11 = inv_main17_3;
          A_11 = inv_main17_4;
          I_11 = inv_main17_5;
          J_11 = inv_main17_6;
          F_11 = inv_main17_7;
          G_11 = inv_main17_8;
          if (!
              ((B_11 == 8496) && (H_11 == 8512) && (F_11 == 1) && (C_11 == 2)
               && (K_11 == 0) && (0 <= A_11) && ((!(B_11 <= 8512))
                                                 || (!(3 <= F_11)))
               && (!(B_11 == 8466))))
              abort ();
          inv_main17_0 = H_11;
          inv_main17_1 = K_11;
          inv_main17_2 = D_11;
          inv_main17_3 = E_11;
          inv_main17_4 = A_11;
          inv_main17_5 = I_11;
          inv_main17_6 = J_11;
          inv_main17_7 = C_11;
          inv_main17_8 = G_11;
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      case 10:
          F_12 = __VERIFIER_nondet_int ();
          if (((F_12 <= -1000000000) || (F_12 >= 1000000000)))
              abort ();
          H_12 = inv_main17_0;
          E_12 = inv_main17_1;
          B_12 = inv_main17_2;
          D_12 = inv_main17_3;
          C_12 = inv_main17_4;
          G_12 = inv_main17_5;
          J_12 = inv_main17_6;
          I_12 = inv_main17_7;
          A_12 = inv_main17_8;
          if (!
              ((!(H_12 == 8466)) && (H_12 == 8496) && (F_12 == 8656)
               && (!(E_12 == 0)) && (0 <= C_12) && ((!(3 <= I_12))
                                                    || (!(H_12 <= 8512)))
               && (!(I_12 == 1))))
              abort ();
          inv_main17_0 = F_12;
          inv_main17_1 = E_12;
          inv_main17_2 = B_12;
          inv_main17_3 = D_12;
          inv_main17_4 = C_12;
          inv_main17_5 = G_12;
          inv_main17_6 = J_12;
          inv_main17_7 = I_12;
          inv_main17_8 = A_12;
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      case 11:
          A_13 = __VERIFIER_nondet_int ();
          if (((A_13 <= -1000000000) || (A_13 >= 1000000000)))
              abort ();
          E_13 = inv_main17_0;
          C_13 = inv_main17_1;
          I_13 = inv_main17_2;
          B_13 = inv_main17_3;
          D_13 = inv_main17_4;
          J_13 = inv_main17_5;
          H_13 = inv_main17_6;
          G_13 = inv_main17_7;
          F_13 = inv_main17_8;
          if (!
              ((!(G_13 == 1)) && (!(E_13 == 8466)) && (E_13 == 8496)
               && (C_13 == 0) && (0 <= D_13) && ((!(3 <= G_13))
                                                 || (!(E_13 <= 8512)))
               && (A_13 == 8512)))
              abort ();
          inv_main17_0 = A_13;
          inv_main17_1 = C_13;
          inv_main17_2 = I_13;
          inv_main17_3 = B_13;
          inv_main17_4 = D_13;
          inv_main17_5 = J_13;
          inv_main17_6 = H_13;
          inv_main17_7 = G_13;
          inv_main17_8 = F_13;
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      case 12:
          A_14 = __VERIFIER_nondet_int ();
          if (((A_14 <= -1000000000) || (A_14 >= 1000000000)))
              abort ();
          H_14 = __VERIFIER_nondet_int ();
          if (((H_14 <= -1000000000) || (H_14 >= 1000000000)))
              abort ();
          E_14 = inv_main17_0;
          G_14 = inv_main17_1;
          F_14 = inv_main17_2;
          D_14 = inv_main17_3;
          C_14 = inv_main17_4;
          K_14 = inv_main17_5;
          B_14 = inv_main17_6;
          I_14 = inv_main17_7;
          J_14 = inv_main17_8;
          if (!
              ((H_14 == 8466) && (!(E_14 == 8466)) && (!(E_14 == 8496))
               && (!(E_14 == 8528)) && (!(E_14 == 8544)) && (!(E_14 == 8560))
               && (E_14 == 8576) && (!(E_14 == 8512)) && (0 <= C_14)
               && ((!(E_14 <= 8512)) || (!(3 <= I_14))) && (A_14 == 2)))
              abort ();
          inv_main17_0 = H_14;
          inv_main17_1 = G_14;
          inv_main17_2 = F_14;
          inv_main17_3 = D_14;
          inv_main17_4 = C_14;
          inv_main17_5 = K_14;
          inv_main17_6 = B_14;
          inv_main17_7 = I_14;
          inv_main17_8 = A_14;
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      case 13:
          E_15 = __VERIFIER_nondet_int ();
          if (((E_15 <= -1000000000) || (E_15 >= 1000000000)))
              abort ();
          G_15 = __VERIFIER_nondet_int ();
          if (((G_15 <= -1000000000) || (G_15 >= 1000000000)))
              abort ();
          C_15 = inv_main17_0;
          B_15 = inv_main17_1;
          I_15 = inv_main17_2;
          J_15 = inv_main17_3;
          K_15 = inv_main17_4;
          H_15 = inv_main17_5;
          D_15 = inv_main17_6;
          A_15 = inv_main17_7;
          F_15 = inv_main17_8;
          if (!
              ((E_15 == 8592) && (!(C_15 == 8466)) && (!(C_15 == 8496))
               && (!(C_15 == 8528)) && (!(C_15 == 8544)) && (!(C_15 == 8560))
               && (C_15 == 8576) && (!(C_15 == 8512)) && (0 <= K_15)
               && ((!(C_15 <= 8512)) || (!(3 <= A_15))) && (!(G_15 == 2))))
              abort ();
          inv_main17_0 = E_15;
          inv_main17_1 = B_15;
          inv_main17_2 = I_15;
          inv_main17_3 = J_15;
          inv_main17_4 = K_15;
          inv_main17_5 = H_15;
          inv_main17_6 = D_15;
          inv_main17_7 = A_15;
          inv_main17_8 = G_15;
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      case 14:
          B_16 = __VERIFIER_nondet_int ();
          if (((B_16 <= -1000000000) || (B_16 >= 1000000000)))
              abort ();
          J_16 = __VERIFIER_nondet_int ();
          if (((J_16 <= -1000000000) || (J_16 >= 1000000000)))
              abort ();
          C_16 = inv_main17_0;
          I_16 = inv_main17_1;
          G_16 = inv_main17_2;
          D_16 = inv_main17_3;
          H_16 = inv_main17_4;
          K_16 = inv_main17_5;
          A_16 = inv_main17_6;
          F_16 = inv_main17_7;
          E_16 = inv_main17_8;
          if (!
              ((J_16 == 4) && (I_16 == 0) && (F_16 == 3) && (!(C_16 == 8466))
               && (!(C_16 == 8496)) && (!(C_16 == 8528)) && (!(C_16 == 8544))
               && (!(C_16 == 8560)) && (!(C_16 == 8576)) && (!(C_16 == 8592))
               && (!(C_16 == 8608)) && (C_16 == 8640) && (!(C_16 == 8512))
               && (0 <= H_16) && ((!(C_16 <= 8512)) || (!(3 <= F_16)))
               && (B_16 == 8656)))
              abort ();
          inv_main17_0 = B_16;
          inv_main17_1 = I_16;
          inv_main17_2 = G_16;
          inv_main17_3 = D_16;
          inv_main17_4 = H_16;
          inv_main17_5 = K_16;
          inv_main17_6 = A_16;
          inv_main17_7 = J_16;
          inv_main17_8 = E_16;
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      case 15:
          I_17 = __VERIFIER_nondet_int ();
          if (((I_17 <= -1000000000) || (I_17 >= 1000000000)))
              abort ();
          G_17 = inv_main17_0;
          F_17 = inv_main17_1;
          C_17 = inv_main17_2;
          E_17 = inv_main17_3;
          H_17 = inv_main17_4;
          J_17 = inv_main17_5;
          D_17 = inv_main17_6;
          A_17 = inv_main17_7;
          B_17 = inv_main17_8;
          if (!
              ((I_17 == 8656) && (!(G_17 == 8466)) && (!(G_17 == 8496))
               && (!(G_17 == 8528)) && (!(G_17 == 8544)) && (!(G_17 == 8560))
               && (!(G_17 == 8576)) && (!(G_17 == 8592)) && (!(G_17 == 8608))
               && (G_17 == 8640) && (!(G_17 == 8512)) && (F_17 == 0)
               && (0 <= H_17) && ((!(3 <= A_17)) || (!(G_17 <= 8512)))
               && (!(A_17 == 3))))
              abort ();
          inv_main17_0 = I_17;
          inv_main17_1 = F_17;
          inv_main17_2 = C_17;
          inv_main17_3 = E_17;
          inv_main17_4 = H_17;
          inv_main17_5 = J_17;
          inv_main17_6 = D_17;
          inv_main17_7 = A_17;
          inv_main17_8 = B_17;
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      case 16:
          F_18 = __VERIFIER_nondet_int ();
          if (((F_18 <= -1000000000) || (F_18 >= 1000000000)))
              abort ();
          I_18 = __VERIFIER_nondet_int ();
          if (((I_18 <= -1000000000) || (I_18 >= 1000000000)))
              abort ();
          J_18 = inv_main17_0;
          E_18 = inv_main17_1;
          H_18 = inv_main17_2;
          A_18 = inv_main17_3;
          K_18 = inv_main17_4;
          B_18 = inv_main17_5;
          G_18 = inv_main17_6;
          C_18 = inv_main17_7;
          D_18 = inv_main17_8;
          if (!
              ((!(J_18 == 8496)) && (!(J_18 == 8528)) && (!(J_18 == 8544))
               && (!(J_18 == 8560)) && (!(J_18 == 8576)) && (!(J_18 == 8592))
               && (!(J_18 == 8608)) && (!(J_18 == 8640)) && (J_18 == 8656)
               && (!(J_18 == 8512)) && (I_18 == 8672) && (F_18 == 3)
               && (C_18 == 2) && (0 <= K_18) && ((!(3 <= C_18))
                                                 || (!(J_18 <= 8512)))
               && (!(J_18 == 8466))))
              abort ();
          inv_main17_0 = I_18;
          inv_main17_1 = E_18;
          inv_main17_2 = H_18;
          inv_main17_3 = A_18;
          inv_main17_4 = K_18;
          inv_main17_5 = B_18;
          inv_main17_6 = G_18;
          inv_main17_7 = F_18;
          inv_main17_8 = D_18;
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      case 17:
          D_19 = __VERIFIER_nondet_int ();
          if (((D_19 <= -1000000000) || (D_19 >= 1000000000)))
              abort ();
          H_19 = inv_main17_0;
          G_19 = inv_main17_1;
          A_19 = inv_main17_2;
          E_19 = inv_main17_3;
          I_19 = inv_main17_4;
          B_19 = inv_main17_5;
          J_19 = inv_main17_6;
          C_19 = inv_main17_7;
          F_19 = inv_main17_8;
          if (!
              ((!(H_19 == 8496)) && (!(H_19 == 8528)) && (!(H_19 == 8544))
               && (!(H_19 == 8560)) && (!(H_19 == 8576)) && (!(H_19 == 8592))
               && (!(H_19 == 8608)) && (!(H_19 == 8640)) && (H_19 == 8656)
               && (!(H_19 == 8512)) && (D_19 == 8672) && (!(C_19 == 2))
               && (0 <= I_19) && ((!(3 <= C_19)) || (!(H_19 <= 8512)))
               && (!(H_19 == 8466))))
              abort ();
          inv_main17_0 = D_19;
          inv_main17_1 = G_19;
          inv_main17_2 = A_19;
          inv_main17_3 = E_19;
          inv_main17_4 = I_19;
          inv_main17_5 = B_19;
          inv_main17_6 = J_19;
          inv_main17_7 = C_19;
          inv_main17_8 = F_19;
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      case 18:
          B_20 = __VERIFIER_nondet_int ();
          if (((B_20 <= -1000000000) || (B_20 >= 1000000000)))
              abort ();
          H_20 = __VERIFIER_nondet_int ();
          if (((H_20 <= -1000000000) || (H_20 >= 1000000000)))
              abort ();
          E_20 = inv_main17_0;
          J_20 = inv_main17_1;
          F_20 = inv_main17_2;
          C_20 = inv_main17_3;
          D_20 = inv_main17_4;
          K_20 = inv_main17_5;
          A_20 = inv_main17_6;
          G_20 = inv_main17_7;
          I_20 = inv_main17_8;
          if (!
              ((!(J_20 == 0)) && (H_20 == 8640) && (G_20 == 4)
               && (!(E_20 == 8466)) && (!(E_20 == 8496)) && (!(E_20 == 8528))
               && (!(E_20 == 8544)) && (!(E_20 == 8560)) && (!(E_20 == 8576))
               && (!(E_20 == 8592)) && (!(E_20 == 8608)) && (!(E_20 == 8640))
               && (!(E_20 == 8656)) && (E_20 == 8672) && (!(E_20 == 8512))
               && (0 <= D_20) && ((!(E_20 <= 8512)) || (!(3 <= G_20)))
               && (B_20 == 5)))
              abort ();
          inv_main17_0 = H_20;
          inv_main17_1 = J_20;
          inv_main17_2 = F_20;
          inv_main17_3 = C_20;
          inv_main17_4 = D_20;
          inv_main17_5 = K_20;
          inv_main17_6 = A_20;
          inv_main17_7 = B_20;
          inv_main17_8 = I_20;
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      case 19:
          A_21 = __VERIFIER_nondet_int ();
          if (((A_21 <= -1000000000) || (A_21 >= 1000000000)))
              abort ();
          J_21 = inv_main17_0;
          D_21 = inv_main17_1;
          G_21 = inv_main17_2;
          I_21 = inv_main17_3;
          F_21 = inv_main17_4;
          E_21 = inv_main17_5;
          H_21 = inv_main17_6;
          B_21 = inv_main17_7;
          C_21 = inv_main17_8;
          if (!
              ((!(D_21 == 0)) && (!(B_21 == 4)) && (!(B_21 == 5))
               && (!(J_21 == 8466)) && (!(J_21 == 8496)) && (!(J_21 == 8528))
               && (!(J_21 == 8544)) && (!(J_21 == 8560)) && (!(J_21 == 8576))
               && (!(J_21 == 8592)) && (!(J_21 == 8608)) && (!(J_21 == 8640))
               && (!(J_21 == 8656)) && (J_21 == 8672) && (!(J_21 == 8512))
               && (0 <= F_21) && ((!(J_21 <= 8512)) || (!(3 <= B_21)))
               && (A_21 == 8640)))
              abort ();
          inv_main17_0 = A_21;
          inv_main17_1 = D_21;
          inv_main17_2 = G_21;
          inv_main17_3 = I_21;
          inv_main17_4 = F_21;
          inv_main17_5 = E_21;
          inv_main17_6 = H_21;
          inv_main17_7 = B_21;
          inv_main17_8 = C_21;
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      case 20:
          G_22 = __VERIFIER_nondet_int ();
          if (((G_22 <= -1000000000) || (G_22 >= 1000000000)))
              abort ();
          D_22 = inv_main17_0;
          I_22 = inv_main17_1;
          J_22 = inv_main17_2;
          C_22 = inv_main17_3;
          B_22 = inv_main17_4;
          H_22 = inv_main17_5;
          F_22 = inv_main17_6;
          A_22 = inv_main17_7;
          E_22 = inv_main17_8;
          if (!
              ((!(D_22 == 8466)) && (!(D_22 == 8496)) && (!(D_22 == 8528))
               && (D_22 == 8544) && (!(D_22 == 8512)) && (J_22 == -1)
               && (0 <= B_22) && ((!(D_22 <= 8512)) || (!(3 <= A_22)))
               && (G_22 == 8560)))
              abort ();
          inv_main17_0 = G_22;
          inv_main17_1 = I_22;
          inv_main17_2 = J_22;
          inv_main17_3 = C_22;
          inv_main17_4 = B_22;
          inv_main17_5 = H_22;
          inv_main17_6 = F_22;
          inv_main17_7 = A_22;
          inv_main17_8 = E_22;
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      case 21:
          G_23 = __VERIFIER_nondet_int ();
          if (((G_23 <= -1000000000) || (G_23 >= 1000000000)))
              abort ();
          E_23 = inv_main17_0;
          A_23 = inv_main17_1;
          H_23 = inv_main17_2;
          D_23 = inv_main17_3;
          B_23 = inv_main17_4;
          I_23 = inv_main17_5;
          J_23 = inv_main17_6;
          F_23 = inv_main17_7;
          C_23 = inv_main17_8;
          if (!
              ((!(H_23 == -1)) && (G_23 == 8560) && (!(E_23 == 8466))
               && (!(E_23 == 8496)) && (!(E_23 == 8528)) && (E_23 == 8544)
               && (!(E_23 == 8512)) && (!(D_23 == 0)) && (0 <= B_23)
               && ((!(E_23 <= 8512)) || (!(3 <= F_23))) && (!(H_23 == -4))))
              abort ();
          inv_main17_0 = G_23;
          inv_main17_1 = A_23;
          inv_main17_2 = H_23;
          inv_main17_3 = D_23;
          inv_main17_4 = B_23;
          inv_main17_5 = I_23;
          inv_main17_6 = J_23;
          inv_main17_7 = F_23;
          inv_main17_8 = C_23;
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      case 22:
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      case 23:
          E_1 = inv_main17_0;
          F_1 = inv_main17_1;
          H_1 = inv_main17_2;
          D_1 = inv_main17_3;
          C_1 = inv_main17_4;
          G_1 = inv_main17_5;
          I_1 = inv_main17_6;
          A_1 = inv_main17_7;
          B_1 = inv_main17_8;
          if (!((0 <= C_1) && (E_1 <= 8512) && (3 <= A_1)))
              abort ();
          inv_main109_0 = E_1;
          inv_main109_1 = F_1;
          inv_main109_2 = H_1;
          inv_main109_3 = D_1;
          inv_main109_4 = C_1;
          inv_main109_5 = G_1;
          inv_main109_6 = I_1;
          inv_main109_7 = A_1;
          inv_main109_8 = B_1;
          D_30 = inv_main109_0;
          I_30 = inv_main109_1;
          H_30 = inv_main109_2;
          A_30 = inv_main109_3;
          G_30 = inv_main109_4;
          F_30 = inv_main109_5;
          E_30 = inv_main109_6;
          C_30 = inv_main109_7;
          B_30 = inv_main109_8;
          if (!(0 <= G_30))
              abort ();
          goto main_error;

      case 24:
          C_2 = inv_main17_0;
          G_2 = inv_main17_1;
          F_2 = inv_main17_2;
          I_2 = inv_main17_3;
          E_2 = inv_main17_4;
          B_2 = inv_main17_5;
          D_2 = inv_main17_6;
          H_2 = inv_main17_7;
          A_2 = inv_main17_8;
          if (!
              ((H_2 == 5) && (!(C_2 == 8466)) && (!(C_2 == 8496))
               && (!(C_2 == 8528)) && (!(C_2 == 8544)) && (!(C_2 == 8560))
               && (!(C_2 == 8576)) && (!(C_2 == 8592)) && (!(C_2 == 8608))
               && (!(C_2 == 8640)) && (!(C_2 == 8656)) && (C_2 == 8672)
               && (!(C_2 == 8512)) && (0 <= E_2) && ((!(C_2 <= 8512))
                                                     || (!(3 <= H_2)))
               && (!(H_2 == 4))))
              abort ();
          inv_main109_0 = C_2;
          inv_main109_1 = G_2;
          inv_main109_2 = F_2;
          inv_main109_3 = I_2;
          inv_main109_4 = E_2;
          inv_main109_5 = B_2;
          inv_main109_6 = D_2;
          inv_main109_7 = H_2;
          inv_main109_8 = A_2;
          D_30 = inv_main109_0;
          I_30 = inv_main109_1;
          H_30 = inv_main109_2;
          A_30 = inv_main109_3;
          G_30 = inv_main109_4;
          F_30 = inv_main109_5;
          E_30 = inv_main109_6;
          C_30 = inv_main109_7;
          B_30 = inv_main109_8;
          if (!(0 <= G_30))
              abort ();
          goto main_error;

      default:
          abort ();
      }
  inv_main17_1:
    switch (__VERIFIER_nondet__Bool ())
      {
      case 0:
          J_3 = __VERIFIER_nondet_int ();
          if (((J_3 <= -1000000000) || (J_3 >= 1000000000)))
              abort ();
          F_3 = inv_main17_0;
          H_3 = inv_main17_1;
          G_3 = inv_main17_2;
          E_3 = inv_main17_3;
          D_3 = inv_main17_4;
          B_3 = inv_main17_5;
          I_3 = inv_main17_6;
          A_3 = inv_main17_7;
          C_3 = inv_main17_8;
          if (!
              ((!(F_3 == 8496)) && (F_3 == 8512) && (J_3 == 8528)
               && (0 <= D_3) && ((!(3 <= A_3)) || (!(F_3 <= 8512)))
               && (!(F_3 == 8466))))
              abort ();
          inv_main17_0 = J_3;
          inv_main17_1 = H_3;
          inv_main17_2 = G_3;
          inv_main17_3 = E_3;
          inv_main17_4 = D_3;
          inv_main17_5 = B_3;
          inv_main17_6 = I_3;
          inv_main17_7 = A_3;
          inv_main17_8 = C_3;
          goto inv_main17_0;

      case 1:
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      default:
          abort ();
      }
  inv_main17_2:
    switch (__VERIFIER_nondet__Bool ())
      {
      case 0:
          F_10 = __VERIFIER_nondet_int ();
          if (((F_10 <= -1000000000) || (F_10 >= 1000000000)))
              abort ();
          G_10 = __VERIFIER_nondet_int ();
          if (((G_10 <= -1000000000) || (G_10 >= 1000000000)))
              abort ();
          B_10 = inv_main17_0;
          E_10 = inv_main17_1;
          A_10 = inv_main17_2;
          D_10 = inv_main17_3;
          H_10 = inv_main17_4;
          J_10 = inv_main17_5;
          I_10 = inv_main17_6;
          K_10 = inv_main17_7;
          C_10 = inv_main17_8;
          if (!
              ((B_10 == 8496) && (G_10 == 8656) && (F_10 == 2)
               && (!(E_10 == 0)) && (K_10 == 1) && (0 <= H_10)
               && ((!(3 <= K_10)) || (!(B_10 <= 8512))) && (!(B_10 == 8466))))
              abort ();
          inv_main17_0 = G_10;
          inv_main17_1 = E_10;
          inv_main17_2 = A_10;
          inv_main17_3 = D_10;
          inv_main17_4 = H_10;
          inv_main17_5 = J_10;
          inv_main17_6 = I_10;
          inv_main17_7 = F_10;
          inv_main17_8 = C_10;
          goto inv_main17_1;

      case 1:
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      default:
          abort ();
      }
  inv_main17_3:
    switch (__VERIFIER_nondet__Bool ())
      {
      case 0:
          A_21 = __VERIFIER_nondet_int ();
          if (((A_21 <= -1000000000) || (A_21 >= 1000000000)))
              abort ();
          J_21 = inv_main17_0;
          D_21 = inv_main17_1;
          G_21 = inv_main17_2;
          I_21 = inv_main17_3;
          F_21 = inv_main17_4;
          E_21 = inv_main17_5;
          H_21 = inv_main17_6;
          B_21 = inv_main17_7;
          C_21 = inv_main17_8;
          if (!
              ((!(D_21 == 0)) && (!(B_21 == 4)) && (!(B_21 == 5))
               && (!(J_21 == 8466)) && (!(J_21 == 8496)) && (!(J_21 == 8528))
               && (!(J_21 == 8544)) && (!(J_21 == 8560)) && (!(J_21 == 8576))
               && (!(J_21 == 8592)) && (!(J_21 == 8608)) && (!(J_21 == 8640))
               && (!(J_21 == 8656)) && (J_21 == 8672) && (!(J_21 == 8512))
               && (0 <= F_21) && ((!(J_21 <= 8512)) || (!(3 <= B_21)))
               && (A_21 == 8640)))
              abort ();
          inv_main17_0 = A_21;
          inv_main17_1 = D_21;
          inv_main17_2 = G_21;
          inv_main17_3 = I_21;
          inv_main17_4 = F_21;
          inv_main17_5 = E_21;
          inv_main17_6 = H_21;
          inv_main17_7 = B_21;
          inv_main17_8 = C_21;
          goto inv_main17_2;

      case 1:
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      default:
          abort ();
      }
  inv_main17_4:
    switch (__VERIFIER_nondet__Bool ())
      {
      case 0:
          E_15 = __VERIFIER_nondet_int ();
          if (((E_15 <= -1000000000) || (E_15 >= 1000000000)))
              abort ();
          G_15 = __VERIFIER_nondet_int ();
          if (((G_15 <= -1000000000) || (G_15 >= 1000000000)))
              abort ();
          C_15 = inv_main17_0;
          B_15 = inv_main17_1;
          I_15 = inv_main17_2;
          J_15 = inv_main17_3;
          K_15 = inv_main17_4;
          H_15 = inv_main17_5;
          D_15 = inv_main17_6;
          A_15 = inv_main17_7;
          F_15 = inv_main17_8;
          if (!
              ((E_15 == 8592) && (!(C_15 == 8466)) && (!(C_15 == 8496))
               && (!(C_15 == 8528)) && (!(C_15 == 8544)) && (!(C_15 == 8560))
               && (C_15 == 8576) && (!(C_15 == 8512)) && (0 <= K_15)
               && ((!(C_15 <= 8512)) || (!(3 <= A_15))) && (!(G_15 == 2))))
              abort ();
          inv_main17_0 = E_15;
          inv_main17_1 = B_15;
          inv_main17_2 = I_15;
          inv_main17_3 = J_15;
          inv_main17_4 = K_15;
          inv_main17_5 = H_15;
          inv_main17_6 = D_15;
          inv_main17_7 = A_15;
          inv_main17_8 = G_15;
          goto inv_main17_3;

      case 1:
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      default:
          abort ();
      }
  inv_main17_5:
    switch (__VERIFIER_nondet__Bool ())
      {
      case 0:
          G_7 = __VERIFIER_nondet_int ();
          if (((G_7 <= -1000000000) || (G_7 >= 1000000000)))
              abort ();
          H_7 = inv_main17_0;
          C_7 = inv_main17_1;
          J_7 = inv_main17_2;
          F_7 = inv_main17_3;
          I_7 = inv_main17_4;
          A_7 = inv_main17_5;
          D_7 = inv_main17_6;
          B_7 = inv_main17_7;
          E_7 = inv_main17_8;
          if (!
              ((!(H_7 == 8496)) && (!(H_7 == 8528)) && (!(H_7 == 8544))
               && (!(H_7 == 8560)) && (!(H_7 == 8576)) && (!(H_7 == 8592))
               && (H_7 == 8608) && (!(H_7 == 8512)) && (G_7 == 8640)
               && (0 <= I_7) && ((!(H_7 <= 8512)) || (!(3 <= B_7)))
               && (!(H_7 == 8466))))
              abort ();
          inv_main17_0 = G_7;
          inv_main17_1 = C_7;
          inv_main17_2 = J_7;
          inv_main17_3 = F_7;
          inv_main17_4 = I_7;
          inv_main17_5 = A_7;
          inv_main17_6 = D_7;
          inv_main17_7 = B_7;
          inv_main17_8 = E_7;
          goto inv_main17_4;

      case 1:
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      default:
          abort ();
      }
  inv_main17_6:
    switch (__VERIFIER_nondet__Bool ())
      {
      case 0:
          A_13 = __VERIFIER_nondet_int ();
          if (((A_13 <= -1000000000) || (A_13 >= 1000000000)))
              abort ();
          E_13 = inv_main17_0;
          C_13 = inv_main17_1;
          I_13 = inv_main17_2;
          B_13 = inv_main17_3;
          D_13 = inv_main17_4;
          J_13 = inv_main17_5;
          H_13 = inv_main17_6;
          G_13 = inv_main17_7;
          F_13 = inv_main17_8;
          if (!
              ((!(G_13 == 1)) && (!(E_13 == 8466)) && (E_13 == 8496)
               && (C_13 == 0) && (0 <= D_13) && ((!(3 <= G_13))
                                                 || (!(E_13 <= 8512)))
               && (A_13 == 8512)))
              abort ();
          inv_main17_0 = A_13;
          inv_main17_1 = C_13;
          inv_main17_2 = I_13;
          inv_main17_3 = B_13;
          inv_main17_4 = D_13;
          inv_main17_5 = J_13;
          inv_main17_6 = H_13;
          inv_main17_7 = G_13;
          inv_main17_8 = F_13;
          goto inv_main17_5;

      case 1:
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      default:
          abort ();
      }
  inv_main17_7:
    switch (__VERIFIER_nondet__Bool ())
      {
      case 0:
          F_18 = __VERIFIER_nondet_int ();
          if (((F_18 <= -1000000000) || (F_18 >= 1000000000)))
              abort ();
          I_18 = __VERIFIER_nondet_int ();
          if (((I_18 <= -1000000000) || (I_18 >= 1000000000)))
              abort ();
          J_18 = inv_main17_0;
          E_18 = inv_main17_1;
          H_18 = inv_main17_2;
          A_18 = inv_main17_3;
          K_18 = inv_main17_4;
          B_18 = inv_main17_5;
          G_18 = inv_main17_6;
          C_18 = inv_main17_7;
          D_18 = inv_main17_8;
          if (!
              ((!(J_18 == 8496)) && (!(J_18 == 8528)) && (!(J_18 == 8544))
               && (!(J_18 == 8560)) && (!(J_18 == 8576)) && (!(J_18 == 8592))
               && (!(J_18 == 8608)) && (!(J_18 == 8640)) && (J_18 == 8656)
               && (!(J_18 == 8512)) && (I_18 == 8672) && (F_18 == 3)
               && (C_18 == 2) && (0 <= K_18) && ((!(3 <= C_18))
                                                 || (!(J_18 <= 8512)))
               && (!(J_18 == 8466))))
              abort ();
          inv_main17_0 = I_18;
          inv_main17_1 = E_18;
          inv_main17_2 = H_18;
          inv_main17_3 = A_18;
          inv_main17_4 = K_18;
          inv_main17_5 = B_18;
          inv_main17_6 = G_18;
          inv_main17_7 = F_18;
          inv_main17_8 = D_18;
          goto inv_main17_6;

      case 1:
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      default:
          abort ();
      }
  inv_main17_8:
    switch (__VERIFIER_nondet__Bool ())
      {
      case 0:
          A_14 = __VERIFIER_nondet_int ();
          if (((A_14 <= -1000000000) || (A_14 >= 1000000000)))
              abort ();
          H_14 = __VERIFIER_nondet_int ();
          if (((H_14 <= -1000000000) || (H_14 >= 1000000000)))
              abort ();
          E_14 = inv_main17_0;
          G_14 = inv_main17_1;
          F_14 = inv_main17_2;
          D_14 = inv_main17_3;
          C_14 = inv_main17_4;
          K_14 = inv_main17_5;
          B_14 = inv_main17_6;
          I_14 = inv_main17_7;
          J_14 = inv_main17_8;
          if (!
              ((H_14 == 8466) && (!(E_14 == 8466)) && (!(E_14 == 8496))
               && (!(E_14 == 8528)) && (!(E_14 == 8544)) && (!(E_14 == 8560))
               && (E_14 == 8576) && (!(E_14 == 8512)) && (0 <= C_14)
               && ((!(E_14 <= 8512)) || (!(3 <= I_14))) && (A_14 == 2)))
              abort ();
          inv_main17_0 = H_14;
          inv_main17_1 = G_14;
          inv_main17_2 = F_14;
          inv_main17_3 = D_14;
          inv_main17_4 = C_14;
          inv_main17_5 = K_14;
          inv_main17_6 = B_14;
          inv_main17_7 = I_14;
          inv_main17_8 = A_14;
          goto inv_main17_7;

      case 1:
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      default:
          abort ();
      }
  inv_main17_9:
    switch (__VERIFIER_nondet__Bool ())
      {
      case 0:
          I_17 = __VERIFIER_nondet_int ();
          if (((I_17 <= -1000000000) || (I_17 >= 1000000000)))
              abort ();
          G_17 = inv_main17_0;
          F_17 = inv_main17_1;
          C_17 = inv_main17_2;
          E_17 = inv_main17_3;
          H_17 = inv_main17_4;
          J_17 = inv_main17_5;
          D_17 = inv_main17_6;
          A_17 = inv_main17_7;
          B_17 = inv_main17_8;
          if (!
              ((I_17 == 8656) && (!(G_17 == 8466)) && (!(G_17 == 8496))
               && (!(G_17 == 8528)) && (!(G_17 == 8544)) && (!(G_17 == 8560))
               && (!(G_17 == 8576)) && (!(G_17 == 8592)) && (!(G_17 == 8608))
               && (G_17 == 8640) && (!(G_17 == 8512)) && (F_17 == 0)
               && (0 <= H_17) && ((!(3 <= A_17)) || (!(G_17 <= 8512)))
               && (!(A_17 == 3))))
              abort ();
          inv_main17_0 = I_17;
          inv_main17_1 = F_17;
          inv_main17_2 = C_17;
          inv_main17_3 = E_17;
          inv_main17_4 = H_17;
          inv_main17_5 = J_17;
          inv_main17_6 = D_17;
          inv_main17_7 = A_17;
          inv_main17_8 = B_17;
          goto inv_main17_8;

      case 1:
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      default:
          abort ();
      }
  inv_main17_10:
    switch (__VERIFIER_nondet__Bool ())
      {
      case 0:
          J_6 = __VERIFIER_nondet_int ();
          if (((J_6 <= -1000000000) || (J_6 >= 1000000000)))
              abort ();
          H_6 = inv_main17_0;
          F_6 = inv_main17_1;
          D_6 = inv_main17_2;
          B_6 = inv_main17_3;
          G_6 = inv_main17_4;
          E_6 = inv_main17_5;
          I_6 = inv_main17_6;
          C_6 = inv_main17_7;
          A_6 = inv_main17_8;
          if (!
              ((!(H_6 == 8496)) && (!(H_6 == 8528)) && (!(H_6 == 8544))
               && (!(H_6 == 8560)) && (!(H_6 == 8576)) && (H_6 == 8592)
               && (!(H_6 == 8512)) && (J_6 == 8608) && (0 <= G_6)
               && ((!(3 <= C_6)) || (!(H_6 <= 8512))) && (!(H_6 == 8466))))
              abort ();
          inv_main17_0 = J_6;
          inv_main17_1 = F_6;
          inv_main17_2 = D_6;
          inv_main17_3 = B_6;
          inv_main17_4 = G_6;
          inv_main17_5 = E_6;
          inv_main17_6 = I_6;
          inv_main17_7 = C_6;
          inv_main17_8 = A_6;
          goto inv_main17_9;

      case 1:
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      default:
          abort ();
      }
  inv_main17_11:
    switch (__VERIFIER_nondet__Bool ())
      {
      case 0:
          F_12 = __VERIFIER_nondet_int ();
          if (((F_12 <= -1000000000) || (F_12 >= 1000000000)))
              abort ();
          H_12 = inv_main17_0;
          E_12 = inv_main17_1;
          B_12 = inv_main17_2;
          D_12 = inv_main17_3;
          C_12 = inv_main17_4;
          G_12 = inv_main17_5;
          J_12 = inv_main17_6;
          I_12 = inv_main17_7;
          A_12 = inv_main17_8;
          if (!
              ((!(H_12 == 8466)) && (H_12 == 8496) && (F_12 == 8656)
               && (!(E_12 == 0)) && (0 <= C_12) && ((!(3 <= I_12))
                                                    || (!(H_12 <= 8512)))
               && (!(I_12 == 1))))
              abort ();
          inv_main17_0 = F_12;
          inv_main17_1 = E_12;
          inv_main17_2 = B_12;
          inv_main17_3 = D_12;
          inv_main17_4 = C_12;
          inv_main17_5 = G_12;
          inv_main17_6 = J_12;
          inv_main17_7 = I_12;
          inv_main17_8 = A_12;
          goto inv_main17_10;

      case 1:
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      default:
          abort ();
      }
  inv_main17_12:
    switch (__VERIFIER_nondet__Bool ())
      {
      case 0:
          G_22 = __VERIFIER_nondet_int ();
          if (((G_22 <= -1000000000) || (G_22 >= 1000000000)))
              abort ();
          D_22 = inv_main17_0;
          I_22 = inv_main17_1;
          J_22 = inv_main17_2;
          C_22 = inv_main17_3;
          B_22 = inv_main17_4;
          H_22 = inv_main17_5;
          F_22 = inv_main17_6;
          A_22 = inv_main17_7;
          E_22 = inv_main17_8;
          if (!
              ((!(D_22 == 8466)) && (!(D_22 == 8496)) && (!(D_22 == 8528))
               && (D_22 == 8544) && (!(D_22 == 8512)) && (J_22 == -1)
               && (0 <= B_22) && ((!(D_22 <= 8512)) || (!(3 <= A_22)))
               && (G_22 == 8560)))
              abort ();
          inv_main17_0 = G_22;
          inv_main17_1 = I_22;
          inv_main17_2 = J_22;
          inv_main17_3 = C_22;
          inv_main17_4 = B_22;
          inv_main17_5 = H_22;
          inv_main17_6 = F_22;
          inv_main17_7 = A_22;
          inv_main17_8 = E_22;
          goto inv_main17_11;

      case 1:
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      default:
          abort ();
      }
  inv_main17_13:
    switch (__VERIFIER_nondet__Bool ())
      {
      case 0:
          D_19 = __VERIFIER_nondet_int ();
          if (((D_19 <= -1000000000) || (D_19 >= 1000000000)))
              abort ();
          H_19 = inv_main17_0;
          G_19 = inv_main17_1;
          A_19 = inv_main17_2;
          E_19 = inv_main17_3;
          I_19 = inv_main17_4;
          B_19 = inv_main17_5;
          J_19 = inv_main17_6;
          C_19 = inv_main17_7;
          F_19 = inv_main17_8;
          if (!
              ((!(H_19 == 8496)) && (!(H_19 == 8528)) && (!(H_19 == 8544))
               && (!(H_19 == 8560)) && (!(H_19 == 8576)) && (!(H_19 == 8592))
               && (!(H_19 == 8608)) && (!(H_19 == 8640)) && (H_19 == 8656)
               && (!(H_19 == 8512)) && (D_19 == 8672) && (!(C_19 == 2))
               && (0 <= I_19) && ((!(3 <= C_19)) || (!(H_19 <= 8512)))
               && (!(H_19 == 8466))))
              abort ();
          inv_main17_0 = D_19;
          inv_main17_1 = G_19;
          inv_main17_2 = A_19;
          inv_main17_3 = E_19;
          inv_main17_4 = I_19;
          inv_main17_5 = B_19;
          inv_main17_6 = J_19;
          inv_main17_7 = C_19;
          inv_main17_8 = F_19;
          goto inv_main17_12;

      case 1:
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      default:
          abort ();
      }
  inv_main17_14:
    switch (__VERIFIER_nondet__Bool ())
      {
      case 0:
          F_5 = __VERIFIER_nondet_int ();
          if (((F_5 <= -1000000000) || (F_5 >= 1000000000)))
              abort ();
          J_5 = inv_main17_0;
          G_5 = inv_main17_1;
          I_5 = inv_main17_2;
          C_5 = inv_main17_3;
          B_5 = inv_main17_4;
          H_5 = inv_main17_5;
          E_5 = inv_main17_6;
          A_5 = inv_main17_7;
          D_5 = inv_main17_8;
          if (!
              ((!(J_5 == 8466)) && (!(J_5 == 8496)) && (!(J_5 == 8528))
               && (!(J_5 == 8544)) && (J_5 == 8560) && (!(J_5 == 8512))
               && (0 <= B_5) && ((!(3 <= A_5)) || (!(J_5 <= 8512)))
               && (F_5 == 8576)))
              abort ();
          inv_main17_0 = F_5;
          inv_main17_1 = G_5;
          inv_main17_2 = I_5;
          inv_main17_3 = C_5;
          inv_main17_4 = B_5;
          inv_main17_5 = H_5;
          inv_main17_6 = E_5;
          inv_main17_7 = A_5;
          inv_main17_8 = D_5;
          goto inv_main17_13;

      case 1:
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      default:
          abort ();
      }
  inv_main17_15:
    switch (__VERIFIER_nondet__Bool ())
      {
      case 0:
          G_9 = __VERIFIER_nondet_int ();
          if (((G_9 <= -1000000000) || (G_9 >= 1000000000)))
              abort ();
          C_9 = inv_main17_0;
          E_9 = inv_main17_1;
          D_9 = inv_main17_2;
          B_9 = inv_main17_3;
          F_9 = inv_main17_4;
          I_9 = inv_main17_5;
          H_9 = inv_main17_6;
          A_9 = inv_main17_7;
          J_9 = inv_main17_8;
          if (!
              ((G_9 == 8496) && (C_9 == 8466) && (0 <= F_9)
               && ((!(3 <= A_9)) || (!(C_9 <= 8512))) && (!(A_9 == 0))))
              abort ();
          inv_main17_0 = G_9;
          inv_main17_1 = E_9;
          inv_main17_2 = D_9;
          inv_main17_3 = B_9;
          inv_main17_4 = F_9;
          inv_main17_5 = I_9;
          inv_main17_6 = H_9;
          inv_main17_7 = A_9;
          inv_main17_8 = J_9;
          goto inv_main17_14;

      case 1:
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      default:
          abort ();
      }
  inv_main17_16:
    switch (__VERIFIER_nondet__Bool ())
      {
      case 0:
          B_16 = __VERIFIER_nondet_int ();
          if (((B_16 <= -1000000000) || (B_16 >= 1000000000)))
              abort ();
          J_16 = __VERIFIER_nondet_int ();
          if (((J_16 <= -1000000000) || (J_16 >= 1000000000)))
              abort ();
          C_16 = inv_main17_0;
          I_16 = inv_main17_1;
          G_16 = inv_main17_2;
          D_16 = inv_main17_3;
          H_16 = inv_main17_4;
          K_16 = inv_main17_5;
          A_16 = inv_main17_6;
          F_16 = inv_main17_7;
          E_16 = inv_main17_8;
          if (!
              ((J_16 == 4) && (I_16 == 0) && (F_16 == 3) && (!(C_16 == 8466))
               && (!(C_16 == 8496)) && (!(C_16 == 8528)) && (!(C_16 == 8544))
               && (!(C_16 == 8560)) && (!(C_16 == 8576)) && (!(C_16 == 8592))
               && (!(C_16 == 8608)) && (C_16 == 8640) && (!(C_16 == 8512))
               && (0 <= H_16) && ((!(C_16 <= 8512)) || (!(3 <= F_16)))
               && (B_16 == 8656)))
              abort ();
          inv_main17_0 = B_16;
          inv_main17_1 = I_16;
          inv_main17_2 = G_16;
          inv_main17_3 = D_16;
          inv_main17_4 = H_16;
          inv_main17_5 = K_16;
          inv_main17_6 = A_16;
          inv_main17_7 = J_16;
          inv_main17_8 = E_16;
          goto inv_main17_15;

      case 1:
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      default:
          abort ();
      }
  inv_main17_17:
    switch (__VERIFIER_nondet__Bool ())
      {
      case 0:
          G_23 = __VERIFIER_nondet_int ();
          if (((G_23 <= -1000000000) || (G_23 >= 1000000000)))
              abort ();
          E_23 = inv_main17_0;
          A_23 = inv_main17_1;
          H_23 = inv_main17_2;
          D_23 = inv_main17_3;
          B_23 = inv_main17_4;
          I_23 = inv_main17_5;
          J_23 = inv_main17_6;
          F_23 = inv_main17_7;
          C_23 = inv_main17_8;
          if (!
              ((!(H_23 == -1)) && (G_23 == 8560) && (!(E_23 == 8466))
               && (!(E_23 == 8496)) && (!(E_23 == 8528)) && (E_23 == 8544)
               && (!(E_23 == 8512)) && (!(D_23 == 0)) && (0 <= B_23)
               && ((!(E_23 <= 8512)) || (!(3 <= F_23))) && (!(H_23 == -4))))
              abort ();
          inv_main17_0 = G_23;
          inv_main17_1 = A_23;
          inv_main17_2 = H_23;
          inv_main17_3 = D_23;
          inv_main17_4 = B_23;
          inv_main17_5 = I_23;
          inv_main17_6 = J_23;
          inv_main17_7 = F_23;
          inv_main17_8 = C_23;
          goto inv_main17_16;

      case 1:
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      default:
          abort ();
      }
  inv_main17_18:
    switch (__VERIFIER_nondet__Bool ())
      {
      case 0:
          A_4 = __VERIFIER_nondet_int ();
          if (((A_4 <= -1000000000) || (A_4 >= 1000000000)))
              abort ();
          B_4 = inv_main17_0;
          H_4 = inv_main17_1;
          E_4 = inv_main17_2;
          F_4 = inv_main17_3;
          C_4 = inv_main17_4;
          J_4 = inv_main17_5;
          I_4 = inv_main17_6;
          G_4 = inv_main17_7;
          D_4 = inv_main17_8;
          if (!
              ((!(B_4 == 8466)) && (!(B_4 == 8496)) && (B_4 == 8528)
               && (!(B_4 == 8512)) && (0 <= C_4) && ((!(3 <= G_4))
                                                     || (!(B_4 <= 8512)))
               && (A_4 == 8544)))
              abort ();
          inv_main17_0 = A_4;
          inv_main17_1 = H_4;
          inv_main17_2 = E_4;
          inv_main17_3 = F_4;
          inv_main17_4 = C_4;
          inv_main17_5 = J_4;
          inv_main17_6 = I_4;
          inv_main17_7 = G_4;
          inv_main17_8 = D_4;
          goto inv_main17_17;

      case 1:
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      default:
          abort ();
      }
  inv_main17_19:
    switch (__VERIFIER_nondet__Bool ())
      {
      case 0:
          C_11 = __VERIFIER_nondet_int ();
          if (((C_11 <= -1000000000) || (C_11 >= 1000000000)))
              abort ();
          H_11 = __VERIFIER_nondet_int ();
          if (((H_11 <= -1000000000) || (H_11 >= 1000000000)))
              abort ();
          B_11 = inv_main17_0;
          K_11 = inv_main17_1;
          D_11 = inv_main17_2;
          E_11 = inv_main17_3;
          A_11 = inv_main17_4;
          I_11 = inv_main17_5;
          J_11 = inv_main17_6;
          F_11 = inv_main17_7;
          G_11 = inv_main17_8;
          if (!
              ((B_11 == 8496) && (H_11 == 8512) && (F_11 == 1) && (C_11 == 2)
               && (K_11 == 0) && (0 <= A_11) && ((!(B_11 <= 8512))
                                                 || (!(3 <= F_11)))
               && (!(B_11 == 8466))))
              abort ();
          inv_main17_0 = H_11;
          inv_main17_1 = K_11;
          inv_main17_2 = D_11;
          inv_main17_3 = E_11;
          inv_main17_4 = A_11;
          inv_main17_5 = I_11;
          inv_main17_6 = J_11;
          inv_main17_7 = C_11;
          inv_main17_8 = G_11;
          goto inv_main17_18;

      case 1:
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      default:
          abort ();
      }
  inv_main17_20:
    switch (__VERIFIER_nondet__Bool ())
      {
      case 0:
          B_20 = __VERIFIER_nondet_int ();
          if (((B_20 <= -1000000000) || (B_20 >= 1000000000)))
              abort ();
          H_20 = __VERIFIER_nondet_int ();
          if (((H_20 <= -1000000000) || (H_20 >= 1000000000)))
              abort ();
          E_20 = inv_main17_0;
          J_20 = inv_main17_1;
          F_20 = inv_main17_2;
          C_20 = inv_main17_3;
          D_20 = inv_main17_4;
          K_20 = inv_main17_5;
          A_20 = inv_main17_6;
          G_20 = inv_main17_7;
          I_20 = inv_main17_8;
          if (!
              ((!(J_20 == 0)) && (H_20 == 8640) && (G_20 == 4)
               && (!(E_20 == 8466)) && (!(E_20 == 8496)) && (!(E_20 == 8528))
               && (!(E_20 == 8544)) && (!(E_20 == 8560)) && (!(E_20 == 8576))
               && (!(E_20 == 8592)) && (!(E_20 == 8608)) && (!(E_20 == 8640))
               && (!(E_20 == 8656)) && (E_20 == 8672) && (!(E_20 == 8512))
               && (0 <= D_20) && ((!(E_20 <= 8512)) || (!(3 <= G_20)))
               && (B_20 == 5)))
              abort ();
          inv_main17_0 = H_20;
          inv_main17_1 = J_20;
          inv_main17_2 = F_20;
          inv_main17_3 = C_20;
          inv_main17_4 = D_20;
          inv_main17_5 = K_20;
          inv_main17_6 = A_20;
          inv_main17_7 = B_20;
          inv_main17_8 = I_20;
          goto inv_main17_19;

      case 1:
          G_8 = __VERIFIER_nondet_int ();
          if (((G_8 <= -1000000000) || (G_8 >= 1000000000)))
              abort ();
          H_8 = __VERIFIER_nondet_int ();
          if (((H_8 <= -1000000000) || (H_8 >= 1000000000)))
              abort ();
          D_8 = inv_main17_0;
          B_8 = inv_main17_1;
          A_8 = inv_main17_2;
          K_8 = inv_main17_3;
          E_8 = inv_main17_4;
          J_8 = inv_main17_5;
          I_8 = inv_main17_6;
          C_8 = inv_main17_7;
          F_8 = inv_main17_8;
          if (!
              ((G_8 == 1) && (D_8 == 8466) && (C_8 == 0) && (0 <= E_8)
               && ((!(3 <= C_8)) || (!(D_8 <= 8512))) && (H_8 == 8496)))
              abort ();
          inv_main17_0 = H_8;
          inv_main17_1 = B_8;
          inv_main17_2 = A_8;
          inv_main17_3 = K_8;
          inv_main17_4 = E_8;
          inv_main17_5 = J_8;
          inv_main17_6 = I_8;
          inv_main17_7 = G_8;
          inv_main17_8 = F_8;
          goto inv_main17_20;

      default:
          abort ();
      }

    // return expression

}

