...
void reach_error() { ((void) sizeof ((0) ? 1 : 0), __extension__ ({ if (0) ; else __assert_fail ("0", "32_1_cilled_ok_nondet_linux-3.4-32_1-drivers--hid--hid-monterey.ko-ldv_main0_sequence_infinite_withcheck_stateful.cil.out.c", 3, __extension__ __PRETTY_FUNCTION__); })); }
...
static struct hid_device_id const mr_devices[1] = { {(__u16 )3, (unsigned short)0, (__u32 )1382, (__u32 )12292, 0UL}};
extern struct hid_device_id const __mod_hid_device_table __attribute__((__unused__,
__alias__("mr_devices"))) ;
static struct hid_driver mr_driver =
     {(char *)"monterey", mr_devices, {(struct list_head *)0, (struct list_head *)0},
    ...
    & mr_report_fixup, & mr_input_mapping, (int (*)(struct hid_device *hdev , struct hid_input *hidinput ,
...
static int mr_init(void) __attribute__((__section__(".init.text"), __no_instrument_function__)) ;
static int mr_init(void)
{ int tmp ;
  {
  {
  tmp = (int )__hid_register_driver(& mr_driver, & __this_module, "hid_monterey");
  }
  return (tmp);
}
}
static void mr_exit(void) __attribute__((__section__(".exit.text"), __no_instrument_function__)) ;
