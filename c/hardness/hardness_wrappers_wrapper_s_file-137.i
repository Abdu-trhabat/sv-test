extern unsigned long __VERIFIER_nondet_ulong(void);
extern long __VERIFIER_nondet_long(void);
extern unsigned char __VERIFIER_nondet_uchar(void);
extern char __VERIFIER_nondet_char(void);
extern unsigned short __VERIFIER_nondet_ushort(void);
extern short __VERIFIER_nondet_short(void);
extern float __VERIFIER_nondet_float(void);
extern double __VERIFIER_nondet_double(void);
extern void abort(void);
extern void __assert_fail(const char *, const char *, unsigned int, const char *) __attribute__ ((__nothrow__ , __leaf__)) __attribute__ ((__noreturn__));
void reach_error(void) { __assert_fail("0", "Req1_Prop1_Batch137Wrapper_S.c", 13, "reach_error"); }
void __VERIFIER_assert(int cond) { if(!(cond)) { ERROR: {reach_error();abort();} } return; }
void assume_abort_if_not(int cond) { if(!cond) { abort(); } }
struct WrapperStruct00;
struct WrapperStruct00 {
 unsigned char var_1_1;
 signed short int var_1_2;
 signed short int var_1_3;
 signed long int var_1_4;
 unsigned char var_1_5;
 unsigned char var_1_6;
 signed char var_1_8;
 signed char var_1_10;
 unsigned char var_1_11;
 double var_1_12;
 double var_1_13;
 unsigned char var_1_14;
 unsigned char var_1_15;
 unsigned char var_1_16;
 unsigned long int var_1_17;
 unsigned char var_1_18;
};
void initially(void);
void step(void);
void updateVariables(void);
void updateLastVariables(void);
int property(void);
int main(void);
unsigned char isInitial = 0;
struct WrapperStruct00 WrapperStruct00 = {
 10,
 16,
 -256,
 -5,
 50,
 4,
 10,
 32,
 1,
 10.5,
 7.2,
 0,
 1,
 1,
 32,
 1
};
void initially(void) {
}
void step(void) {
 WrapperStruct00.var_1_16 = WrapperStruct00.var_1_15;
 WrapperStruct00.var_1_17 = 16u;
 WrapperStruct00.var_1_18 = WrapperStruct00.var_1_14;
 signed short int stepLocal_1 = WrapperStruct00.var_1_2;
 signed long int stepLocal_0 = WrapperStruct00.var_1_4;
 if ((WrapperStruct00.var_1_2 + WrapperStruct00.var_1_3) < stepLocal_0) {
  if (stepLocal_1 <= WrapperStruct00.var_1_4) {
   WrapperStruct00.var_1_1 = ((((WrapperStruct00.var_1_5) < (WrapperStruct00.var_1_6)) ? (WrapperStruct00.var_1_5) : (WrapperStruct00.var_1_6)));
  } else {
   WrapperStruct00.var_1_1 = WrapperStruct00.var_1_6;
  }
 } else {
  if (WrapperStruct00.var_1_18) {
   WrapperStruct00.var_1_1 = WrapperStruct00.var_1_5;
  } else {
   WrapperStruct00.var_1_1 = WrapperStruct00.var_1_6;
  }
 }
 if ((WrapperStruct00.var_1_12 / 1.3) <= WrapperStruct00.var_1_13) {
  if (((WrapperStruct00.var_1_5 * WrapperStruct00.var_1_17) >= WrapperStruct00.var_1_6) && WrapperStruct00.var_1_18) {
   WrapperStruct00.var_1_11 = (! WrapperStruct00.var_1_14);
  } else {
   WrapperStruct00.var_1_11 = WrapperStruct00.var_1_15;
  }
 }
 if (WrapperStruct00.var_1_11) {
  if (WrapperStruct00.var_1_11) {
   WrapperStruct00.var_1_8 = ((((WrapperStruct00.var_1_10) < 0 ) ? -(WrapperStruct00.var_1_10) : (WrapperStruct00.var_1_10)));
  } else {
   WrapperStruct00.var_1_8 = ((((WrapperStruct00.var_1_10) > (2)) ? (WrapperStruct00.var_1_10) : (2)));
  }
 }
}
void updateVariables(void) {
 WrapperStruct00.var_1_2 = __VERIFIER_nondet_short();
 assume_abort_if_not(WrapperStruct00.var_1_2 >= -32768);
 assume_abort_if_not(WrapperStruct00.var_1_2 <= 32767);
 WrapperStruct00.var_1_3 = __VERIFIER_nondet_short();
 assume_abort_if_not(WrapperStruct00.var_1_3 >= -32768);
 assume_abort_if_not(WrapperStruct00.var_1_3 <= 32767);
 WrapperStruct00.var_1_4 = __VERIFIER_nondet_long();
 assume_abort_if_not(WrapperStruct00.var_1_4 >= -2147483648);
 assume_abort_if_not(WrapperStruct00.var_1_4 <= 2147483647);
 WrapperStruct00.var_1_5 = __VERIFIER_nondet_uchar();
 assume_abort_if_not(WrapperStruct00.var_1_5 >= 0);
 assume_abort_if_not(WrapperStruct00.var_1_5 <= 254);
 WrapperStruct00.var_1_6 = __VERIFIER_nondet_uchar();
 assume_abort_if_not(WrapperStruct00.var_1_6 >= 0);
 assume_abort_if_not(WrapperStruct00.var_1_6 <= 254);
 WrapperStruct00.var_1_10 = __VERIFIER_nondet_char();
 assume_abort_if_not(WrapperStruct00.var_1_10 >= -126);
 assume_abort_if_not(WrapperStruct00.var_1_10 <= 126);
 WrapperStruct00.var_1_12 = __VERIFIER_nondet_double();
 assume_abort_if_not((WrapperStruct00.var_1_12 >= -922337.2036854776000e+13F && WrapperStruct00.var_1_12 <= -1.0e-20F) || (WrapperStruct00.var_1_12 <= 9223372.036854776000e+12F && WrapperStruct00.var_1_12 >= 1.0e-20F ));
 WrapperStruct00.var_1_13 = __VERIFIER_nondet_double();
 assume_abort_if_not((WrapperStruct00.var_1_13 >= -922337.2036854776000e+13F && WrapperStruct00.var_1_13 <= -1.0e-20F) || (WrapperStruct00.var_1_13 <= 9223372.036854776000e+12F && WrapperStruct00.var_1_13 >= 1.0e-20F ));
 WrapperStruct00.var_1_14 = __VERIFIER_nondet_uchar();
 assume_abort_if_not(WrapperStruct00.var_1_14 >= 0);
 assume_abort_if_not(WrapperStruct00.var_1_14 <= 0);
 WrapperStruct00.var_1_15 = __VERIFIER_nondet_uchar();
 assume_abort_if_not(WrapperStruct00.var_1_15 >= 1);
 assume_abort_if_not(WrapperStruct00.var_1_15 <= 1);
}
void updateLastVariables(void) {
}
int property(void) {
 return (((((((WrapperStruct00.var_1_2 + WrapperStruct00.var_1_3) < WrapperStruct00.var_1_4) ? ((WrapperStruct00.var_1_2 <= WrapperStruct00.var_1_4) ? (WrapperStruct00.var_1_1 == ((unsigned char) ((((WrapperStruct00.var_1_5) < (WrapperStruct00.var_1_6)) ? (WrapperStruct00.var_1_5) : (WrapperStruct00.var_1_6))))) : (WrapperStruct00.var_1_1 == ((unsigned char) WrapperStruct00.var_1_6))) : (WrapperStruct00.var_1_18 ? (WrapperStruct00.var_1_1 == ((unsigned char) WrapperStruct00.var_1_5)) : (WrapperStruct00.var_1_1 == ((unsigned char) WrapperStruct00.var_1_6)))) && (WrapperStruct00.var_1_11 ? (WrapperStruct00.var_1_11 ? (WrapperStruct00.var_1_8 == ((signed char) ((((WrapperStruct00.var_1_10) < 0 ) ? -(WrapperStruct00.var_1_10) : (WrapperStruct00.var_1_10))))) : (WrapperStruct00.var_1_8 == ((signed char) ((((WrapperStruct00.var_1_10) > (2)) ? (WrapperStruct00.var_1_10) : (2)))))) : 1)) && (((WrapperStruct00.var_1_12 / 1.3) <= WrapperStruct00.var_1_13) ? ((((WrapperStruct00.var_1_5 * WrapperStruct00.var_1_17) >= WrapperStruct00.var_1_6) && WrapperStruct00.var_1_18) ? (WrapperStruct00.var_1_11 == ((unsigned char) (! WrapperStruct00.var_1_14))) : (WrapperStruct00.var_1_11 == ((unsigned char) WrapperStruct00.var_1_15))) : 1)) && (WrapperStruct00.var_1_16 == ((unsigned char) WrapperStruct00.var_1_15))) && (WrapperStruct00.var_1_17 == ((unsigned long int) 16u))) && (WrapperStruct00.var_1_18 == ((unsigned char) WrapperStruct00.var_1_14))
;
}
int main(void) {
 isInitial = 1;
 initially();
 while (1) {
  updateLastVariables();
  updateVariables();
  step();
  __VERIFIER_assert(property());
  isInitial = 0;
 }
 return 0;
}
