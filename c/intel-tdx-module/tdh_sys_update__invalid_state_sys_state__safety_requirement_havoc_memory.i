...
static inline void tdh_sys_update__common_precond() {
    ...
    assume_exit_if_not(leaf_opcode.leaf == TDH_SYS_UPDATE_LEAF);
}
...
static inline bool_t state_handoff_data_is_valid() {
    return (((handoff_data_header_t*) get_sysinfo_table()->data_rgn_base)->valid == ((bool_t)1));
}
...
void tdh_sys_update__invalid_state_sys_state__precond() {
    ...
    assume_exit_if_not(!state_sys_state_is_valid() && state_lp_state_is_valid() && state_handoff_data_is_valid() && state_handoff_version_is_valid());
}
void tdh_sys_update__invalid_state_sys_state__postcond() {
    tdx_module_local_t* local_data = get_local_data();
    __VERIFIER_assert((local_data->vmm_regs.rax == 0xC000050800000000ULL) || (local_data->vmm_regs.rax == 0x8000020200000000ULL));
...
int main() {
    ...
    tdh_sys_update__invalid_state_sys_state__precond();

    // Target method
    tdh_sys_update__call();

    // after_target methods
    tdh_sys_update__invalid_state_sys_state__postcond();
