...
void tdh_phymem_page_reclaim__unexpected__postcond() {
    ...
    __VERIFIER_assert((local_data->vmm_regs.rcx == reclaimed_page_pamt_entry_ptr->pt) || (local_data->vmm_regs.rcx == 0));
    __VERIFIER_assert((local_data->vmm_regs.rdx == page_owner_pa.raw) || (local_data->vmm_regs.rdx == 0));
    __VERIFIER_assert(( ((local_data->vmm_regs.r8 >> 3) == 0) && ((local_data->vmm_regs.r8 & 7) == (uint64_t) reclaimed_page_leaf_size) ) || (local_data->vmm_regs.r8 == 0));
...
int main() {
    ...
    tdh_phymem_page_reclaim__unexpected__precond();

    // Target method
    tdh_phymem_page_reclaim__call();

    // after_target methods
    tdh_phymem_page_reclaim__unexpected__postcond();
