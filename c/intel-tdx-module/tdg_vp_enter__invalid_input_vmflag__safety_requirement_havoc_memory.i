...
void tdg_vp_enter__invalid_input_vmflag__precond() {
    ...
    assume_exit_if_not(!input_vmflag_is_valid() && input_gpa_is_valid());
}
void tdg_vp_enter__invalid_input_vmflag__postcond() {
    ...
    __VERIFIER_assert(return_status == api_error_with_operand_id(0xC000010000000000ULL, 1ULL) || return_status == api_error_with_operand_id(0xC000010000000000ULL, 2ULL));
    tdg_vp_enter__common_postcond();
}
int main() {
    // before_target methods
    init_tdg_vp_xxx();
    tdg_vp_enter__invalid_input_vmflag__precond();

    // Target method
    tdg_vp_enter__call();

    // after_target methods
    tdg_vp_enter__invalid_input_vmflag__postcond();
