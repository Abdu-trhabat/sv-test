...
void tdh_phymem_page_reclaim__common_precond() {
    ...
    assume_abort_if_not(leaf_opcode.leaf == TDH_PHYMEM_PAGE_RECLAIM_LEAF);
...
void tdh_phymem_page_reclaim__invalid_entry(){
    ...
    pamt_entry_t* reclaimed_page_pamt_entry_ptr = ((void*)0);
    bool_t pamt_get_block_result = pamt_get_block(reclaimed_page_pa, &reclaimed_page_pamt_block);
    assume_abort_if_not(pamt_get_block_result == ((bool_t)1));
    ...
    assume_abort_if_not(pamt_walk_result == 0x0000000000000000ULL);
    ...
    if (reclaimed_page_pamt_entry_ptr->pt != PT_TDR){
        ...
        api_error_type map_result = lock_and_map_implicit_tdr(page_owner_pa, 128ULL, 1, TDX_LOCK_SHARED, &tdr_pamt_entry_ptr, &tdr_locked_flag, &tdr_ptr);
        if(map_result == 0x0000000000000000ULL) {
...
int main() {
    ...
    tdh_phymem_page_reclaim__invalid_entry();
