...
void tdh_sys_config__invalid_input_num_ptr__postcond() {
    tdx_module_local_t* local_data = get_local_data();
    __VERIFIER_assert((local_data->vmm_regs.rax == api_error_with_operand_id(0xC000010000000000ULL, 2ULL)) || (local_data->vmm_regs.rax == 0x8000020200000000ULL));
    tdh_sys_config__common_postcond();
}
int main() {
    // before_target methods
    init_tdh_sys_config();
    tdh_sys_config__invalid_input_num_ptr__precond();

    // Target method
    tdh_sys_config__call();

    // after_target methods
    tdh_sys_config__invalid_input_num_ptr__postcond();
