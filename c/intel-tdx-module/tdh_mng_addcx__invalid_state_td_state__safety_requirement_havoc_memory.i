...
void tdh_mng_addcx__invalid_state_td_state__postcond() {
    tdx_module_local_t* local_data = get_local_data();
    __VERIFIER_assert((local_data->vmm_regs.rax == 0xE000060400000000ULL) || ((local_data->vmm_regs.rax >> 32) == (0x8000020000000000ULL >> 32)));
    tdh_mng_addcx__common_postcond();
}
int main() {
    // before_target methods
    init_tdh_mng_xxx();
    tdh_mng_addcx__invalid_state_td_state__precond();

    // Target method
    tdh_mng_addcx__call();

    // after_target methods
    tdh_mng_addcx__invalid_state_td_state__postcond();
