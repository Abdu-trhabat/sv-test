...
void tdh_mr_finalize__unexpected__precond() {
    tdh_mr_finalize__common_precond();
    assume_exit_if_not(!all_conditions_valid());
}
void tdh_mr_finalize__unexpected__postcond() {
    tdx_module_local_t* local_data = get_local_data();
...
int main() {
    ...
    init_tdh_mr_xxx();
    tdh_mr_finalize__unexpected__precond();

    // Target method
    tdh_mr_finalize__call();

    // after_target methods
    tdh_mr_finalize__unexpected__postcond();
