...
static inline void tdg_sys_rd__common_precond() {
    ...
    assume_exit_if_not(leaf_opcode.leaf == TDG_SYS_RD_LEAF);
}
...
void tdg_sys_rd__unexpected__postcond() {
    ...
    __VERIFIER_assert(tdx_local_data_ptr->vp_ctx.tdvps->guest_state.gpr_state.rdx == -1);
...
int main() {
    ...
    init_tdg_sys_xxx();
    tdg_sys_rd__unexpected__precond();

    // Target method
    tdg_sys_rd__call();

    // after_target methods
    tdg_sys_rd__unexpected__postcond();
    close_tdg_sys_xxx_with_check();
    insert_cover_point();
