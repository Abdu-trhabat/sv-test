...
extern const state_flags_t state_flags_lookup[11];
typedef enum {
    PT_NDA = 0,
    PT_RSVD = 1,
    PT_REG = 3,
...
const state_flags_t state_flags_lookup[11] = {
  [0] = {
...
static inline bool_t op_state_is_any_initialized(op_state_e op_state)
{
                                              ;
...
api_error_type tdh_mng_vpflushdone(uint64_t target_tdr_pa)
{
    ...
    global_data_ptr->kot.entries[curr_hkid].state = (uint8_t)2;
EXIT:
    if (kot_locked_flag)
    {
...
static inline void tdh_mng_vpflushdone__common_precond() {
    ...
    assume_exit_if_not(leaf_opcode.leaf == TDH_MNG_VPFLUSHDONE_LEAF);
}
...
void tdh_mng_vpflushdone__invalid_state_op_state__precond() {
    ...
    assume_exit_if_not(input_tdr_hkid_is_valid() && state_tdr_metadata_is_valid() && state_tdr_lifecycle_is_valid() && !state_op_state_is_valid());
}
void tdh_mng_vpflushdone__invalid_state_op_state__postcond() {
    tdx_module_local_t* local_data = get_local_data();
    __VERIFIER_assert((local_data->vmm_regs.rax == 0x8000082400000000ULL) || ((local_data->vmm_regs.rax >> 32) == (0x8000020000000000ULL >> 32)));
    tdh_mng_vpflushdone__common_postcond();
}
int main() {
    // before_target methods
    init_tdh_mng_xxx();
    tdh_mng_vpflushdone__invalid_state_op_state__precond();

    // Target method
    tdh_mng_vpflushdone__call();

    // after_target methods
    tdh_mng_vpflushdone__invalid_state_op_state__postcond();
