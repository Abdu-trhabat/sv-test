...
static inline void tdg_servtd_wr__common_precond() {
    ...
    assume_exit_if_not(leaf_opcode.leaf == TDG_SERVTD_WR_LEAF);
    target_tdr_ptr = ((void*)0);
    target_tdr_pamt_entry_ptr = ((void*)0);
    target_tdcs_ptr = ((void*)0);
    pa_t target_tdr_pa;
    uint64_t target_slot;
    ...
    if (target_slot < 1) {
        ...
        assume_exit_if_not(fv_invariant_tdr_t(tmp_target_tdr_ptr));
        do { assume_exit_if_not(target_tdr_ptr == tmp_target_tdr_ptr); if (1) { target_tdr_ptr = tmp_target_tdr_ptr; } } while (0);
        if (map_tdr_status == 0x0000000000000000ULL) {
            ...
            if (map_tdcs_status == 0x0000000000000000ULL) {
                tdcs_t* tmp_target_tdcs_ptr = TDXFV_malloc(sizeof(tdcs_t));
                TDXFV_NONDET_struct_tdcs_t(tmp_target_tdcs_ptr);
                assume_exit_if_not(fv_invariant_tdcs_t(tmp_target_tdcs_ptr));
                do { assume_exit_if_not(target_tdcs_ptr == tmp_target_tdcs_ptr); if (1) { target_tdcs_ptr = tmp_target_tdcs_ptr; } } while (0);
            }
        }
    }
}
void tdg_servtd_wr__free() {
    if (target_tdr_ptr != ((void*)0)) {
        free(target_tdr_ptr);
    }
    if (target_tdcs_ptr != ((void*)0)) {
...
void tdg_servtd_wr__expected__precond() {
    ...
    assume_exit_if_not(input_field_is_valid());
}
...
int main() {
    ...
    init_tdg_servtd_xxx();
    tdg_servtd_wr__expected__precond();

    // Target method
    tdg_servtd_wr__call();

    // after_target methods
    tdg_servtd_wr__expected__postcond();
    tdg_servtd_wr__free();
    close_tdg_servtd_xxx_with_check();
    insert_cover_point();
