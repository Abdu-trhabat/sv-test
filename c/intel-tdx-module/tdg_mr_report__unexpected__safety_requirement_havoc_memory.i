...
static inline void tdg_mr_report__common_precond() {
    ...
    assume_exit_if_not(leaf_opcode.leaf == TDG_MR_REPORT_LEAF);
}
...
void tdg_mr_report__unexpected__precond() {
    ...
    assume_exit_if_not(!input_r8_is_valid());
}
void tdg_mr_report__unexpected__postcond() {
    __VERIFIER_assert(get_local_data()->vp_ctx.tdvps->guest_state.gpr_state.rax != 0x0000000000000000ULL);
    tdg_mr_report__common_postcond();
}
int main() {
    // before_target methods
    init_tdg_mr_report();
    tdg_mr_report__unexpected__precond();

    // Target method
    tdg_mr_report__call();

    // after_target methods
    tdg_mr_report__unexpected__postcond();
    close_tdg_mr_report_with_check();
