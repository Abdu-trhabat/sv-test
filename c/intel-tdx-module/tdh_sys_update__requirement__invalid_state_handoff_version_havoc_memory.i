...
void tdh_sys_update__common_precond() {
    ...
    assume_abort_if_not(leaf_opcode.leaf == TDH_SYS_UPDATE_LEAF);
}
void tdh_sys_update__invalid_state_handoff_version(){
    ...
    assume_abort_if_not(global_data->num_of_init_lps == global_data_fv.num_of_lps);
    assume_abort_if_not(((handoff_data_header_t*) sysinfo_table->data_rgn_base)->valid == ((bool_t)1));
    assume_abort_if_not((((handoff_data_header_t*) sysinfo_table->data_rgn_base)->hv < global_data_fv.min_update_hv) || (((handoff_data_header_t*) sysinfo_table->data_rgn_base)->hv > global_data_fv.module_hv));
...
static inline void copy_global_field_from_handoff(void* field_ptr, uint32_t field_size, uint8_t** data)
{
...
void retrieve_handoff_data(uint16_t hv, uint32_t size, uint8_t* data)
{
    tdx_module_global_t* g_d = get_global_data();
                             ;
    (void)(hv);
    uint32_t total_required_size = sizeof(((tdx_module_global_t *)0)->kot.entries) + sizeof(((tdx_module_global_t *)0)->wbt_entries) + sizeof(((tdx_module_global_t *)0)->tdmr_table) + sizeof(((tdx_module_global_t *)0)->num_of_tdmr_entries) + sizeof(((tdx_module_global_t *)0)->hkid) + sizeof(((tdx_module_global_t *)0)->pkg_config_bitmap);
    if ( __builtin_expect((!(total_required_size <= size)), 0) ) { ; tdx_report_error_and_halt(0x0002, 5); };
...
api_error_type tdh_sys_update(void)
{
    ...
    if ( __builtin_expect((!((size > 0) && (size <= buff_size))), 0) ) { ; tdx_report_error_and_halt((0xB000 | ((TDH_SYS_UPDATE_LEAF) & 0xFF)), 0); };
    ...
    handoff_data_hdr->valid = ((bool_t)0);
...
int main() {
    ...
    tdh_sys_update__invalid_state_handoff_version();
