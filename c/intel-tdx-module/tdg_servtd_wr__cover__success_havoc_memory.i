...
static inline void break_servtd_binding_handle(servtd_binding_handle_t handle, uint256_t servtd_uuid,
...
extern int memcmp (const void *__s1, const void *__s2, size_t __n)
     __attribute__ ((__nothrow__ )) __attribute__ ((__pure__)) __attribute__ ((__nonnull__ (1, 2)));
                                         ...
                                         ;
                                          ;
                                                  ;
static inline bool_t is_equal_256bit(uint256_t a, uint256_t b)
{
...
static inline bool_t tdx_memcmp(void * a, void * b, uint64_t nbytes)
{
...
void tdg_servtd_wr__common_precond() {
    ...
    assume_abort_if_not(leaf_opcode.leaf == TDG_SERVTD_WR_LEAF);
}
...
void tdg_servtd_wr__post_cover_success() {
    tdx_module_local_t* tdx_local_data_ptr = get_local_data();
    assume_abort_if_not(tdx_local_data_ptr->vp_ctx.tdvps->guest_state.gpr_state.rax == 0x0000000000000000ULL);
        ...
        )
{
    ...
    errc = hpa_check_with_pwr_2_alignment(tdr_hpa, (4 * 0x400ULL));
    if (errc != 0x0000000000000000ULL)
    {
    ...
    errc = non_shared_hpa_metadata_check_and_lock(tdr_hpa, lock_type,
            expected_pt, pamt_block, pamt_entry, &leaf_size, ((bool_t)1), ((bool_t)1));
    if (errc != 0x0000000000000000ULL)
    {
        return api_error_with_operand_id(errc, operand_id);
    }
    *is_locked = ((bool_t)1);
    pa_t hpa_with_hkid = assign_hkid_to_hpa(((void*)0), tdr_hpa);
        ...
        )
{
    return check_any_td_state_map_tdcs_and_lock(tdr_p, mapping_type, op_state_lock_type,
                                                map_migsc_links, current_leaf, ((bool_t)1), guest_side_flow, tdcs_p);
}
bool_t check_gpa_validity(
        pa_t gpa,
        bool_t gpaw,
                           ...
                           pa_t target_td)
{
    ...
    async_tdexit_internal(tdexit_case, empty_reason, 0, cross_td_status, target_td.raw, 0, ((bool_t)0));
}
void td_vmexit_to_vmm(uint8_t vcpu_state, uint8_t last_td_exit, uint64_t scrub_mask,
                      uint16_t xmm_select, bool_t is_td_dead, bool_t is_trap_exit)
{
...
static inline bool_t is_operand_busy_error_code(api_error_type error)
{
    if ((((error) >> 32) & 0xFFFFFFFFULL) == (((0x8000020000000000ULL) >> 32) & 0xFFFFFFFFULL))
    {
        return ((bool_t)1);
    }
    return ((bool_t)0);
                                       ...
                                       bool_t write, uint64_t wr_value, uint64_t wr_request_mask)
{
    ...
    tdr_t * target_tdr_ptr = ((void*)0);
    pamt_block_t target_tdr_pamt_block;
    pamt_entry_t * target_tdr_pamt_entry_ptr = ((void*)0);
    bool_t target_tdr_locked_flag = ((bool_t)0);
    tdcs_t * target_tdcs_ptr = ((void*)0);
    bool_t target_bindings_locked_flag = ((bool_t)0);
    uint256_t target_uuid;
    pa_t target_tdr_pa;
    uint64_t target_slot;
    __attribute__ ((aligned(64))) measurement_t tdinfo_hash = { 0 };
    ...
    api_error_type cross_td_trap_status = 0x0000000000000000ULL;
    ...
    if (!write)
    {
        lp->vp_ctx.tdvps->guest_state.gpr_state.rdx = ((uint64_t)(-1ULL));
    }
    ...
    return_val = othertd_check_state_map_tdcs_and_lock(target_tdr_ptr,
                                                       TDX_RANGE_RW, TDX_LOCK_SHARED, ((bool_t)1),
                                                       write ? TDG_SERVTD_WR_LEAF : TDG_SERVTD_RD_LEAF, ((bool_t)1), &target_tdcs_ptr);
    if (return_val != 0x0000000000000000ULL)
    {
        if (is_operand_busy_error_code(return_val))
        {
     ...
     if (!is_equal_256bit(target_tdr_ptr->management_fields.td_uuid, target_uuid))
     {
         if (is_equal_256bit(target_tdcs_ptr->migration_fields.preimport_uuid, target_uuid))
         {
             ...
             return_val = 0xC0000D0800000000ULL;
                                                           ;
             goto EXIT;
         }
         else
         {
             return_val = 0xC0000D0700000000ULL;
                                          ;
             goto EXIT;
         }
     }
     if ((return_val = acquire_sharex_lock_hp(&(target_tdcs_ptr->service_td_fields.servtd_bindings_lock),
                                             TDX_LOCK_SHARED, ((bool_t)1))) != 0x0000000000000000ULL)
     {
         return_val = api_error_with_operand_id(return_val, 174ULL);
                                                    ;
         goto EXIT;
     }
     target_bindings_locked_flag = ((bool_t)1);
     if (target_tdcs_ptr->service_td_fields.servtd_bindings_table[target_slot].state != SERVTD_BOUND)
     {
         cross_td_trap_status = api_error_with_operand_id(0xC0000D0500000000ULL, target_slot);
         goto EXIT;
     }
     ...
     {
         cross_td_trap_status = 0xC0000D0400000000ULL;
         goto EXIT;
     }
     if ((return_val = get_tdinfo_and_teeinfohash(lp->vp_ctx.tdcs,
             ...
             ((void*)0), &tdinfo_hash, ((bool_t)1))) != 0x0000000000000000ULL)
     {
     ...
     {
         cross_td_trap_status = 0xC0000D0300000000ULL;
         goto EXIT;
     }
...
EXIT:
    ...
    if ( __builtin_expect((cross_td_trap_status != 0x0000000000000000ULL), 0) )
    {
        async_tdexit_cross_td(0x9000000900000000ULL, cross_td_trap_status, target_tdr_pa);
    }
    return return_val;
}
api_error_type tdg_servtd_wr(uint64_t req_binding_handle, uint64_t requested_field_code,
                             uint64_t wr_data, uint64_t wr_mask)
{
    ...
    return tdg_servtd_rd_wr(binding_handle, field_code, ((bool_t)1), wr_data, wr_mask);
}
...
int main() {
    ...
    tdg_servtd_wr__free_entry();
    tdg_servtd_wr__post_cover_success();
    return 0;
}