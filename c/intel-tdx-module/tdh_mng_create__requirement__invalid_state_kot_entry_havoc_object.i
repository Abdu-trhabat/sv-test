...
void tdh_mng_create__common_precond() {
    ...
    assume_abort_if_not(leaf_opcode.leaf == TDH_MNG_CREATE_LEAF);
}
void tdh_mng_create__invalid_state_kot_entry() {
    ...
    assume_abort_if_not(((tdr_pa.full_pa & global_data->hkid_mask) >> global_data->hkid_start_bit) == 0);
    assume_abort_if_not((hkid_info.reserved == 0) && (hkid_info.hkid >= global_data->private_hkid_min) && (hkid_info.hkid <= global_data->private_hkid_max));
...
int main() {
    ...
    tdh_mng_create__invalid_state_kot_entry();
