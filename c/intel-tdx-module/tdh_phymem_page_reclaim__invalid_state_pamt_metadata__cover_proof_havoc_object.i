...
void tdh_phymem_page_reclaim__invalid_state_pamt_metadata__precond() {
    ...
    assume_exit_if_not(input_pa_is_valid() && !state_pamt_metadata_is_valid());
}
void tdh_phymem_page_reclaim__invalid_state_pamt_metadata__postcond() {
    tdx_module_local_t* local_data = get_local_data();
    __VERIFIER_assert((local_data->vmm_regs.rax == 0xC000030000000000ULL) || ((local_data->vmm_regs.rax >> 32) == (0x8000020000000000ULL >> 32)));
...
int main() {
    ...
    tdh_phymem_page_reclaim__invalid_state_pamt_metadata__precond();

    // Target method
    tdh_phymem_page_reclaim__call();

    // after_target methods
    tdh_phymem_page_reclaim__invalid_state_pamt_metadata__postcond();
