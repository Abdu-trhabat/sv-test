...
void tdh_mr_finalize__common_precond() {
    ...
    assume_abort_if_not(leaf_opcode.leaf == TDH_MR_FINALIZE_LEAF);
}
void tdh_mr_finalize__invalid_state_lifecycle() {
    ...
    assume_abort_if_not((tdr_fv.management_fields.num_tdcx >= MIN_NUM_TDCS_PAGES) && (tdr_fv.management_fields.num_tdcx <= MAX_NUM_TDCS_PAGES) && (tdcs_fv.management_fields.op_state == OP_STATE_INITIALIZED));
    ...
    api_error_type map_result = check_lock_and_map_explicit_tdr(
        (pa_t) local_data->vmm_regs.rcx, 1ULL, 1, TDX_LOCK_EXCLUSIVE, PT_TDR,
        &tdr_pamt_block, &tdr_pamt_entry_ptr, &tdr_locked_flag, &tdr_ptr
    );
    assume_abort_if_not(map_result == 0x0000000000000000ULL);
    assume_abort_if_not(tdr_pamt_entry_ptr->pt == PT_TDR);
    local_data->vmm_regs.rax = tdh_mr_finalize(local_data->vmm_regs.rcx);
    __VERIFIER_assert(local_data->vmm_regs.rax == 0x8000081000000000ULL);
}
...
int main() {
    ...
    tdh_mr_finalize__invalid_state_lifecycle();
