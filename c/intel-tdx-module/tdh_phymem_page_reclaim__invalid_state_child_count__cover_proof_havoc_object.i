...
{
    tdx_module_local_t * local_data_ptr = get_local_data();
    pa_t reclaimed_page_pa = {.raw = page_pa};
    pamt_block_t reclaimed_page_pamt_block;
    pamt_entry_t * reclaimed_page_pamt_entry_ptr;
    bool_t reclaimed_page_pamt_locked_flag = ((bool_t)0);
    page_size_t reclaimed_page_leaf_size;
    page_size_api_input_t reclaimed_page_level = {.raw = 0};
    pa_t page_owner_pa = {.raw = 0};
    ...
    if (!is_addr_aligned_pwr_of_2(reclaimed_page_pa.raw, (4 * 0x400ULL)) ||
    ...
    {
                                                                    ;
        return_val = api_error_with_operand_id(0xC000010100000000ULL, 1ULL);
        goto EXIT;
    }
    if ((return_val = pamt_walk(reclaimed_page_pa, reclaimed_page_pamt_block, TDX_LOCK_EXCLUSIVE,
                                &reclaimed_page_leaf_size, ((bool_t)0), ((bool_t)0), &reclaimed_page_pamt_entry_ptr)) != 0x0000000000000000ULL)
    {
                                                                    ;
    ...
    reclaimed_page_pamt_locked_flag = ((bool_t)1);
    if ((reclaimed_page_pamt_entry_ptr->pt == PT_NDA) ||
        (reclaimed_page_pamt_entry_ptr->pt == PT_RSVD))
    {
                                                        ;
        return_val = api_error_with_operand_id(0xC000030000000000ULL, 1ULL);
        goto EXIT;
    }
    ...
    if ((reclaimed_page_leaf_size > PT_4KB) &&
        (!is_addr_aligned_pwr_of_2(reclaimed_page_pa.raw, (4 * 0x400ULL) << (9 * reclaimed_page_leaf_size))))
    {
                                                                                                       ;
        return_val = api_error_with_operand_id(0xC000010000000000ULL, 1ULL);
        goto EXIT;
    ...
    if (reclaimed_page_pamt_entry_ptr->pt != PT_TDR)
    {
        ...
        {
                                                                ;
            return_val = 0xC000060700000000ULL;
            goto EXIT;
        }
    ...
    else
    {
        ...
        {
                                                          ;
            return_val = 0xC000040000000000ULL;
            goto EXIT;
        }
    }
...
static inline void tdh_phymem_page_reclaim__common_precond() {
    ...
    assume_exit_if_not(leaf_opcode.leaf == TDH_PHYMEM_PAGE_RECLAIM_LEAF);
    ...
    reclaimed_page_pamt_entry_ptr = ((void*)0);
}
...
static inline bool_t state_pamt_metadata_is_valid() {
    ...
    api_error_code_e pamt_walk_result = pamt_walk(
        ...
        &reclaimed_page_leaf_size, ((bool_t)0), ((bool_t)0), &reclaimed_page_pamt_entry_ptr
    );
    if (!(
        (pamt_walk_result == 0x0000000000000000ULL) &&
        (reclaimed_page_pamt_entry_ptr != ((void*)0))
    )) {
        return ((bool_t)0);
    }
...
static inline bool_t state_lifecycle_is_valid() {
    if (reclaimed_page_pamt_entry_ptr == ((void*)0)) {
        return ((bool_t)0);
    }
    tdr_t* tdr_ptr = ((void*)0);
    if (reclaimed_page_pamt_entry_ptr->pt != PT_TDR){
        ...
        api_error_type map_result = lock_and_map_implicit_tdr(
            page_owner_pa, 128ULL, 1, TDX_LOCK_SHARED, &tdr_pamt_entry_ptr, &tdr_locked_flag, &tdr_ptr
        );
        if (!(map_result == 0x0000000000000000ULL && tdr_ptr != ((void*)0))) {
            return ((bool_t)0);
        }
    } else {
...
void tdh_phymem_page_reclaim__invalid_state_child_count__precond() {
    ...
    assume_exit_if_not(input_pa_is_valid() && state_pamt_metadata_is_valid() && state_lifecycle_is_valid() && !state_child_count_is_valid());
}
void tdh_phymem_page_reclaim__invalid_state_child_count__postcond() {
    tdx_module_local_t* local_data = get_local_data();
    __VERIFIER_assert((local_data->vmm_regs.rax == 0xC000040000000000ULL) || ((local_data->vmm_regs.rax >> 32) == (0x8000020000000000ULL >> 32)));
    __VERIFIER_assert(local_data->vmm_regs.rcx == reclaimed_page_pamt_entry_ptr->pt);
    __VERIFIER_assert(local_data->vmm_regs.rdx == page_owner_pa.raw);
    __VERIFIER_assert((local_data->vmm_regs.r8 >> 3) == 0);
    __VERIFIER_assert((local_data->vmm_regs.r8 & 7) == (uint64_t) reclaimed_page_leaf_size);
    __VERIFIER_assert(local_data->vmm_regs.r9 == 0);
    __VERIFIER_assert(local_data->vmm_regs.r10 == 0);
    __VERIFIER_assert(local_data->vmm_regs.r11 == 0);
    tdh_phymem_page_reclaim__common_postcond();
}
int main() {
    // before_target methods
    init_tdh_phymem_reclaim();
    tdh_phymem_page_reclaim__invalid_state_child_count__precond();

    // Target method
    tdh_phymem_page_reclaim__call();

    // after_target methods
    tdh_phymem_page_reclaim__invalid_state_child_count__postcond();
    close_tdh_phymem_reclaim_with_check();
    insert_cover_point();
