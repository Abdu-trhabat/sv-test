...
void td_call__exit_status_success() {
    assume_exit_if_not(get_local_data()->vp_ctx.tdvps->guest_state.gpr_state.rax == 0x0000000000000000ULL);
}
...
int main() {
    ...
    init_tdg_mr_report();
    tdg_mr_report__unconstrained__precond();

    // Target method
    tdg_mr_report__call();

    // after_target methods
    td_call__exit_status_success();
    insert_cover_point();
    close_tdg_mr_report();
