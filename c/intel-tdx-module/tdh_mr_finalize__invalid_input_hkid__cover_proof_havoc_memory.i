...
typedef struct __attribute__((__packed__)) servtd_hash_buff_s
{
    measurement_t info_hash;
    uint16_t type;
...
IppStatus ippsHashUnpack_rmf (const Ipp8u* pBuffer, IppsHashState_rmf* pState);
...
void calculate_servtd_hash(tdcs_t* tdcs_ptr, bool_t handle_avx_state)
{
    servtd_hash_buff_t servtd_hash_buff[1];
    basic_memset_to_zero((void*)servtd_hash_buff, (sizeof(servtd_hash_buff_t) * 1));
    ...
    {
        __attribute__ ((aligned(16))) uint128_t xmms[16];
        if (handle_avx_state)
        {
...
api_error_type tdh_mr_finalize(uint64_t target_tdr_pa)
{
    ...
    return_val = check_state_map_tdcs_and_lock(tdr_ptr, TDX_RANGE_RW, TDX_LOCK_NO_LOCK,
                                               ((bool_t)0), TDH_MR_FINALIZE_LEAF, &tdcs_ptr);
    if (return_val != 0x0000000000000000ULL)
    {
                                                                                  ;
        goto EXIT;
    ...
    {
                                ;
        return_val = 0xC000060900000000ULL;
        goto EXIT;
    }
    store_xmms_in_buffer(xmms);
    ...
    calculate_servtd_hash(tdcs_ptr, ((bool_t)0));
    load_xmms_from_buffer(xmms);
    basic_memset_to_zero(xmms, sizeof(xmms));
...
void tdh_mr_finalize__call() {
    tdx_module_local_t* local_data = get_local_data();
    local_data->vmm_regs.rax = tdh_mr_finalize(local_data->vmm_regs.rcx);
}
static inline void tdh_mr_finalize__common_precond() {
    tdx_leaf_and_version_t leaf_opcode;
    leaf_opcode.raw = get_local_data()->vmm_regs.rax;
    assume_exit_if_not(leaf_opcode.leaf == TDH_MR_FINALIZE_LEAF);
}
static inline bool_t input_hkid_is_valid() {
...
static inline bool_t state_num_tdcx_and_op_state_is_valid() {
    return (
...
static inline bool_t state_tdr_metadata_is_valid() {
    ...
    api_error_type map_result = check_lock_and_map_explicit_tdr(
        (pa_t) get_local_data()->vmm_regs.rcx, 1ULL, 1, TDX_LOCK_EXCLUSIVE, PT_TDR,
        &tdr_pamt_block, &tdr_pamt_entry_ptr, &tdr_locked_flag, &tdr_ptr
    );
    if (!(
        map_result == 0x0000000000000000ULL &&
...
static inline void tdh_mr_finalize__common_postcond() {
...
void tdh_mr_finalize__invalid_input_hkid__precond() {
    tdh_mr_finalize__common_precond();
    assume_exit_if_not(!input_hkid_is_valid() && state_td_state_is_valid() && state_lifecycle_is_valid() && state_num_tdcx_and_op_state_is_valid() && state_tdr_metadata_is_valid());
}
void tdh_mr_finalize__invalid_input_hkid__postcond() {
    tdx_module_local_t* local_data = get_local_data();
    ...
    tdh_mr_finalize__common_postcond();
}
int main() {
    // before_target methods
    init_tdh_mr_xxx();
    tdh_mr_finalize__invalid_input_hkid__precond();

    // Target method
    tdh_mr_finalize__call();

    // after_target methods
    tdh_mr_finalize__invalid_input_hkid__postcond();
    close_tdh_mr_xxx_with_check();
    insert_cover_point();
