...
static inline void zero_area_cacheline(void* dst, uint64_t nBytes)
{
...
api_error_type tdh_mng_create(uint64_t target_tdr_pa, hkid_api_input_t hkid_info)
{
    ...
    lfence();
    if (global_data->kot.entries[td_hkid].state != 0)
    {
                                                                ;
        return_val = 0xC000082000000000ULL;
        goto EXIT;
    }
    zero_area_cacheline(tdr_ptr, (4 * 0x400ULL));
    ...
    global_data->kot.entries[td_hkid].state = (uint8_t)1;
...
static inline void tdh_mng_create__common_precond() {
    ...
    assume_exit_if_not(leaf_opcode.leaf == TDH_MNG_CREATE_LEAF);
}
...
static inline bool_t state_kot_entry_is_valid() {
    if (input_rdx_is_valid()) {
        hkid_api_input_t hkid_info = { .raw = get_local_data()->vmm_regs.rdx };
        return get_global_data()->kot.entries[hkid_info.hkid].state == 0;
    }
    return ((bool_t)1);
}
...
void tdh_mng_create__invalid_state_kot_entry__precond() {
    ...
    assume_exit_if_not(input_rcx_is_valid() && input_rdx_is_valid() && !state_kot_entry_is_valid() && state_tdr_page_is_valid());
}
...
int main() {
    ...
    init_tdh_mng_xxx();
    tdh_mng_create__invalid_state_kot_entry__precond();

    // Target method
    tdh_mng_create__call();

    // after_target methods
    tdh_mng_create__invalid_state_kot_entry__postcond();
