...
{
    int size;
    uint8_t state[((3 * 0x400ULL) + 512)];
} aes_gcm_ctx_t;
typedef uint8_t crypto_iv_t[12];
...
typedef struct __attribute__((__packed__)) mbmd_header_s
{
    union {
        struct {
            uint16_t size;
        ...
        uint8_t fields_included_in_mac[24];
    };
} mbmd_header_t;
typedef struct __attribute__((__packed__)) mbmd_mem_s
{
    mbmd_header_t header;
    uint64_t num_gpas : 16;
    uint64_t reserved : 48;
    uint8_t mac[16];
} mbmd_mem_t;
typedef struct __attribute__((__packed__)) mbmd_immutable_td_state_s
{
    mbmd_header_t header;
    uint16_t num_f_migs;
    uint16_t reserved_0;
...
typedef struct __attribute__((__packed__)) mbmd_td_state_s
{
    mbmd_header_t header;
    uint64_t reserved;
    uint8_t mac[16];
} mbmd_td_state_t;
typedef struct __attribute__((__packed__)) mbmd_vp_state_s
{
    mbmd_header_t header;
    uint64_t vp_index : 16;
    uint64_t reserved : 48;
    uint8_t mac[16];
} mbmd_vp_state_t;
typedef struct __attribute__((__packed__)) mbmd_epoch_token_s
{
    mbmd_header_t header;
    uint64_t total_mb;
    uint8_t mac[16];
} mbmd_epoch_token_t;
typedef struct __attribute__((__packed__)) mbmd_abort_token_s
{
    mbmd_header_t header;
    uint64_t reserved;
    uint8_t mac[16];
} mbmd_abort_token_t;
typedef struct __attribute__((__packed__)) mbmd_s
{
    union {
        mbmd_header_t header;
        mbmd_mem_t mem;
        mbmd_immutable_td_state_t immutable_td_state;
        mbmd_td_state_t td_state;
        mbmd_vp_state_t vp_state;
...
typedef struct __attribute__((__packed__)) migsc_s {
    uint16_t migsc_i;
    uint8_t reserved_0[6];
    uint64_t iv_counter;
    uint32_t next_mb_counter;
    uint32_t expected_mb_counter;
    struct {
    ...
    __attribute__ ((aligned(256))) aes_gcm_ctx_t aes_gcm_context;
} migsc_t;
...
static inline bool_t migsc_lock(migsc_link_t *mgsc_link)
{
    return _lock_bts_64b(&mgsc_link->raw, 0) == 0;
}
static inline void migsc_unlock(migsc_link_t *mgsc_link)
{
    bool_t old;
    old = _lock_btr_64b(&mgsc_link->raw, 0);
    if ( __builtin_expect((!(old)), 0) ) { ; tdx_report_error_and_halt(0x0001, 9); };
}
...
static inline void reset_to_next_iv(migsc_t *migsc, uint64_t iv_counter, uint16_t migs_index)
{
    migs_iv_t iv;
    iv.iv_counter = iv_counter;
    iv.migs_index = migs_index;
    iv.reserved = 0;
    if (aes_gcm_refresh_context(&migsc->aes_gcm_context) != 0)
    {
        { ; tdx_arch_fatal_error(); };
    }
    if (aes_gcm_reset(&migsc->aes_gcm_context, &iv) != 0)
    {
        { ; tdx_arch_fatal_error(); };
    }
}
...
void tdh_import_abort__common_precond() {
    tdx_module_local_t* local_data = get_local_data();
    tdx_leaf_and_version_t leaf_opcode;
    leaf_opcode.raw = local_data->vmm_regs.rax;
    assume_abort_if_not(leaf_opcode.leaf == TDH_IMPORT_ABORT_LEAF);
}
void tdh_import_abort__invalid_entry() {
    tdx_module_local_t* local_data = get_local_data();
    bool_t precond = ((bool_t)1);
    pa_t tdr_pa = { .raw = local_data->vmm_regs.rcx };
    precond = precond && (((tdr_pa.full_pa & global_data_fv.hkid_mask) >> global_data_fv.hkid_start_bit) == 0);
    hpa_and_size_t mbmd_hpa_and_size = { .raw = local_data->vmm_regs.r8 };
    precond = precond && (mbmd_hpa_and_size.size >= sizeof(mbmd_t));
    precond = precond && (local_data->vmm_regs.r10 == 0);
    precond = precond && (tdr_fv.management_fields.fatal == ((bool_t)0));
    ...
    precond = precond && (tdcs_fv.executions_ctl_fields.attributes.debug == ((bool_t)1));
    precond = precond && (
        (tdcs_fv.management_fields.op_state == OP_STATE_MEMORY_IMPORT) ||
        (tdcs_fv.management_fields.op_state == OP_STATE_STATE_IMPORT) ||
        (tdcs_fv.management_fields.op_state == OP_STATE_POST_IMPORT) ||
        (tdcs_fv.management_fields.op_state == OP_STATE_FAILED_IMPORT)
    );
...
void tdh_import_abort__common_postcond() {
...
IppStatus ippsAES_GCMGetTag (Ipp8u* pDstTag, int tagLen, const IppsAES_GCMState* pState);
static void clean_ctx(aes_gcm_ctx_t *ctx)
{
    basic_memset_to_zero(&ctx->state, ((3 * 0x400ULL) + 512));
}
aes_gcm_api_error aes_gcm_init(const key256_t *key, aes_gcm_ctx_t *ctx, const migs_iv_t *iv)
{
                         ;
    get_local_data()->reset_avx_state = ((bool_t)1);
    ...
    if (0 != status)
    {
        ...
        goto EXIT;
                                                             ...
                                                             ;
    if (ctx->size > (int)((3 * 0x400ULL) + 512))
    {
        status = -6;
                                                                                                                                 ;
        goto EXIT;
    }
    status = ippsAES_GCMInit((const Ipp8u*)key->bytes, 32, pState, ctx->size);
    if (0 != status)
    {
                                                        ;
        goto EXIT;
...
aes_gcm_api_error aes_gcm_finalize(aes_gcm_ctx_t *ctx, uint8_t *mac)
{
    ...
    get_local_data()->reset_avx_state = ((bool_t)1);
    IppsAES_GCMState *pState = (IppsAES_GCMState*) &ctx->state;
    IppStatus status = ippsAES_GCMGetTag((Ipp8u*)mac, 16, pState);
    if (0 != status)
    {
                                                        ;
    }
...
void migsc_init(migsc_t *migsc, const key256_t *key)
{
    ...
    migsc->interrupted_state.valid = ((bool_t)0);
    migs_iv_t iv;
    iv.iv_counter = 1ull;
    iv.migs_index = 0u;
    iv.reserved = 0;
    if (aes_gcm_init(key, &migsc->aes_gcm_context, &iv) != 0)
    {
        { ; tdx_arch_fatal_error(); };
    }
}
...
api_error_type tdh_import_abort(uint64_t target_tdr_pa, uint64_t hpa_and_size_pa, uint64_t migs_i)
{
    ...
    bool_t tdr_pamt_locked_flag = ((bool_t)0);
    mbmd_t *mbmd_p = ((void*)0);
    hpa_and_size_t mbmd_hpa_and_size;
    mbmd_t mbmd;
    api_error_type return_val = 0xC000010000000000ULL;
    migsc_t *migsc_p = ((void*)0);
    pa_t migsc_pa;
    bool_t migsc_locked_flag = ((bool_t)0);
    ...
    return_val = check_state_map_tdcs_and_lock(tdr_p, TDX_RANGE_RW, TDX_LOCK_NO_LOCK,
                                               ((bool_t)0), TDH_IMPORT_ABORT_LEAF, &tdcs_p);
    if (return_val != 0x0000000000000000ULL)
    {
                                                                                  ;
        goto EXIT;
    ...
    if (migs_i != 0)
    {
        return_val = api_error_with_operand_id(0xC000010000000000ULL, 10ULL);
        goto EXIT;
    }
    ...
    if ((return_val = shared_hpa_check_with_pwr_2_alignment(mbmd_hpa, 128)) != 0x0000000000000000ULL)
    {
        return_val = api_error_with_operand_id(return_val, 8ULL);
        goto EXIT;
    }
    lfence();
    if (!(migsc_lock(&tdcs_p->b_migsc_link)))
    {
        return_val = api_error_with_operand_id(0x8000020000000000ULL, 171ULL);
        goto EXIT;
    }
    migsc_locked_flag = ((bool_t)1);
    ...
    if (!tdcs_p->b_migsc_link.initialized)
    {
        ...
        tdcs_p->b_migsc_link.initialized = ((bool_t)1);
    }
    migsc_p->interrupted_state.valid = ((bool_t)0);
    migsc_p->iv_counter++;
    ...
    return_val = api_error_fatal(0x0000000000000000ULL);
EXIT:
    if (migsc_p != ((void*)0))
    {
        free_la(migsc_p);
...
int main() {
    ...
    tdh_import_abort__invalid_entry();
