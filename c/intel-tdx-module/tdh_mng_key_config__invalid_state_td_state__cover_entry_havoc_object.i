...
api_error_type tdh_mng_key_config(uint64_t target_tdr_pa)
{
    ...
    if (tdr_ptr->key_management_fields.pkg_config_bitmap & ((uint64_t)(1ULL<<(local_data->lp_info.pkg))))
    {
                                                                 ;
        return_val = 0x0000081500000000ULL;
        goto EXIT;
    }
    ...
    tdr_ptr->key_management_fields.pkg_config_bitmap |= (uint64_t)(1ULL<<(local_data->lp_info.pkg));
...
void tdh_mng_key_config__call() {
    tdx_module_local_t* local_data = get_local_data();
    local_data->vmm_regs.rax = tdh_mng_key_config(local_data->vmm_regs.rcx);
}
static inline void tdh_mng_key_config__common_precond() {
    tdx_leaf_and_version_t leaf_opcode;
    leaf_opcode.raw = get_local_data()->vmm_regs.rax;
    assume_exit_if_not(leaf_opcode.leaf == TDH_MNG_KEY_CONFIG_LEAF);
}
...
static inline bool_t state_tdr_lifecycle_is_valid() {
    return get_local_data()->vp_ctx.tdr->management_fields.lifecycle_state == TD_HKID_ASSIGNED;
}
void tdh_mng_key_config__invalid_state_td_state__precond() {
    tdh_mng_key_config__common_precond();
    assume_exit_if_not(input_rcx_is_valid() && state_tdr_metadata_is_valid() && !state_td_state_is_valid() && state_tdr_lifecycle_is_valid());
}
int main() {
    // before_target methods
    init_tdh_mng_xxx();
    tdh_mng_key_config__invalid_state_td_state__precond();
    insert_cover_point();

    // Target method
    tdh_mng_key_config__call();
