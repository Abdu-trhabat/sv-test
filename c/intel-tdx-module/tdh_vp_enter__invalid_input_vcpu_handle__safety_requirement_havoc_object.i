...
void tdh_vp_enter__invalid_input_vcpu_handle__postcond() {
    tdx_module_local_t* local_data = get_local_data();
    __VERIFIER_assert((local_data->vmm_regs.rax == api_error_with_operand_id(0xC000010000000000ULL, 1ULL)) || ((local_data->vmm_regs.rax >> 32) == (0x8000020000000000ULL >> 32)) || (local_data->vmm_regs.rax == 0xC000090100000000ULL));
    tdh_vp_enter__common_postcond_on_error();
}
int main() {
    // before_target methods
    init_tdh_vp_xxx();
    tdh_vp_enter__invalid_input_vcpu_handle__precond();

    // Target method
    tdh_vp_enter__call();

    // after_target methods
    tdh_vp_enter__invalid_input_vcpu_handle__postcond();
