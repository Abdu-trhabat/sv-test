...
void tdh_phymem_page_reclaim__valid_entry(){
    ...
    assume_abort_if_not((reclaimed_page_pamt_entry_ptr->pt != PT_NDA) && (reclaimed_page_pamt_entry_ptr->pt != PT_RSVD));
    ...
    if (reclaimed_page_pamt_entry_ptr->pt != PT_TDR){
        ...
        assume_abort_if_not(map_result == 0x0000000000000000ULL);
        assume_abort_if_not(tdr_ptr->management_fields.lifecycle_state == TD_TEARDOWN);
    } else {
        tdr_ptr = map_pa_with_global_hkid((void*)local_data->vmm_regs.rcx, 1);
        assume_abort_if_not(tdr_ptr->management_fields.lifecycle_state == TD_TEARDOWN);
        assume_abort_if_not(tdr_ptr->management_fields.chldcnt == 0);
    }
...
int main() {
    ...
    tdh_phymem_page_reclaim__valid_entry();
