...
void tdh_mng_key_config__invalid_state_tdr_lifecycle() {
    tdx_module_local_t* local_data = get_local_data();
    tdx_module_global_t* global_data = get_global_data();
    ...
    assume_abort_if_not(tdr_fv.management_fields.lifecycle_state != TD_HKID_ASSIGNED);
    local_data->vmm_regs.rax = tdh_mng_key_config(local_data->vmm_regs.rcx);
    __VERIFIER_assert(local_data->vmm_regs.rax == 0xC000060700000000ULL);
}
...
int main() {
    ...
    tdh_mng_key_config__invalid_state_tdr_lifecycle();
