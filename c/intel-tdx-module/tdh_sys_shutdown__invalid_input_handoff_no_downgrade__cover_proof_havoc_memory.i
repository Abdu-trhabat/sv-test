...
static inline void tdh_sys_shutdown__common_precond() {
    ...
    assume_exit_if_not(leaf_opcode.leaf == TDH_SYS_SHUTDOWN_LEAF);
}
...
void tdh_sys_shutdown__invalid_input_handoff_no_downgrade__precond() {
    ...
    assume_exit_if_not(input_handoff_version_is_valid_version() && !input_handoff_version_is_valid_no_downgrade());
}
...
int main() {
    ...
    tdh_sys_shutdown__invalid_input_handoff_no_downgrade__precond();

    // Target method
    tdh_sys_shutdown__call();

    // after_target methods
    tdh_sys_shutdown__invalid_input_handoff_no_downgrade__postcond();
