...
void fv_setup_tdmr_info() {
    bool_t all_tdmr_info_entries_are_valid = ((bool_t)1);
    for (int i = 0; i < 64; i++) {
        all_tdmr_info_entries_are_valid &= fv_invariant_tdmr_info_entry_t(&global_data_fv_ptr->tdmr_info_copy[i]);
    }
    bool_t all_tdmr_sorted_and_non_overlapping = ((bool_t)1);
    for (int i = 1; i < 64; i++) {
    ...
    assume_exit_if_not(all_tdmr_info_entries_are_valid && all_tdmr_sorted_and_non_overlapping);
}
void fv_teardown_with_check_tdmr_info() {
    bool_t all_tdmr_info_entries_are_valid = ((bool_t)1);
    for (int i = 0; i < 64; i++) {
    ...
    __VERIFIER_assert(all_tdmr_info_entries_are_valid && all_tdmr_sorted_and_non_overlapping);
}
...
static inline void tdh_sys_config__common_precond() {
    ...
    assume_exit_if_not(leaf_opcode.leaf == TDH_SYS_CONFIG_LEAF);
    tdmr_info_entry_t** tdmr_array = TDXFV_malloc(sizeof(tdmr_info_entry_t*) * 64);
    for (int i = 0; i < 64; i++) {
        tdmr_array[i] = TDXFV_malloc(sizeof(tdmr_info_entry_t));
        TDXFV_NONDET_struct_tdmr_info_entry_t(tdmr_array[i]);
    }
    assume_exit_if_not(get_local_data()->vmm_regs.rcx == (uint64_t)tdmr_array);
}
...
bool_t tdmr_info_entry_is_valid(tdmr_info_entry_t* tdmr_entry) {
    bool_t base_and_size_are_valid = (
        (tdmr_entry->tdmr_base % ((0x400ULL * 0x400ULL) * 0x400ULL) == 0) &&
        (tdmr_entry->tdmr_size > 0) &&
        (tdmr_entry->tdmr_size % ((0x400ULL * 0x400ULL) * 0x400ULL) == 0) &&
        (tdmr_entry->tdmr_base < (uint64_t)(1ULL<<(52ULL)))
    );
    bool_t pamt_bases_are_valid = (
        (tdmr_entry->pamt_1g_base < (uint64_t)(1ULL<<(52ULL))) &&
        (tdmr_entry->pamt_2m_base < (uint64_t)(1ULL<<(52ULL))) &&
        (tdmr_entry->pamt_4k_base < (uint64_t)(1ULL<<(52ULL)))
    );
...
static inline bool_t input_rcx_is_valid() {
    tdmr_info_entry_t** tdmr_array = (tdmr_info_entry_t**) get_local_data()->vmm_regs.rcx;
    bool_t input_tdmr_is_valid = ((bool_t)1);
    for (int idx = 0; idx < 64; idx++) {
        input_tdmr_is_valid &= tdmr_info_entry_is_valid(tdmr_array[idx]);
    }
    bool_t input_tdmr_not_overlapping = ((bool_t)1);
...
void tdh_sys_config__invalid_state_tdmr_info__precond() {
    ...
    assume_exit_if_not(input_r8_is_valid() && input_rdx_is_valid() && !input_rcx_is_valid() && state_sys_state_is_valid() && state_lp_state_is_valid());
}
void tdh_sys_config__invalid_state_tdmr_info__postcond() {
    tdx_module_local_t* local_data = get_local_data();
    __VERIFIER_assert((local_data->vmm_regs.rax == 0xC0000A1000000000ULL) || (local_data->vmm_regs.rax == 0xC0000A2000000000ULL) || (local_data->vmm_regs.rax == 0xC0000A0000000000ULL) || (local_data->vmm_regs.rax == 0xC0000A2100000000ULL) || (local_data->vmm_regs.rax == 0xC0000A0100000000ULL) || (local_data->vmm_regs.rax == 0xC0000A1100000000ULL) || (local_data->vmm_regs.rax == 0xC0000A1200000000ULL) || (local_data->vmm_regs.rax == 0x8000020200000000ULL) || (local_data->vmm_regs.rax == api_error_with_operand_id(0xC000010000000000ULL, 1ULL)));
    tdh_sys_config__common_postcond();
}
int main() {
    // before_target methods
    init_tdh_sys_config();
    tdh_sys_config__invalid_state_tdmr_info__precond();

    // Target method
    tdh_sys_config__call();

    // after_target methods
    tdh_sys_config__invalid_state_tdmr_info__postcond();
