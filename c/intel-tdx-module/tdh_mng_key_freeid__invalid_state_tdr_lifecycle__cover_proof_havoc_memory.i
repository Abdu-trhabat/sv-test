...
api_error_type tdh_mng_key_freeid(uint64_t target_tdr_pa)
{
    ...
    {
                                                    ;
        return_val = api_error_with_operand_id(0x8000020000000000ULL, 186ULL);
        goto EXIT;
    }
    kot_locked_flag = ((bool_t)1);
    curr_hkid = tdr_ptr->key_management_fields.hkid;
                                                                                             ;
    if (global_data_ptr->kot.entries[curr_hkid].wbinvd_bitmap != 0)
    {
                                                                             ;
        return_val = 0x8000081700000000ULL;
        goto EXIT;
    }
    global_data_ptr->kot.entries[curr_hkid].state = (uint8_t)0;
...
static inline void tdh_mng_key_freeid__common_precond() {
    ...
    assume_exit_if_not(leaf_opcode.leaf == TDH_MNG_KEY_FREEID_LEAF);
}
...
static inline bool_t state_kot_state_is_valid() {
    ...
    return (
        (get_global_data()->kot.entries[hkid].state == 2) &&
...
void tdh_mng_key_freeid__invalid_state_tdr_lifecycle__precond() {
    ...
    assume_exit_if_not(input_rcx_is_valid() && state_tdr_metadata_is_valid() && !state_tdr_lifecycle_is_valid() && state_kot_state_is_valid());
}
void tdh_mng_key_freeid__invalid_state_tdr_lifecycle__postcond() {
    tdx_module_local_t* local_data = get_local_data();
    __VERIFIER_assert((local_data->vmm_regs.rax == 0xC000060700000000ULL) || ((local_data->vmm_regs.rax >> 32) == (0x8000020000000000ULL >> 32)));
    tdh_mng_key_freeid__common_postcond();
}
int main() {
    // before_target methods
    init_tdh_mng_xxx();
    tdh_mng_key_freeid__invalid_state_tdr_lifecycle__precond();

    // Target method
    tdh_mng_key_freeid__call();

    // after_target methods
    tdh_mng_key_freeid__invalid_state_tdr_lifecycle__postcond();
