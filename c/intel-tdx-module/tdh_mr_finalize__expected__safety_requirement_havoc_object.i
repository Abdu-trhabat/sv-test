...
static inline bool_t all_conditions_valid() {
    return (
        ...
        state_num_tdcx_and_op_state_is_valid() &&
...
void tdh_mr_finalize__expected__precond() {
    tdh_mr_finalize__common_precond();
    assume_exit_if_not(all_conditions_valid());
}
void tdh_mr_finalize__expected__postcond() {
...
int main() {
    ...
    init_tdh_mr_xxx();
    tdh_mr_finalize__expected__precond();

    // Target method
    tdh_mr_finalize__call();

    // after_target methods
    tdh_mr_finalize__expected__postcond();
