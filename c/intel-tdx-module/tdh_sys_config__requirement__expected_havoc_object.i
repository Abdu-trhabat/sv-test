...
static inline bool_t is_private_hkid(uint16_t hkid)
{
...
void tdh_sys_config__common_precond() {
    ...
    assume_abort_if_not(leaf_opcode.leaf == TDH_SYS_CONFIG_LEAF);
...
void tdh_sys_config__valid_entry() {
    ...
    assume_abort_if_not(global_private_hkid.reserved == 0);
    assume_abort_if_not((global_private_hkid.hkid >= global_data->private_hkid_min) && (global_private_hkid.hkid <= global_data->private_hkid_max));
    assume_abort_if_not((local_data->vmm_regs.rdx >= 1) && (local_data->vmm_regs.rdx <= 64));
    tdmr_info_entry_t** tdmr_array = (tdmr_info_entry_t**) local_data->vmm_regs.rcx;
    for (int idx = 0; idx < local_data->vmm_regs.rdx; idx++) {
        assume_abort_if_not((tdmr_array[idx]->tdmr_base % ((0x400ULL * 0x400ULL) * 0x400ULL)) == 0);
        assume_abort_if_not((tdmr_array[idx]->tdmr_size > 0) && ((tdmr_array[idx]->tdmr_size % ((0x400ULL * 0x400ULL) * 0x400ULL)) == 0));
        assume_abort_if_not(tdmr_array[idx]->tdmr_base < (uint64_t)(1ULL<<(52ULL)));
        pa_t tdmr_base_pa = (pa_t) tdmr_array[idx]->tdmr_base;
        assume_abort_if_not(((tdmr_base_pa.full_pa * global_data->hkid_mask) >> global_data->hkid_start_bit) == 0);
        assume_abort_if_not(tdmr_array[idx]->pamt_1g_base < (uint64_t)(1ULL<<(52ULL)));
        pa_t pamt_1g_pa = (pa_t) tdmr_array[idx]->pamt_1g_base;
        assume_abort_if_not(((pamt_1g_pa.full_pa * global_data->hkid_mask) >> global_data->hkid_start_bit) == 0);
        assume_abort_if_not(tdmr_array[idx]->pamt_2m_base < (uint64_t)(1ULL<<(52ULL)));
        pa_t pamt_2m_pa = (pa_t) tdmr_array[idx]->pamt_2m_base;
        assume_abort_if_not(((pamt_2m_pa.full_pa * global_data->hkid_mask) >> global_data->hkid_start_bit) == 0);
        assume_abort_if_not(tdmr_array[idx]->pamt_4k_base < (uint64_t)(1ULL<<(52ULL)));
        pa_t pamt_4k_pa = (pa_t) tdmr_array[idx]->pamt_4k_base;
        assume_abort_if_not(((pamt_4k_pa.full_pa * global_data->hkid_mask) >> global_data->hkid_start_bit) == 0);
        for (int rsv_idx = 1; rsv_idx < 16U; rsv_idx++) {
            assume_abort_if_not(tdmr_array[idx]->rsvd_areas[rsv_idx-1].offset <= tdmr_array[idx]->rsvd_areas[rsv_idx].offset);
            assume_abort_if_not((tdmr_array[idx]->rsvd_areas[rsv_idx-1].size != 0) || (tdmr_array[idx]->rsvd_areas[rsv_idx].size == 0));
            assume_abort_if_not(tdmr_array[idx]->rsvd_areas[rsv_idx-1].offset + tdmr_array[idx]->rsvd_areas[rsv_idx-1].size <= tdmr_array[idx]->rsvd_areas[rsv_idx].offset);
        }
        for (int rsv_idx = 0; rsv_idx < 16U; rsv_idx++) {
            assume_abort_if_not(tdmr_array[idx]->rsvd_areas[rsv_idx].size >= 0);
            assume_abort_if_not(tdmr_array[idx]->rsvd_areas[rsv_idx].offset >= tdmr_array[idx]->tdmr_base);
            assume_abort_if_not((tdmr_array[idx]->rsvd_areas[rsv_idx].offset + tdmr_array[idx]->rsvd_areas[rsv_idx].size) < (tdmr_array[idx]->tdmr_base + tdmr_array[idx]->tdmr_size));
        }
    }
    for (int idx = 1; idx < local_data->vmm_regs.rdx; idx++) {
        assume_abort_if_not(tdmr_array[idx-1]->tdmr_base < tdmr_array[idx]->tdmr_base);
        assume_abort_if_not(tdmr_array[idx-1]->tdmr_base + tdmr_array[idx-1]->tdmr_size <= tdmr_array[idx]->tdmr_base);
    }
    assume_abort_if_not(global_data->global_state.sys_state == SYSINIT_DONE);
    assume_abort_if_not(global_data->num_of_init_lps == global_data->num_of_lps);
...
static inline void update_pamt_array (tdmr_info_entry_t* tdmr_info_copy, pamt_data_t pamt_data_array[], uint32_t i)
{
...
static inline void copy_tdmr_info_entry (tdmr_info_entry_t* tdmr_info_src, tdmr_info_entry_t* tdmr_info_target)
{
...
static api_error_type check_tdmr_area_addresses_and_size(tdmr_info_entry_t tdmr_info_copy[64], uint32_t i)
{
    if (!is_addr_aligned_pwr_of_2(tdmr_info_copy[i].tdmr_base, ((0x400ULL * 0x400ULL) * 0x400ULL)) ||
        !is_addr_aligned_pwr_of_2(tdmr_info_copy[i].tdmr_size, ((0x400ULL * 0x400ULL) * 0x400ULL)) ||
        tdmr_info_copy[i].tdmr_size <= 0)
    {
                                                                               ;
        return api_error_with_multiple_info(0xC0000A0000000000ULL, (uint8_t)i, 0, 0, 0);
    }
                                       ...
                                       uint64_t entry_size, uint64_t tdmr_size)
{
    ...
    if (!is_addr_aligned_pwr_of_2(pamt_base, (4 * 0x400ULL)) || !is_addr_aligned_pwr_of_2(pamt_size, (4 * 0x400ULL)))
    {
                                                                                                ;
        return ((bool_t)0);
    }
                                              ...
                                              uint64_t pamt_base, uint64_t pamt_size)
{
    ...
    bool_t valid_rsvd_area = ((bool_t)0);
...
static api_error_type check_pamt_overlaps_available_areas(tdmr_info_entry_t tdmr_info_copy[64], uint32_t i)
{
    for (uint32_t j=0; j <= i ; j++)
    {
        ...
        {
                                                                                      ;
            return api_error_with_multiple_info(0xC0000A1200000000ULL, (uint8_t)i, PT_4KB, (uint8_t)j, 0);
        }
        if (is_pamt_overlaps_available_area(&tdmr_info_copy[j], tdmr_info_copy[i].pamt_2m_base, tdmr_info_copy[i].pamt_2m_size))
        {
                                                                                      ;
            return api_error_with_multiple_info(0xC0000A1200000000ULL, (uint8_t)i, PT_2MB, (uint8_t)j, 0);
        }
        if (is_pamt_overlaps_available_area(&tdmr_info_copy[j], tdmr_info_copy[i].pamt_1g_base, tdmr_info_copy[i].pamt_1g_size))
        {
                                                                                      ;
            return api_error_with_multiple_info(0xC0000A1200000000ULL, (uint8_t)i, PT_1GB, (uint8_t)j, 0);
        }
        if (j < i)
        {
            ...
            {
                                                                                          ;
                return api_error_with_multiple_info(0xC0000A1200000000ULL, (uint8_t)j, PT_4KB, (uint8_t)i, 0);
            }
            if (is_pamt_overlaps_available_area(&tdmr_info_copy[i], tdmr_info_copy[j].pamt_2m_base, tdmr_info_copy[j].pamt_2m_size))
            {
                                                                                          ;
                return api_error_with_multiple_info(0xC0000A1200000000ULL, (uint8_t)j, PT_2MB, (uint8_t)i, 0);
            }
            if (is_pamt_overlaps_available_area(&tdmr_info_copy[i], tdmr_info_copy[j].pamt_1g_base, tdmr_info_copy[j].pamt_1g_size))
            {
                                                                                          ;
                return api_error_with_multiple_info(0xC0000A1200000000ULL, (uint8_t)j, PT_1GB, (uint8_t)i, 0);
            }
        }
    }
    return 0x0000000000000000ULL;
...
static api_error_type check_all_pamt_overlap(tdmr_info_entry_t tdmr_info_copy[64], uint32_t i,
        pamt_data_t pamt_data_array[])
{
    ...
    {
                                                             ;
        return api_error_with_multiple_info(0xC0000A1200000000ULL, (uint8_t)i, PT_4KB, (uint8_t)i, 0);
    }
    if (is_overlap(tdmr_info_copy[i].pamt_4k_base, tdmr_info_copy[i].pamt_4k_size,
    ...
    {
                                                             ;
        return api_error_with_multiple_info(0xC0000A1200000000ULL, (uint8_t)i, PT_2MB, (uint8_t)i, 0);
    }
    return 0x0000000000000000ULL;
}
static api_error_type check_pamt_addresses(tdmr_info_entry_t tdmr_info_copy[64], uint32_t i)
{
    if (!check_pamt_addresses_and_size(tdmr_info_copy[i].pamt_1g_base,
            tdmr_info_copy[i].pamt_1g_size, ((0x400ULL * 0x400ULL) * 0x400ULL), tdmr_info_copy[i].tdmr_size))
    {
                                                           ;
        return api_error_with_multiple_info(0xC0000A1000000000ULL, (uint8_t)i, PT_1GB, 0, 0);
    }
    if (!check_pamt_addresses_and_size(tdmr_info_copy[i].pamt_2m_base,
            tdmr_info_copy[i].pamt_2m_size, (2 * (0x400ULL * 0x400ULL)), tdmr_info_copy[i].tdmr_size))
    {
                                                           ;
        return api_error_with_multiple_info(0xC0000A1000000000ULL, (uint8_t)i, PT_2MB, 0, 0);
    }
    if (!check_pamt_addresses_and_size(tdmr_info_copy[i].pamt_4k_base,
            tdmr_info_copy[i].pamt_4k_size, (4 * 0x400ULL), tdmr_info_copy[i].tdmr_size))
    {
                                                           ;
        return api_error_with_multiple_info(0xC0000A1000000000ULL, (uint8_t)i, PT_4KB, 0, 0);
    }
    return 0x0000000000000000ULL;
}
static api_error_type check_pamt_in_cmr(tdmr_info_entry_t tdmr_info_copy[64], uint32_t i)
{
    if (!is_area_in_cmr(tdmr_info_copy[i].pamt_1g_base,
            tdmr_info_copy[i].pamt_1g_base + tdmr_info_copy[i].pamt_1g_size))
    {
                                                                            ;
        return api_error_with_multiple_info(0xC0000A1100000000ULL, (uint8_t)i, PT_1GB, 0, 0);
    }
    if (!is_area_in_cmr(tdmr_info_copy[i].pamt_2m_base,
            tdmr_info_copy[i].pamt_2m_base + tdmr_info_copy[i].pamt_2m_size))
    {
                                                                            ;
        return api_error_with_multiple_info(0xC0000A1100000000ULL, (uint8_t)i, PT_2MB, 0, 0);
    }
    if (!is_area_in_cmr(tdmr_info_copy[i].pamt_4k_base,
            tdmr_info_copy[i].pamt_4k_base + tdmr_info_copy[i].pamt_4k_size))
    {
                                                                            ;
        return api_error_with_multiple_info(0xC0000A1100000000ULL, (uint8_t)i, PT_4KB, 0, 0);
    }
    return 0x0000000000000000ULL;
}
static api_error_type check_tdmr_pamt_areas(tdmr_info_entry_t tdmr_info_copy[64],
        uint32_t i, pamt_data_t pamt_data_array[])
{
    api_error_type err;
    if (((err = check_pamt_addresses(tdmr_info_copy, i)) != 0x0000000000000000ULL))
    {
                                                                     ;
        return err;
    }
    if (((err = check_all_pamt_overlap(tdmr_info_copy, i,pamt_data_array)) != 0x0000000000000000ULL))
    {
                                                             ;
        return err;
    }
    if (((err = check_pamt_overlaps_available_areas(tdmr_info_copy, i)) != 0x0000000000000000ULL))
    {
                                                                                ;
        return err;
    }
    if (((err = check_pamt_in_cmr(tdmr_info_copy, i)) != 0x0000000000000000ULL))
    {
                                                                                          ;
        return err;
    }
...
static api_error_type check_tdmr_reserved_areas(tdmr_info_entry_t tdmr_info_copy[64], uint32_t i)
{
    for (uint32_t j = 0; j < 16U; j++)
    {
        ...
        if (area_size == 0)
        {
            if (j < 16U -1 && tdmr_info_copy[i].rsvd_areas[j+1].size != 0)
            {
                return api_error_with_multiple_info(0xC0000A2100000000ULL,
        ...
        else
        {
            ...
            {
                                                                                   ;
                return api_error_with_multiple_info(0xC0000A2000000000ULL,
                        (uint8_t)i, (uint8_t)j, 0, 0);
            }
            if (j > 0)
            {
            ...
            if (!is_addr_aligned_pwr_of_2(area_offset, (4 * 0x400ULL)) ||
                !is_addr_aligned_pwr_of_2(area_size, (4 * 0x400ULL)))
            {
                                                     ;
                return api_error_with_multiple_info(0xC0000A2000000000ULL,
                        (uint8_t)i, (uint8_t)j, 0, 0);
...
static api_error_type check_tdmr_available_areas(tdmr_info_entry_t tdmr_info_copy[64], uint32_t i)
{
    ...
    for (uint32_t j = 0; j < 16U; j++)
    {
        ...
        {
                                                      ;
            return api_error_with_multiple_info(0xC0000A0200000000ULL, (uint8_t)i, 0, 0, 0);
        }
        if (!valid_rsvd_area)
        {
            break;
...
static void set_tdmr_info_in_global_data(tdmr_info_entry_t tdmr_info_copy[64], uint32_t i)
{
    tdx_module_global_t* global_data_ptr = get_global_data();
    ...
    global_data_ptr->tdmr_table[i].num_of_pamt_blocks = (uint32_t)(tdmr_info_copy[i].tdmr_size / ((0x400ULL * 0x400ULL) * 0x400ULL));
...
static api_error_type check_and_set_tdmrs(tdmr_info_entry_t tdmr_info_copy[64],
        uint64_t i, pamt_data_t pamt_data_array[])
{
    ...
    {
                                                                                   ;
        return api_error_with_multiple_info(0xC0000A0100000000ULL, (uint8_t)i, 0, 0, 0);
    }
    ...
    api_error_type err;
    if ((err = check_tdmr_area_addresses_and_size(tdmr_info_copy, (uint32_t)i)) != 0x0000000000000000ULL)
    {
        return err;
    }
    if ((err = check_tdmr_reserved_areas(tdmr_info_copy, (uint32_t)i)) != 0x0000000000000000ULL)
    {
        return err;
    }
    if ((err = check_tdmr_pamt_areas(tdmr_info_copy, (uint32_t)i, pamt_data_array)) != 0x0000000000000000ULL)
    {
        return err;
    }
    if ((err = check_tdmr_available_areas(tdmr_info_copy, (uint32_t)i)) != 0x0000000000000000ULL)
    {
        return err;
    }
...
api_error_type tdh_sys_config(uint64_t tdmr_info_array_pa,
                             ...
                             hkid_api_input_t global_private_hkid)
{
    tdmr_info_entry_t* tdmr_info_p;
    tdmr_info_entry_t* tdmr_info_copy;
    bool_t tdmr_info_p_init = ((bool_t)0);
    pa_t tdmr_info_pa = {.raw = tdmr_info_array_pa};
    uint64_t* tdmr_pa_array = ((void*)0);
    ...
    retval = shared_hpa_check_with_pwr_2_alignment(tdmr_info_pa, 512);
    if (retval != 0x0000000000000000ULL)
    {
        retval = api_error_with_operand_id(retval, 1ULL);
                                                                                                                     ;
        goto EXIT;
    }
    if (num_of_tdmr_entries > 64)
    {
                                                                                                          ;
        retval = api_error_with_operand_id(0xC000010000000000ULL, 2ULL);
        goto EXIT;
    }
    ...
    {
                                                     ;
        retval = api_error_with_operand_id(0xC000010000000000ULL, 8ULL);
        goto EXIT;
    }
    tdx_global_data_ptr->kot.entries[hkid].state = 3;
    tdx_global_data_ptr->hkid = hkid;
    tdmr_pa_array = map_pa(tdmr_info_pa.raw_void, TDX_RANGE_RO);
    tdmr_info_p_init = ((bool_t)1);
    pa_t tdmr_entry;
    pamt_data_t pamt_data_array[64];
    api_error_type err;
    tdmr_info_copy = tdx_global_data_ptr->tdmr_info_copy;
    for(uint64_t i = 0; i < num_of_tdmr_entries; i++)
    {
        tdmr_entry.raw = tdmr_pa_array[i];
        retval = shared_hpa_check_with_pwr_2_alignment(tdmr_entry, 512);
        if (retval != 0x0000000000000000ULL)
        {
            retval = api_error_with_operand_id(retval, 1ULL);
                                                                                                                  ;
        ...
        free_la(tdmr_info_p);
        if ((err = check_and_set_tdmrs(tdmr_info_copy, i, pamt_data_array)) != 0x0000000000000000ULL)
        {
                                                     ;
            retval = err;
            goto EXIT;
