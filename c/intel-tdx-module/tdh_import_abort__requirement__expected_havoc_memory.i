...
void tdh_import_abort__valid_entry() {
    tdx_module_local_t* local_data = get_local_data();
    pa_t tdr_pa = { .raw = local_data->vmm_regs.rcx };
    assume_abort_if_not(((tdr_pa.full_pa & global_data_fv.hkid_mask) >> global_data_fv.hkid_start_bit) == 0);
    hpa_and_size_t mbmd_hpa_and_size = { .raw = local_data->vmm_regs.r8 };
    assume_abort_if_not(mbmd_hpa_and_size.size >= sizeof(mbmd_t));
    assume_abort_if_not(local_data->vmm_regs.r10 == 0);
    ...
    assume_abort_if_not(tdr_fv.management_fields.num_tdcx >= MIN_NUM_TDCS_PAGES);
    assume_abort_if_not(tdcs_fv.executions_ctl_fields.attributes.debug == ((bool_t)1));
    assume_abort_if_not((tdcs_fv.management_fields.op_state == OP_STATE_MEMORY_IMPORT) || (tdcs_fv.management_fields.op_state == OP_STATE_STATE_IMPORT) || (tdcs_fv.management_fields.op_state == OP_STATE_POST_IMPORT) || (tdcs_fv.management_fields.op_state == OP_STATE_FAILED_IMPORT));
