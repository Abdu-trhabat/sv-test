...
CATEGORIES_TO_IGNORE = {
    ...
    "Unused_DeviceDriversLinux64Regression": "(no corresponding .c files)",
}
        ...
        if not os.path.exists(CBMC_GIT_PATH + "src/goto-cc/goto-cc"):
            subprocess.check_call(
                [
                    ...
                    "clone",
                    "--depth=1",
                    "http://github.com/diffblue/cbmc.git",
                    CBMC_GIT_PATH,
                ]
            )
            subprocess.check_call(
                ["make", "-j2", "minisat2-download"], cwd=CBMC_GIT_PATH + "src"
            )
            subprocess.check_call(
                ["make", "-j2", "CXX=g++", "goto-diff.dir", "goto-cc.dir"],
                cwd=CBMC_GIT_PATH + "src",
            )
        cwd = os.getcwd()
    ...
    with (
        ...
        try:
            ...
            stdout, stderr = subprocess.Popen(
                ["goto-diff", "--verbosity", "2", "-u", origout, taskout],
            ...
            if len(stdout) > 0:
                ...
                is_error = print_error_or_warning(
                    "Difference on " + taskfile + " detected", taskfile
                )
        except Exception as e:
            is_error = print_error_or_warning(
                "Exception during check: " + str(e), taskfile
            )
...
if args.directories:
    print(
        "Analyzing only files from the following directories:\n    "
