...
float gelu(float x)
{
 float cdf = 0.5f * erff(x / 1.41421356237309504880168872420969807856967187537694807317667973799f) + 0.5f;
 return x * cdf;
}
int main()
{
 ...
 float x2 = x1 + 1e-5f;
 if(!(!__builtin_isnan (x1) && !__builtin_isinf_sign (x1))) abort();
 float y1 = gelu(x1);
 float y2 = gelu(x2);
 float derivative = (y2 - y1) / 1e-5f;
 if(!(__builtin_isgreaterequal(derivative, 0.0f))) reach_error();
    return 0;
}
