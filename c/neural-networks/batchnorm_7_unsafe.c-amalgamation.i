...
void batchnorm(const float x[4][4], float y[4][4],
...
int main()
{
 float x[4][4], y[4][4], mean[4], std[4];
 float gamma[4] = {0.123456f, 0.654321f, 0.333333f, 0.987654f};
 float beta[4] = {-0.42f, 0.33f, -0.12f, 0.01f};
 ...
 x[3][2] = x[0][2];
 x[3][3] = x[0][3];
 if(!(__builtin_isgreater(x[0][0], -1e18f) && __builtin_isless(x[0][0], 1e18f))) abort();;
 if(!(__builtin_isgreater(x[0][1], -1e18f) && __builtin_isless(x[0][1], 1e18f))) abort();;
 if(!(__builtin_isgreater(x[0][2], -1e18f) && __builtin_isless(x[0][2], 1e18f))) abort();;
 if(!(__builtin_isgreater(x[0][3], -1e18f) && __builtin_isless(x[0][3], 1e18f))) abort();;
 batchnorm(x, y, mean, std, gamma, beta, 4, 4);
 if(!(__builtin_islessequal(y[0][0], y[3][0]))) reach_error();
    return 0;
}
