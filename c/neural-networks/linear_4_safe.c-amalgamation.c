...
int main() /* check_permutation_size_2 */
{
	float x[INPUT_SIZE], y[OUTPUT_SIZE];
	
	float W[OUTPUT_SIZE][INPUT_SIZE] = {{0.0f, 1.0f},
										{1.0f, 0.0f}};
	...
	__VERIFIER_assert(isgreaterequal(x[0], y[1])); /* Expected result: verification successful */
