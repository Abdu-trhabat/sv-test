...
int main()
{
 ...
 if(!(!__builtin_isnan (x[0]) && !__builtin_isinf_sign (x[0]))) abort();;
 if(!(!__builtin_isnan (x[1]) && !__builtin_isinf_sign (x[1]))) abort();;
 if(!(!__builtin_isnan (x[2]) && !__builtin_isinf_sign (x[2]))) abort();;
 if(!(!__builtin_isnan (x[3]) && !__builtin_isinf_sign (x[3]))) abort();;
 if(!(!__builtin_isnan (x[4]) && !__builtin_isinf_sign (x[4]))) abort();;
 if(!(!__builtin_isnan (x[5]) && !__builtin_isinf_sign (x[5]))) abort();;
 if(!(!__builtin_isnan (x[6]) && !__builtin_isinf_sign (x[6]))) abort();;
 if(!(!__builtin_isnan (x[7]) && !__builtin_isinf_sign (x[7]))) abort();;
 if(!(!__builtin_isnan (x[8]) && !__builtin_isinf_sign (x[8]))) abort();;
 if(!(!__builtin_isnan (x[9]) && !__builtin_isinf_sign (x[9]))) abort();;
 conv1d(x, y, kernel, 10, 9, 1);
 if(!(y[0] == x[4])) reach_error();
    return 0;
}
