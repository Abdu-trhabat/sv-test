...
int main()
{
 ...
 float x2 = x1 + 1e-3f;
 if(!(__builtin_isgreaterequal(x1, -1e38f) && __builtin_islessequal(x1, 1e38f))) abort();;
 float y1 = gauss(x1);
 float y2 = gauss(x2);
 float derivative = (y2 - y1) / 1e-3f;
 float d = -0.5 * (x1 + x2) * gauss(0.5 * (x1 + x2));
 float diff = fabsf(derivative - d);
 if(!(__builtin_islessequal(diff, 1e-5f))) reach_error();
    return 0;
}
