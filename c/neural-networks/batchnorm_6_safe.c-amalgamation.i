...
int main()
{
 ...
 if(!(__builtin_isgreater(x[1][0], -1e18f) && __builtin_isless(x[1][0], 1e18f))) abort();;
 if(!(__builtin_isgreater(x[2][0], -1e18f) && __builtin_isless(x[2][0], 1e18f))) abort();;
 if(!(__builtin_isgreater(x[3][0], -1e18f) && __builtin_isless(x[3][0], 1e18f))) abort();;
 batchnorm(x, y, mean, std, gamma, beta, 4, 4);
 if(!(y[0][0] + y[1][0] + y[2][0] + y[3][0] == y[0][1] + y[1][1] + y[2][1] + y[3][1])) reach_error();
    return 0;
}
