...
int main()
{
 ...
 if(!(__builtin_isgreaterequal(x1, -32.0f) && __builtin_islessequal(x1, 32.0f))) abort();
 float x2 = x1 + 1e-2f;
 if(!(__builtin_isgreaterequal(x1, 0.0f))) abort();
 float y1 = sinf(x1);
 float y2 = sinf(x2);
 float derivative = (y2 - y1) / 1e-2f;
 float dref = cosf(x1 / 2.0f + x2 / 2.0f);
 float error = fabsf(derivative - dref);
 if(!(__builtin_islessequal(error, 1e-6f))) reach_error();
    return 0;
}
