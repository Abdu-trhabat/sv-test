...
int main() /* check_bound_size_2_4 */
{
	...
	__VERIFIER_assume(isgreaterequal(x[0][2], 0.0f) && islessequal(x[0][2], BATCHNORM_CHECK_RANGE));
	__VERIFIER_assume(isgreaterequal(x[0][3], 0.0f) && islessequal(x[0][3], BATCHNORM_CHECK_RANGE));
