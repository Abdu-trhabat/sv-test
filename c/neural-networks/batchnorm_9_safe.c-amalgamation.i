...
int main()
{
 ...
 x[1][2] = x[0][2];
 x[1][3] = x[0][3];
 if(!(!__builtin_isnan (x[0][0]) && !__builtin_isinf_sign (x[0][0]))) abort();;
 if(!(!__builtin_isnan (x[0][2]) && !__builtin_isinf_sign (x[0][2]))) abort();;
 if(!(!__builtin_isnan (x[0][3]) && !__builtin_isinf_sign (x[0][3]))) abort();;
 ...
 if(!(__builtin_islessequal(y[0][0], y[0][1]))) reach_error();
    return 0;
}
