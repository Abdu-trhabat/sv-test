...
void linear(const float* x, float* y, const float W[2][8], const float* b, int m, int n)
{
 int i, j;
 for(i = 0; i < n; ++i) {
  y[i] = b[i];
...
{
 float x[8], y[2];
 float W[2][8] = {{ 2.3f, 2.3f, 2.3f, 2.3f, -2.3f, -2.3f, -2.3f, -2.3f},
 ...
 if(!(__builtin_isgreater(x[6], -1e37f) && __builtin_isless(x[6], 1e37f))) abort();;
 if(!(__builtin_isgreater(x[7], -1e37f) && __builtin_isless(x[7], 1e37f))) abort();;
 linear(x, y, W, b, 8, 2);
 if(!(y[0] == -y[1])) reach_error();
    return 0;
}
