...
void batchnorm(const float x[2][4], float y[2][4],
               float* mean, float* std, const float* gamma, const float* beta, int n, int batch_size)
{
 ...
 for(i = 0; i < n; ++i) {
  ...
  std[i] = sqrtf(std[i] / ((float) batch_size - 1.0f)) + 1e-8;
 }
 for(i = 0; i < n; ++i)
  for(j = 0; j < batch_size; ++j)
...
int main()
{
 float x[2][4], y[2][4], mean[2], std[2];
 float gamma[2] = {0.123456f, 0.654321f};
 float beta[2] = {-0.42f, -0.333333f};
 ...
 x[1][2] = x[0][2];
 x[1][3] = x[0][3];
 if(!(__builtin_isgreaterequal(x[0][0], 0.0f) && __builtin_islessequal(x[0][0], 1e18f))) abort();;
 if(!(__builtin_isgreaterequal(x[0][1], 0.0f) && __builtin_islessequal(x[0][1], 1e18f))) abort();;
 if(!(__builtin_isgreaterequal(x[0][2], 0.0f) && __builtin_islessequal(x[0][2], 1e18f))) abort();;
 if(!(__builtin_isgreaterequal(x[0][3], 0.0f) && __builtin_islessequal(x[0][3], 1e18f))) abort();;
 batchnorm(x, y, mean, std, gamma, beta, 2, 4);
 if(!(__builtin_islessequal(y[0][0], y[1][0]))) reach_error();
    return 0;
}
