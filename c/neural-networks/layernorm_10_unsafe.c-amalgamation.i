...
int main()
{
 ...
 if(!(__builtin_isgreater(x[0], -1e18f) && __builtin_isless(x[0], 1e18f))) abort();;
 if(!(__builtin_isgreater(x[1], -1e18f) && __builtin_isless(x[1], 1e18f))) abort();;
 if(!(__builtin_isgreater(x[2], -1e18f) && __builtin_isless(x[2], 1e18f))) abort();;
 if(!(__builtin_isgreater(x[3], -1e18f) && __builtin_isless(x[3], 1e18f))) abort();;
 if(!(__builtin_isgreater(x[4], -1e18f) && __builtin_isless(x[4], 1e18f))) abort();;
 if(!(__builtin_isgreater(x[5], -1e18f) && __builtin_isless(x[5], 1e18f))) abort();;
 layernorm(x, y, 1.0f, 0.0f, 6);
 int i;
 float sum = y[0];
 for(i = 1; i < 6; ++i)
  sum += y[i];
 if(!(fabsf(sum) <= 1e-1)) reach_error();
    return 0;
}
