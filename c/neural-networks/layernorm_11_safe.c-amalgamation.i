...
int main()
{
 ...
 if(!(__builtin_isgreater(x[6], -1e18f) && __builtin_isless(x[6], 1e18f))) abort();;
 if(!(__builtin_isgreater(x[7], -1e18f) && __builtin_isless(x[7], 1e18f))) abort();;
 layernorm(x, y, 1.0f, 0.0f, 8);
 int i;
 float sum = y[0];
 for(i = 1; i < 8; ++i)
  sum += y[i];
 if(!(fabsf(sum) <= 1e2)) reach_error();
    return 0;
}
