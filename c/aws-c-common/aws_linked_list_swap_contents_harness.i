...
void reach_error() { __assert_fail("0", "aws_linked_list_swap_contents_harness.i", 208, "reach_error"); }
extern void abort(void);
...
void aws_linked_list_swap_contents_harness() {
    ...
    ensure_linked_list_is_allocated(&a, 20);
    ensure_linked_list_is_allocated(&b, 20);
    ...
    if (aws_linked_list_empty(&a)) {
        __VERIFIER_assert(old_b.tail.prev == old_b_head);
    } else {
        __VERIFIER_assert(a.head.next == old_b.head.next);
        __VERIFIER_assert(a.tail.prev == old_b.tail.prev);
    }

    if (aws_linked_list_empty(&b)) {
        __VERIFIER_assert(old_a.tail.prev == old_a_head);
    } else {
        __VERIFIER_assert(b.head.next == old_a.head.next);
        __VERIFIER_assert(b.tail.prev == old_a.tail.prev);
    }
}
int main() { aws_linked_list_swap_contents_harness(); return 0; }
