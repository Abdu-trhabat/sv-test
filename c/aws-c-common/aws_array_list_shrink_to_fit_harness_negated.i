...
void reach_error() { __assert_fail("0", "aws_array_list_shrink_to_fit_harness.i", 208, "reach_error"); }
extern void abort(void);
...
void *memmove_impl(void *dest, const void *src, size_t n) {
    if (n > 0) {
        (void)*(char *)dest;
        (void)*(const char *)src;
        (void)*(((unsigned char *)dest) + n - 1);
        (void)*(((const unsigned char *)src) + n - 1);
    }
    return dest;
}
...
void aws_array_list_shrink_to_fit_harness() {
    ...
    if (!aws_array_list_shrink_to_fit(&list)) {
        __VERIFIER_assert(!((list.current_size == 0 && list.data == ((void *)0)) || (list.data != ((void *)0) && list.current_size == list.length * list.item_size)))
...
int main() { aws_array_list_shrink_to_fit_harness(); return 0; }
