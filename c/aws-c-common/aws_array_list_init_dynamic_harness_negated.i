...
void reach_error() { __assert_fail("0", "aws_array_list_init_dynamic_harness.i", 208, "reach_error"); }
extern void abort(void);
...
void aws_array_list_init_dynamic_harness() {
    ...
    if (aws_array_list_init_dynamic(&list, allocator, initial_item_allocation, item_size) == (0)) {
        ...
        __VERIFIER_assert(!(aws_array_list_is_valid(&list)));
        __VERIFIER_assert(!(list.alloc == allocator));
        __VERIFIER_assert(!(list.item_size == item_size));
        __VERIFIER_assert(!(list.length == 0));
        __VERIFIER_assert(!(list.current_size == item_size * initial_item_allocation));
    } else {

        __VERIFIER_assert(!(aws_is_mem_zeroed(&(list), sizeof(list))));
    }
}
int main() { aws_array_list_init_dynamic_harness(); return 0; }
