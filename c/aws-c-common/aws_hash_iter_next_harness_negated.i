...
void reach_error() { __assert_fail("0", "aws_hash_iter_next_harness.i", 208, "reach_error"); }
extern void abort(void);
...
void aws_hash_iter_next_harness() {
    ...
    __VERIFIER_assert(!(aws_hash_iter_is_valid(&iter)));
    __VERIFIER_assert(!(iter.status == AWS_HASH_ITER_STATUS_DONE || iter.status == AWS_HASH_ITER_STATUS_READY_FOR_USE));
    __VERIFIER_assert(!((!(old_status == AWS_HASH_ITER_STATUS_DONE) || (iter.status == AWS_HASH_ITER_STATUS_DONE))));
    __VERIFIER_assert(!(aws_hash_table_is_valid(&map)));
    check_hash_table_unchanged(&map, &old_byte);
}
int main() { aws_hash_iter_next_harness(); return 0; }
