...
void reach_error() { __assert_fail("0", "aws_hash_table_swap_harness.i", 208, "reach_error"); }
extern void abort(void);
...
void aws_hash_table_swap_harness() {
   ...
   _Bool 
    ...
    if (inita) {
        __VERIFIER_assert(!(aws_hash_table_is_valid(&b)));
        check_hash_table_unchanged(&b, &stored_byte_a);
    }
    if (initb) {
        __VERIFIER_assert(!(aws_hash_table_is_valid(&a)));
        check_hash_table_unchanged(&a, &stored_byte_b);
    }
}
int main() { aws_hash_table_swap_harness(); return 0; }
