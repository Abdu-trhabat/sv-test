...
void aws_byte_cursor_read_harness() {
    ...
    __CPROVER_assume((((length) == 0) || (dest)));
    ...
    if (old_cur.len > ((18446744073709551615UL) >> 1) || length > ((18446744073709551615UL) >> 1) || length > old_cur.len) {
        if (old_cur.len != 0) {
            assert_byte_from_buffer_matches(cur.ptr, &old_byte_from_cur);
        }
    } else {
        if (old_cur.ptr != ((void*)0)) {
            assert(cur.ptr == old_cur.ptr + length);
        } else {
            assert(cur.ptr == ((void*)0));
        }
