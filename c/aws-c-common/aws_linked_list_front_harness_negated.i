...
void reach_error() { __assert_fail("0", "aws_linked_list_front_harness.i", 208, "reach_error"); }
extern void abort(void);
...
void aws_linked_list_front_harness() {
    ...
    ensure_linked_list_is_allocated(&list, 20);
    ...
    __VERIFIER_assert(!(aws_linked_list_is_valid(&list)));
    __VERIFIER_assert(!(aws_linked_list_node_prev_is_valid(front)));
    __VERIFIER_assert(!(aws_linked_list_node_next_is_valid(front)));
    __VERIFIER_assert(!(front == old_first));
}
int main() { aws_linked_list_front_harness(); return 0; }
