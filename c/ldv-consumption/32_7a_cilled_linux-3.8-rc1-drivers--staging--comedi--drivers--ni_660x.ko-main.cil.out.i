...
void reach_error() { ((void) sizeof ((0) ? 1 : 0), __extension__ ({ if (0) ; else __assert_fail ("0", "32_7a_cilled_linux-3.8-rc1-drivers--staging--comedi--drivers--ni_660x.ko-main.cil.out.c", 3, __extension__ __PRETTY_FUNCTION__); })); }
...
struct mite_struct *mite_alloc(struct pci_dev *arg0) {
  return ldv_malloc(sizeof(struct mite_struct));
}
struct mite_dma_descriptor_ring *mite_alloc_ring(struct mite_struct *arg0) {
  return ldv_malloc(sizeof(struct mite_dma_descriptor_ring));
}
int __VERIFIER_nondet_int(void);
