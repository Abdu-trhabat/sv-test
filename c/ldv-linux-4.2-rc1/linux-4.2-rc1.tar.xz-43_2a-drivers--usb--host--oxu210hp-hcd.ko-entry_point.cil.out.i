...
void reach_error() { ((void) sizeof ((0) ? 1 : 0), __extension__ ({ if (0) ; else __assert_fail ("0", "linux-4.2-rc1.tar.xz-43_2a-drivers--usb--host--oxu210hp-hcd.ko-entry_point.cil.out.c", 3, __extension__ __PRETTY_FUNCTION__); })); }
...
int ldv_del_timer_25(struct timer_list *ldv_func_arg1 ) ;
...
int ldv___platform_driver_register_30(struct platform_driver *ldv_func_arg1 , struct module *ldv_func_arg2 ) ;
extern void platform_driver_unregister(struct platform_driver * ) ;
void ldv_platform_driver_unregister_31(struct platform_driver *ldv_func_arg1 ) ;
...
__inline static void timer_action(struct oxu_hcd *oxu , enum ehci_timer_action action )
{
  ...
  if (tmp___0 == 0) {
    ...
    ldv_mod_timer_24(& oxu->watchdog, t);
  } else {
  }
  return;
}
...
static int ehci_reset(struct oxu_hcd *oxu )
{
  ...
  descriptor.filename = "/home/ldvuser/mutilin/launch/work/current--X--drivers/--X--defaultlinux-4.2-rc1.tar.xz--X--43_2a--X--cpachecker/linux-4.2-rc1.tar.xz/csd_deg_dscv/9566/dscv_tempdir/dscv/ri/43_2a/drivers/usb/host/oxu210hp-hcd.c";
  descriptor.format = "%s\n";
                                ...
                                int port_status )
{
  ...
  if ((port_status & 4) == 0) {
    ...
    descriptor___0.filename = "/home/ldvuser/mutilin/launch/work/current--X--drivers/--X--defaultlinux-4.2-rc1.tar.xz--X--43_2a--X--cpachecker/linux-4.2-rc1.tar.xz/csd_deg_dscv/9566/dscv_tempdir/dscv/ri/43_2a/drivers/usb/host/oxu210hp-hcd.c";
...
static void ehci_mem_cleanup(struct oxu_hcd *oxu )
{
  ...
  ldv_del_timer_25(& oxu->urb_timer);
                        ...
                        gfp_t mem_flags )
{
  ...
  tmp___1 = ldv__builtin_expect((unsigned long )qh == (unsigned long )((struct ehci_qh *)0),
                             0L);
  if (tmp___1 != 0L) {
    rc = -12;
    goto done;
  ...
  tmp___3 = ldv__builtin_expect((unsigned int )qh->qh_state == 3U, 1L);
  if (tmp___3 != 0L) {
    tmp___2 = qh_get(qh);
    qh_link_async(oxu, tmp___2);
  } else {
  }
  done:
                       ...
                       gfp_t mem_flags )
{
  ...
  tmp___2 = oxu_to_hcd(oxu);
  tmp___2->self.bandwidth_int_reqs = tmp___2->self.bandwidth_int_reqs + 1;
  done:
  spin_unlock_irqrestore(& oxu->lock, flags);
  if (status != 0) {
    qtd_list_free(oxu, urb, qtd_list);
...
static irqreturn_t oxu210_hcd_irq(struct usb_hcd *hcd )
{
  ...
  if ((status & 4U) != 0U) {
    ...
    ldv_mod_timer_26(& hcd->rh_timer, oxu->reset_done[i]);
    ldv_35102:
    tmp___9 = i;
    i = i - 1U;
    if (tmp___9 != 0U) {
  ...
  if (tmp___16 != 0L) {
    ...
    descriptor___1.filename = "/home/ldvuser/mutilin/launch/work/current--X--drivers/--X--defaultlinux-4.2-rc1.tar.xz--X--43_2a--X--cpachecker/linux-4.2-rc1.tar.xz/csd_deg_dscv/9566/dscv_tempdir/dscv/ri/43_2a/drivers/usb/host/oxu210hp-hcd.c";
    ...
    descriptor___2.filename = "/home/ldvuser/mutilin/launch/work/current--X--drivers/--X--defaultlinux-4.2-rc1.tar.xz--X--43_2a--X--cpachecker/linux-4.2-rc1.tar.xz/csd_deg_dscv/9566/dscv_tempdir/dscv/ri/43_2a/drivers/usb/host/oxu210hp-hcd.c";
...
static void oxu_watchdog(unsigned long param )
{
  ...
  tmp___0 = constant_test_bit(3L, (unsigned long const volatile *)(& oxu->actions));
  if (tmp___0 != 0) {
...
static void oxu_stop(struct usb_hcd *hcd )
{
  ...
  ldv_del_timer_sync_27(& oxu->watchdog);
...
static void oxu_endpoint_disable(struct usb_hcd *hcd , struct usb_host_endpoint *ep )
{
  ...
  tmp___2 = list_empty((struct list_head const *)(& qh->qtd_list));
  tmp___3 = oxu_to_hcd(oxu);
  dev_err((struct device const *)tmp___3->self.controller, "qh %p (#%02x) state %d%s\n",
          qh, (int )ep->desc.bEndpointAddress, (int )qh->qh_state, tmp___2 != 0 ? (char *)"" : (char *)"(has tds)");
  goto ldv_35239;
  }
  ldv_35239:
  ep->hcpriv = (void *)0;
                           ...
                           char *buf , u16 wLength )
{
  ...
  if ((temp & 128U) != 0U) {
    ...
    oxu->reset_done[(int )wIndex] = tmp___0 + (unsigned long )jiffies;
  } else {
  }
  goto ldv_35299;
  case 18: ;
  ...
  if ((temp & 64U) != 0U) {
    if (oxu->reset_done[(int )wIndex] == 0UL) {
      ...
      tmp___2 = oxu_to_hcd(oxu);
      ldv_mod_timer_28(& tmp___2->rh_timer, oxu->reset_done[(int )wIndex]);
    } else
  ...
  if ((temp & 256U) != 0U && (long )((unsigned long )jiffies - oxu->reset_done[(int )wIndex]) >= 0L) {
    ...
    tmp___5 = readl((void const volatile *)status_reg);
    tmp___6 = check_reset_complete(oxu, (int )wIndex, status_reg, (int )tmp___5);
    temp = (u32 )tmp___6;
  } else {
  }
  if ((int )temp & 1) {
    tmp___9 = variable_test_bit((long )wIndex, (unsigned long const volatile *)(& oxu->companion_ports));
    if (tmp___9 != 0) {
  ...
  if ((int )temp & 1) {
    status = status | 1U;
    tmp___10 = oxu_port_speed(oxu, temp);
  ...
  if ((status & 4294901760U) != 0U) {
    ...
    if (tmp___12 != 0L) {
      tmp___11 = oxu_to_hcd(oxu);
      __dynamic_dev_dbg(& descriptor___0, (struct device const *)tmp___11->self.controller,
                        "%s\n", (char *)(& _buf));
    } else {
    }
  } else {
  ...
  *((u8 *)__gu_p) = (unsigned char )status;
  ...
  tmp___13 = device_may_wakeup(& (hcd->self.root_hub)->dev);
  if ((int )tmp___13) {
    temp = temp | 7340032U;
  } else {
  }
  ...
  tmp___14 = msecs_to_jiffies(50U);
  oxu->reset_done[(int )wIndex] = tmp___14 + (unsigned long )jiffies;
  writel(temp, (void volatile *)status_reg);
  goto ldv_35341;
  case 21: ;
...
static int oxu_bus_suspend(struct usb_hcd *hcd )
{
  ...
  ldv_del_timer_sync_29(& oxu->watchdog);
  ehci_halt(oxu);
  hcd->state = 4;
  mask = 55;
...
static int oxu_driver_init(void)
{
  int tmp ;
  {
  tmp = ldv___platform_driver_register_30(& oxu_driver, & __this_module);
  return (tmp);
}
}
...
{
  {
  ldv_platform_driver_unregister_31(& oxu_driver);
  return;
}
}
int ldv_retval_2 ;
