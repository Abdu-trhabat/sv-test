...
void reach_error() { ((void) sizeof ((0) ? 1 : 0), __extension__ ({ if (0) ; else __assert_fail ("0", "linux-4.2-rc1.tar.xz-43_2a-drivers--net--wan--lmc--lmc.ko-entry_point.cil.out.c", 3, __extension__ __PRETTY_FUNCTION__); })); }
...
struct __anonstruct_lmc_av9110_t_330 {
   u32 n ;
   u32 m ;
   u32 v ;
   u32 x ;
...
typedef struct __anonstruct_lmc_av9110_t_330 lmc_av9110_t;
union __anonunion_cardspec_331 {
   lmc_av9110_t ssi ;
};
struct lmc___ctl {
   u32 cardtype ;
   ...
   u32 keepalive_onoff ;
   u32 ticks ;
   union __anonunion_cardspec_331 cardspec ;
   u32 circuit_type ;
};
struct tulip_desc_t {
   s32 status ;
...
struct pci_dev {
   ...
   struct pci_vpd *vpd ;
   union __anonunion____missing_field_name_232___0 __annonCompField65 ;
   struct pci_ats *ats ;
   phys_addr_t rom ;
   size_t romlen ;
...
int ldv_del_timer_113(struct timer_list *ldv_func_arg1 ) ;
...
struct pci_dev *lmc_driver_group1 ;
int ldv_irq_line_1_2 ;
int ldv_state_variable_4 ;
struct net_device *lmc_ops_group1 ;
...
int ldv___pci_register_driver_115(struct pci_driver *ldv_func_arg1 , struct module *ldv_func_arg2 ,
                                  char const *ldv_func_arg3 ) ;
extern void pci_unregister_driver(struct pci_driver * ) ;
void ldv_pci_unregister_driver_116(struct pci_driver *ldv_func_arg1 ) ;
...
void ldv_free_netdev_110(struct net_device *dev ) ;
void ldv_free_netdev_111(struct net_device *dev ) ;
...
int ldv_register_netdev_109(struct net_device *dev ) ;
...
struct pci_device_id const __mod_pci__lmc_pci_tbl_device_table[3U] ;
...
static int lmc_init_one(struct pci_dev *pdev , struct pci_device_id const *ent )
{
  ...
  __raw_spin_lock_init(& sc->lmc_lock.__annonCompField18.rlock, "&(&sc->lmc_lock)->rlock",
                       & __key);
  pci_set_master(pdev);
...
static int lmc_open(struct net_device *dev )
{
  ...
  lmc_softreset(sc);
  tmp___0 = ldv_request_irq_112((unsigned int )dev->irq, & lmc_interrupt, 128UL, (char const *)(& dev->name),
                                (void *)dev);
  if (tmp___0 != 0) {
...
static int lmc_ifdown(struct net_device * const dev )
{
  ...
  if (sc->got_irq == 1) {
    ldv_free_irq_114((unsigned int )dev->irq, (void *)dev);
    sc->got_irq = 0;
  } else {
  }
  i = 0;
...
static int lmc_driver_init(void)
{
  int tmp ;
  {
  tmp = ldv___pci_register_driver_115(& lmc_driver, & __this_module, "lmc");
  return (tmp);
}
}
...
{
  {
  ldv_pci_unregister_driver_116(& lmc_driver);
  return;
}
}
unsigned int lmc_mii_readreg(lmc_softc_t * const sc , unsigned int devaddr , unsigned int regno )
...
{
  {
  if ((unsigned long )handler == (unsigned long )(& lmc_interrupt)) {
    return (1);
  } else {
  }
  return (0);
...
int ldv_irq_1(int state , int line , void *data )
{
  ...
  if (state != 0) {
    ...
    if (state == 1) {
      LDV_IN_INTERRUPT = 2;
      irq_retval = lmc_interrupt(line, data);
      LDV_IN_INTERRUPT = 1;
      return (state);
    } else {
    }
...
void ldv_pci_driver_3(void)
{
  ...
  lmc_driver_group1 = (struct pci_dev *)tmp;
  return;
}
}
void ldv_net_device_ops_4(void)
{
  ...
  lmc_ops_group1 = (struct net_device *)tmp;
  return;
}
}
void choose_interrupt_1(void)
...
int main(void)
{
  ...
  if (ldv_state_variable_4 != 0) {
    ...
    if (ldv_state_variable_4 == 1) {
      lmc_ioctl(lmc_ops_group1, ldvarg3, ldvarg2);
    ...
    case 2: ;
    if (ldv_state_variable_4 == 3) {
      hdlc_change_mtu(lmc_ops_group1, ldvarg1);
      ldv_state_variable_4 = 3;
    } else {
    }
    if (ldv_state_variable_4 == 2) {
    ...
    case 3: ;
    if (ldv_state_variable_4 == 2) {
      ldv_retval_1 = lmc_open(lmc_ops_group1);
      if (ldv_retval_1 == 0) {
        ldv_state_variable_4 = 3;
      } else {
      }
    ...
    case 4: ;
    if (ldv_state_variable_4 == 3) {
      hdlc_start_xmit(ldvarg0, lmc_ops_group1);
      ldv_state_variable_4 = 3;
    } else {
    }
    goto ldv_46586;
    ...
    case 6: ;
    if (ldv_state_variable_4 == 1) {
      lmc_driver_timeout(lmc_ops_group1);
      ldv_state_variable_4 = 1;
    } else {
    }
    if (ldv_state_variable_4 == 3) {
  ...
  if (ldv_state_variable_3 != 0) {
    ...
    if (ldv_state_variable_3 == 1) {
      ldv_retval_3 = lmc_init_one(lmc_driver_group1, (struct pci_device_id const *)ldvarg4);
      if (ldv_retval_3 == 0) {
    ...
    if (ldv_state_variable_3 == 2) {
      lmc_remove_one(lmc_driver_group1);
