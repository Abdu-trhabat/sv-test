...
typedef enum ldv_25369 status_type_t;
union map_info {
   void *ptr ;
};
struct dm_dev {
...
void ldv_mutex_lock_mutex_of_dm_thin_pool_table(struct mutex *lock ) ;
void ldv_mutex_unlock_mutex_of_dm_thin_pool_table(struct mutex *lock ) ;
...
{ 
  ...
  sector = end_sect;
  submit_bio(type, bio);
  ___might_sleep("/work/ldvuser/mutilin/launch/work/current--X--drivers/--X--defaultlinux-4.2-rc1.tar.xz--X--32_7a--X--cpachecker/linux-4.2-rc1.tar.xz/csd_deg_dscv/965/dscv_tempdir/dscv/ri/32_7a/drivers/md/dm-thin.c",
                 424, 0);
  _cond_resched();
  ldv_35055: ;
  if (nr_sects != 0UL) {
...
{ 
  ...
  if (tmp___0 != 0L) {
    __asm__  volatile   ("1:\tud2\n.pushsection __bug_table,\"a\"\n2:\t.long 1b - 2b, %c0 - 2b\n\t.word %c1, 0\n\t.org 2b+%c2\n.popsection": : "i" ((char *)"/work/ldvuser/mutilin/launch/work/current--X--drivers/--X--defaultlinux-4.2-rc1.tar.xz--X--32_7a--X--cpachecker/linux-4.2-rc1.tar.xz/csd_deg_dscv/965/dscv_tempdir/dscv/ri/32_7a/drivers/md/dm-thin.c"),
                         "i" (554), "i" (12UL));
    ldv_35128: ;
    goto ldv_35128;
  } else {
...
{ 
  ...
  if (tmp___0 != 0L) {
    warn_slowpath_null("/work/ldvuser/mutilin/launch/work/current--X--drivers/--X--defaultlinux-4.2-rc1.tar.xz--X--32_7a--X--cpachecker/linux-4.2-rc1.tar.xz/csd_deg_dscv/965/dscv_tempdir/dscv/ri/32_7a/drivers/md/dm-thin.c",
                       1407);
  } else {
...
{ 
  ...
  if ((unsigned long )pool->wq != (unsigned long )((struct workqueue_struct *)0)) {
    ldv_destroy_workqueue_17(pool->wq);
  } else {
...
{ 
  ...
  __raw_spin_lock_init(& pool->lock.__annonCompField17.rlock, "&(&pool->lock)->rlock",
                       & __key___5);
  bio_list_init(& pool->deferred_flush_bios);
  ...
  dm_deferred_set_destroy(pool->shared_read_ds);
  bad_shared_read_ds: 
  ldv_destroy_workqueue_18(pool->wq);
  bad_wq: 
  dm_kcopyd_client_destroy(pool->copier);
  bad_kcopyd_client: 
  dm_bio_prison_destroy(pool->prison);
...
{ 
  ...
  {
  pt = (struct pool_c *)ti->private;
  ldv_mutex_lock_19(& dm_thin_pool_table.mutex);
  unbind_control_target(pt->pool, ti);
  __pool_dec(pt->pool);
  dm_put_device(ti, pt->metadata_dev);
  dm_put_device(ti, pt->data_dev);
  kfree((void const   *)pt);
  ldv_mutex_unlock_20(& dm_thin_pool_table.mutex);
  return;
}
}
...
{ 
  ...
  {
  pool_created = 0;
  ldv_mutex_lock_21(& dm_thin_pool_table.mutex);
  if (argc <= 3U) {
  ...
  ldv_mutex_unlock_22(& dm_thin_pool_table.mutex);
  return (0);
  out_flags_changed: 
  __pool_dec(pool);
  out_free_pt: 
  ...
  out_unlock: 
  ldv_mutex_unlock_23(& dm_thin_pool_table.mutex);
  return (r);
}
}
...
{ 
  ...
  pt = (struct pool_c *)ti->private;
  pool = pt->pool;
  ldv_cancel_delayed_work_24(& pool->waker);
  ldv_cancel_delayed_work_25(& pool->no_space_timeout);
  ldv_flush_workqueue_26(pool->wq);
  commit(pool);
  return;
}
}
...
{ 
  ...
  ldv_mutex_lock_27(& dm_thin_pool_table.mutex);
  __pool_dec(tc->pool);
  ...
  kfree((void const   *)tc);
  ldv_mutex_unlock_28(& dm_thin_pool_table.mutex);
  return;
}
}
...
{ 
  ...
  {
  ldv_mutex_lock_29(& dm_thin_pool_table.mutex);
  if (argc != 2U && argc != 3U) {
    ti->error = (char *)"Invalid argument count";
    r = -22;
    goto out_unlock;
  ...
  __raw_spin_lock_init(& tc->lock.__annonCompField17.rlock, "&(&tc->lock)->rlock",
                       & __key);
  ...
  ldv_mutex_unlock_30(& dm_thin_pool_table.mutex);
  tmp___3 = spinlock_check(& (tc->pool)->lock);
  flags = _raw_spin_lock_irqsave(tmp___3);
  if ((int )(tc->pool)->suspended) {
    spin_unlock_irqrestore(& (tc->pool)->lock, flags);
    ldv_mutex_lock_31(& dm_thin_pool_table.mutex);
  ...
  kfree((void const   *)tc);
  out_unlock: 
  ldv_mutex_unlock_32(& dm_thin_pool_table.mutex);
  return (r);
}
}
...
{ 
  ...
  {
  ldv_mutex_lock_mutex_of_dm_thin_pool_table(ldv_func_arg1);
  mutex_lock(ldv_func_arg1);
  return;
}
}
...
{ 
  ...
  {
  ldv_mutex_unlock_mutex_of_dm_thin_pool_table(ldv_func_arg1);
  mutex_unlock(ldv_func_arg1);
  return;
}
}
...
{ 
  ...
  if (tmp != 0L) {
    __asm__  volatile   ("1:\tud2\n.pushsection __bug_table,\"a\"\n2:\t.long 1b - 2b, %c0 - 2b\n\t.word %c1, 0\n\t.org 2b+%c2\n.popsection": : "i" ((char *)"/work/ldvuser/mutilin/launch/work/current--X--drivers/--X--defaultlinux-4.2-rc1.tar.xz--X--32_7a--X--cpachecker/linux-4.2-rc1.tar.xz/csd_deg_dscv/965/dscv_tempdir/dscv/ri/32_7a/drivers/md/dm-thin-metadata.c"),
                         "i" (1214), "i" (12UL));
    ldv_32857: ;
    goto ldv_32857;
  } else {
...
static int ldv_mutex_mutex_of_dm_thin_pool_table  =    1;
int ldv_mutex_lock_interruptible_mutex_of_dm_thin_pool_table(struct mutex *lock ) 
{ 
  int nondetermined ;

  {
  if (ldv_mutex_mutex_of_dm_thin_pool_table != 1) {
    ldv_error();
  } else {
  ...
  nondetermined = ldv_undef_int();
  if (nondetermined != 0) {
    ldv_mutex_mutex_of_dm_thin_pool_table = 2;
    return (0);
  } else {
    return (-4);
  }
...
int ldv_mutex_lock_killable_mutex_of_dm_thin_pool_table(struct mutex *lock ) 
{ 
  int nondetermined ;
...
int ldv_mutex_trylock_mutex_of_dm_thin_pool_table(struct mutex *lock ) 
{ 
  int is_mutex_held_by_another_thread ;
...
int ldv_atomic_dec_and_mutex_lock_mutex_of_dm_thin_pool_table(atomic_t *cnt , struct mutex *lock ) 
{ 
  int atomic_value_after_dec ;
...
int ldv_mutex_is_locked_mutex_of_dm_thin_pool_table(struct mutex *lock ) 
{ 
  int nondetermined ;
...
{ 
  ...
  {
  if (ldv_mutex_mutex_of_dm_thin_pool_table != 2) {
    ldv_error();
  } else {

  }
  ldv_mutex_mutex_of_dm_thin_pool_table = 1;
  return;
}
}
void ldv_usb_lock_device_mutex_of_dm_thin_pool_table(void) 
{ 


  {
  ldv_mutex_lock_mutex_of_dm_thin_pool_table((struct mutex *)0);
  return;
}
}
int ldv_usb_trylock_device_mutex_of_dm_thin_pool_table(void) 
{ 
  int tmp ;

  {
  tmp = ldv_mutex_trylock_mutex_of_dm_thin_pool_table((struct mutex *)0);
  return (tmp);
}
}
int ldv_usb_lock_device_for_reset_mutex_of_dm_thin_pool_table(void) 
{ 
  int tmp ;
  int tmp___0 ;
...
void ldv_usb_unlock_device_mutex_of_dm_thin_pool_table(void) 
{ 


  {
  ldv_mutex_unlock_mutex_of_dm_thin_pool_table((struct mutex *)0);
  return;
}
}
void ldv_check_final_state(void) 
...
#include "model/linux-4.2-rc1.tar.xz-32_7a-drivers--md--dm-thin-pool.ko-entry_point_true-unreach-call.cil.out.env.c"
