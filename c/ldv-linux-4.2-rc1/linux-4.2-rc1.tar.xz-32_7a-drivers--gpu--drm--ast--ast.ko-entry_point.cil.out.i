...
void reach_error() { ((void) sizeof ((0) ? 1 : 0), __extension__ ({ if (0) ; else __assert_fail ("0", "linux-4.2-rc1.tar.xz-32_7a-drivers--gpu--drm--ast--ast.ko-entry_point.cil.out.c", 3, __extension__ __PRETTY_FUNCTION__); })); }
...
__inline static int ldv_kref_put_mutex_54(struct kref *kref , void (*release)(struct kref * ) ,
                                          struct mutex *lock ) ;
...
__inline static void drm_gem_object_unreference_unlocked(struct drm_gem_object *obj )
{
  ...
  dev = obj->dev;
  tmp = ldv_kref_put_mutex_54(& obj->refcount, & drm_gem_object_free, & dev->struct_mutex);
  if (tmp != 0) {
...
int ldv_mutex_lock_interruptible_125(struct mutex *ldv_func_arg1 ) ;
...
__inline static int ww_mutex_lock_interruptible___0(struct ww_mutex *lock , struct ww_acquire_ctx *ctx )
{
  ...
  if ((unsigned long )ctx != (unsigned long )((struct ww_acquire_ctx *)0)) {
    ...
    tmp___0 = ldv_mutex_lock_interruptible_125(& lock->base);
    return (tmp___0);
  }
}
}
__inline static int ww_mutex_trylock___0(struct ww_mutex *lock )
{
  int tmp ;
  {
  tmp = ldv_mutex_trylock_126(& lock->base);
  return (tmp);
}
}
...
__inline static void drm_gem_object_unreference_unlocked___1(struct drm_gem_object *obj )
{
  ...
  tmp = ldv_kref_put_mutex_54(& obj->refcount, & drm_gem_object_free, & dev->struct_mutex);
  if (tmp != 0) {
    ldv_mutex_unlock_129(& dev->struct_mutex);
  } else {
    lock_acquire(& dev->struct_mutex.dep_map, 0U, 0, 0, 1, (struct lockdep_map *)0,
                 0UL);
    lock_release(& dev->struct_mutex.dep_map, 0, 0UL);
...
int ast_fbdev_init(struct drm_device *dev )
{
  ...
  __raw_spin_lock_init(& afbdev->dirty_lock.__annonCompField17.rlock, "&(&afbdev->dirty_lock)->rlock",
                       & __key);
...
int ldv_mutex_lock_interruptible_160(struct mutex *ldv_func_arg1 ) ;
...
int ldv_mutex_lock_interruptible_195(struct mutex *ldv_func_arg1 ) ;
...
int ldv_mutex_lock_interruptible_230(struct mutex *ldv_func_arg1 ) ;
