...
void reach_error() { ((void) sizeof ((0) ? 1 : 0), __extension__ ({ if (0) ; else __assert_fail ("0", "linux-4.2-rc1.tar.xz-43_2a-drivers--staging--lustre--lustre--obdecho--obdecho.ko-entry_point.cil.out.c", 3, __extension__ __PRETTY_FUNCTION__); })); }
struct va_list;
...
const struct cl_lock_slice *cl_lock_at(const struct cl_lock *arg0, const struct lu_device_type *arg1) {
  return ldv_malloc(sizeof(struct cl_lock_slice));
}
...
struct obd_export *class_conn2export(struct lustre_handle *arg0) {
  return ldv_malloc(sizeof(struct obd_export));
}
int __VERIFIER_nondet_int(void);
