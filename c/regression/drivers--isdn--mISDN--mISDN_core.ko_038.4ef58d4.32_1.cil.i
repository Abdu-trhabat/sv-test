...
void reach_error() { __assert_fail("0", "drivers--isdn--mISDN--mISDN_core.ko_038.4ef58d4.32_1.cil.i", 3, "reach_error"); }
...
{ 
  ...
  ret = -14;
  might_fault();
  tmp___2 = ldv__builtin_expect(! (! (sz == -1 || (unsigned long )sz >= n)), 1);
  if (tmp___2) {
    tmp___0 = _copy_from_user(to, from, n);
    ret = tmp___0;
  } else {
    __ret_warn_on = 1;
    tmp___1 = ldv__builtin_expect(! (! __ret_warn_on), 0);
    if (tmp___1) {
