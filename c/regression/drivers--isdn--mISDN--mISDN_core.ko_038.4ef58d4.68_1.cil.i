...
void reach_error() { __assert_fail("0", "drivers--isdn--mISDN--mISDN_core.ko_038.4ef58d4.68_1.cil.i", 3, "reach_error"); }
...
{ 
  ...
  while (1) {
    init_timer_key(& ft->tl, "&ft->tl", & __key___3);
    break;
  }
  ft->event = event;
  ft->arg = arg;
...
{ 
  ...
  while (1) {
    init_timer_key(& ft->tl, "&ft->tl", & __key___4);
    break;
  }
  ft->event = event;
  ft->arg = arg;
...
{ 
  ...
  while (1) {
    ...
    lockdep_init_map(& ch->workq.lockdep_map, "&ch->workq", & __key___8, 0);
...
{ 
  ...
  while (1) {
    __constr_expr_0.counter = 0;
    ch->workq.data = __constr_expr_0;
    lockdep_init_map(& ch->workq.lockdep_map, "&ch->workq", & __key___9, 0);
    INIT_LIST_HEAD(& ch->workq.entry);
    while (1) {
...
{ 
  ...
  err = 0;
  while (1) {
    _lock_kernel("mISDNStackd", "/work/ldvuser/novikov/work/current--X--drivers/isdn/mISDN/mISDN_core.ko--X--defaultlinux--X--68_1--X--cpachecker/linux/csd_deg_dscv/30/dscv_tempdir/dscv/ri/68_1/drivers/isdn/mISDN/stack.c",
                 208);
    break;
  }
  tmp = get_current();
  ...
  while (1) {
    _unlock_kernel("mISDNStackd", "/work/ldvuser/novikov/work/current--X--drivers/isdn/mISDN/mISDN_core.ko--X--defaultlinux--X--68_1--X--cpachecker/linux/csd_deg_dscv/30/dscv_tempdir/dscv/ri/68_1/drivers/isdn/mISDN/stack.c",
                   212);
    break;
  }
  if (*debug___2 & 32U) {
...
{ 
  ...
  while (1) {
    __rwlock_init(& newst->l1sock.lock, "&newst->l1sock.lock", & __key___11);
    break;
  }
  while (1) {
    __init_waitqueue_head(& newst->workq, & __key___12);
    break;
  }
  skb_queue_head_init___0(& newst->msgq);
  while (1) {
    __mutex_init(& newst->lmutex, "&newst->lmutex", & __key___13);
    break;
  }
  dev->D.st = newst;
...
{ 
  ...
  {
  while (1) {
    __spin_lock_init(& list->lock, "&list->lock", & __key___14);
    break;
  }
  __skb_queue_head_init(list);
  return;
...
{ 
  ...
  {
  while (1) {
    __spin_lock_init(& list->lock, "&list->lock", & __key___15);
    break;
  }
  __skb_queue_head_init(list);
  return;
...
{ 
  ...
  while (1) {
    __rwlock_init(& mgr->lock, "&mgr->lock", & __key___16);
    break;
  }
...
{ 
  ...
  while (1) {
    __spin_lock_init(& dev->lock, "&dev->lock", & __key___17);
    break;
  }
  dev->work = 0;
  while (1) {
    __init_waitqueue_head(& dev->wait, & __key___18);
    break;
  }
...
{ 
  ...
  if (! timeout) {
    ...
    while (1) {
      init_timer_key(& timer->tl, "&timer->tl", & __key___19);
      break;
    }
