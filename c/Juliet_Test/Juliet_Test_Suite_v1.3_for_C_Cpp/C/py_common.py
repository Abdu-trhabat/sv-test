    ...
    for btc in baseline_testcases:
        ...
        if result != None:
            ...
            print_with_timestamp(
                "Could not determine the functional variant in " + btc_file_name
            )
            exit(1)
...
def map_weakness_classes(file):
    ...
    for record in records:
        ...
        if cwe in dict.keys():
            ...
            print_with_timestamp(
                'WARNING: CWE "' + cwe + '" is assigned to more than 1 weakness class.'
            )
        else:
            dict[cwe] = [wclass]
...
def run_commands(commands, use_shell=False):
    ...
    subprocess.check_call(
        command, shell=use_shell, stderr=sys.stderr, stdout=sys.stdout
    )
...
def break_up_filename(file_name):
    ...
    if result == None:
        ...
        print_with_timestamp(
            'WARNING: file "'
            + file_name
            + '" is not going to be parsed into parts! (blank values will be used)'
        )
    else:
...
def move_testcase_to_split_directories(
    dir, functional_variants, testcase_files, file_count_limit
):
    """
    ...
    for func_var in functional_variants:
        ...
        if (
            func_var_testcase_files_count + number_of_files_in_subdir
        ) > file_count_limit:
            is_subdir_needed = True
        ...
        print_with_timestamp(
            'Moving the test cases for the following functional variant "'
            + func_var
            + '" to subdirecory "'
            + func_var_dir
            + '"'
