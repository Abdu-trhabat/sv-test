...
def create_batch_file(
    cwe, cflags, lflags, c_files_exist, cpp_files_exist, is_dir_split
):
    contents = ""
    contents += (
...
def help():
    ...
    sys.stderr.write(
        "   create_per_cwe_files.py (builds per CWE files for all testcases without debug flags)\n"
    )
    sys.stderr.write(
...
split_linker_flags = (
    '/I"..\\..\\..\\testcasesupport" /W3 /MT /GS /RTC1 /bigobj /EHsc /nologo'
)
    ...
    for dir in cwe_dirs:
        ...
        if is_dir_split:
            ...
            for sub_dir in cwe_sub_dirs:
                ...
                fcl = update_main_cpp_and_testcases_h.generate_calls_to_fxs(
                    testcase_files
                )
                update_main_cpp_and_testcases_h.update_main_cpp(
                    sub_dir, "main.cpp", fcl
                )
                update_main_cpp_and_testcases_h.update_testcases_h(
                    sub_dir, "testcases.h", fcl
                )
                ...
                if linux_testcase_exists:
                    ...
                    linux_fcl = (
                        update_main_cpp_and_testcases_h.generate_calls_to_linux_fxs(
                            testcase_files
                        )
                    )
                    update_main_cpp_and_testcases_h.update_main_cpp(
                    ...
                    py_common.print_with_timestamp(
                        "No Makefile created for "
                        + cwe
                        + ". All of the test cases are Windows-specific."
                    )
