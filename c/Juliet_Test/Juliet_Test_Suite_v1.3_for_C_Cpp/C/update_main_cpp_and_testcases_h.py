...
def build_list_of_primary_c_cpp_testcase_files(directory, testcaseregexes):
    ...
    for root, dirs, files in os.walk(directory):
        for name in files:
            ...
            result = re.search(
                py_common.get_primary_testcase_filename_regex(), name, re.IGNORECASE
            )
...
def generate_calls_to_fxs(testcase_files):
    ...
    for fullfilepath in testcase_files:
        ...
        if filename.endswith(".cpp"):
            ...
            fcl.cpp_bad_lines.append(
                '\tprintLine("Calling ' + root + "::" + bad + '");'
            )
            fcl.cpp_bad_lines.append("\t" + root + "::" + bad + "\n")
            fcl.cpp_h_bad_lines.append("\tnamespace " + root + " { void " + bad + "}\n")

            fcl.cpp_good_lines.append(
                '\tprintLine("Calling ' + root + "::" + good + '");'
            )
            fcl.cpp_good_lines.append("\t" + root + "::" + good + "\n")
            fcl.cpp_h_good_lines.append(
                "\tnamespace " + root + " { void " + good + "}\n"
            )
            ...
            result = re.search(
                "Template File: point-flaw-badonly", file_contents, re.IGNORECASE
            )
            if result == None:
            ...
            raise Exception(
                "filename ends with something we don't handle!: " + fullfilepath
            )
    ...
    for fullfilepath in testcase_files:
        ...
        if ("w32" not in filename) and ("wchar_t" not in filename):
            ...
            if filename.endswith(".cpp"):
                ...
                fcl.cpp_bad_lines.append("\t" + root + "::" + bad + "\n")
                fcl.cpp_h_bad_lines.append(
                    "\tnamespace " + root + " { void " + bad + "}\n"
                )
...
def update_main_cpp(file_path, main_dot_cpp, fcl):
    ...
    update_file(
        file_path,
        main_dot_cpp,
        auto_gen_c_good_tag_start,
        auto_gen_c_good_tag_end,
        fcl.c_good_lines,
    )
    update_file(
        file_path,
        main_dot_cpp,
        auto_gen_c_bad_tag_start,
        auto_gen_c_bad_tag_end,
        fcl.c_bad_lines,
    )
    update_file(
        file_path,
        main_dot_cpp,
        auto_gen_cpp_good_tag_start,
        auto_gen_cpp_good_tag_end,
        fcl.cpp_good_lines,
    )
    update_file(
        file_path,
        main_dot_cpp,
        auto_gen_cpp_bad_tag_start,
        auto_gen_cpp_bad_tag_end,
        fcl.cpp_bad_lines,
    )
...
def update_testcases_h(file_path, testcases_dot_h, fcl):
    ...
    update_file(
        ...
        testcases_dot_h,
        auto_gen_c_good_h_tag_start,
        auto_gen_c_good_h_tag_end,
        fcl.c_h_good_lines,
    )
    update_file(
        file_path,
        testcases_dot_h,
        auto_gen_c_bad_h_tag_start,
        auto_gen_c_bad_h_tag_end,
        fcl.c_h_bad_lines,
    )
    update_file(
        file_path,
        ...
        auto_gen_cpp_good_h_tag_start,
        auto_gen_cpp_good_h_tag_end,
        fcl.cpp_h_good_lines,
    )
    update_file(
        file_path,
        testcases_dot_h,
        auto_gen_cpp_bad_h_tag_start,
        auto_gen_cpp_bad_h_tag_end,
        fcl.cpp_h_bad_lines,
    )
...
def update_main_cpp_and_testcases_h(testcaseregexes):
    ...
    testcase_files = build_list_of_primary_c_cpp_testcase_files(
        testcase_location, testcaseregexes
    )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "-h":
            sys.stderr.write(
                "Usage: update-main_cpp_and_testcases_h.py (optional regexes of testcases)\nSuch as\n"
            )
