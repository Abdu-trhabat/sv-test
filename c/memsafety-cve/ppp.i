...
int dumpppp(unsigned char* buf, int bufSize)
{
  ...
  spkt.cnt = rpkt.cnt = 0;
  spkt.esc = rpkt.esc = 0;
  while ((c = (((pointer) < (bufSize)) ? (buf)[(pointer)++] : (-1))) != (-1)) {
    switch (c) {
    case 1:
    case 2:
      dir = c == 1 ? "sent" : "rcvd";
      pkt = c == 1 ? &spkt : &rpkt;
      n = (((pointer) < (bufSize)) ? (buf)[(pointer)++] : (-1));
      int tmp = (((pointer) < (bufSize)) ? (buf)[(pointer)++] : (-1));
      if (n == (-1) || tmp == (-1)) {
        return 1;
      }
      n = (n << 8) + tmp;
      for (; n > 0; --n) {
        c = (((pointer) < (bufSize)) ? (buf)[(pointer)++] : (-1));
        switch (c) {
        case (-1):
          printf("\nEOF\n");
          if (spkt.cnt > 0)
...
int main() {
  int size = getNumberInRange(128, 128 * 2);
